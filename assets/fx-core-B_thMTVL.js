...
vec4 markState(vec2 uv, float state, float prog, float strong, float t){
  ...
  return A;
}`;function w0(a,t,o,i){a.lineWidth=4*t;const n=i.arrow;n.line==="dash"?a.setLineDash([12*t*n.gap,10*n.gap]):n.line==="dot"?(a.setLineDash([.5,12*n.gap]),a.lineCap="round",a.lineWidth=5*t):a.setLineDash([]),o!=null&&n.line!=="solid"&&n.line!=="taper"&&(a.lineDashOffset=-o*40*n.speed)}function E0(a,t,o,i,n,l={}){const r=n.lut,u=n.arrow||{},f=u.w??1,S=u.speed??1,d=u.glow??1,h=l.pulse??1,e=o/256,c=e*(l.scale??1),s=t/2,v=i*.9*S%1,y=l.prog!=null?Math.max(0,Math.min(1,l.prog)):Math.min(1,v/.55),A=l.prog!=null?1:v>.88?(1-v)/.12:1;a.clearRect(0,0,t,o);const p=A*(.45+.55*h),M=o-24*e,k=58*e,R=M+(k-M)*y,m=(L,D)=>r(L).replace("rgb(","rgba(").replace(")",`,${D.toFixed(3)})`),w=1.1*c*f,x=13*c*f,b=42*c*(.7+.3*f),_=!l.noTip&&y>.28?Math.min(1,(y-.28)/.22)*b*.42:0,T=R+_,C=a.createLinearGradient(0,M,0,T);if(C.addColorStop(0,m(.55,0)),C.addColorStop(.18,m(.64,.3*p)),C.addColorStop(.4,m(.76,.8*p)),C.addColorStop(.65,m(.88,.98*p)),C.addColorStop(1,m(.97,p)),a.save(),a.filter=`blur(${7*c}px)`,a.globalAlpha=.55,a.fillStyle=C,a.beginPath(),a.moveTo(s-w,M),a.lineTo(s+w,M),a.lineTo(s+x*.95,T),a.lineTo(s-x*.95,T),a.closePath(),a.fill(),a.restore(),a.globalAlpha=1,a.fillStyle=C,a.beginPath(),a.moveTo(s-w/2,M),a.lineTo(s+w/2,M),a.lineTo(s+x/2,T),a.lineTo(s-x/2,T),a.closePath(),a.fill(),a.globalAlpha=p,y>.28&&!l.noTip){const L=b,D=Math.min(1,(y-.28)/.22)*p,P=R+L*.3;a.globalAlpha=D;const B={color:r(.95),glowColor:r(.85),glow:12*d};n.glyph&&(n.glyph(a,"LIFT_TIP",s,P,L,B)||n.glyph(a,"TIP_TRI",s,P,L*.93,B))||(a.strokeStyle=r(.95),a.lineWidth=13*c*f,a.lineCap="round",a.lineJoin="round",a.shadowColor=r(.9),a.shadowBlur=18*c*d,a.beginPath(),a.moveTo(s-26*c,P+14*c),a.lineTo(s,P-16*c),a.lineTo(s+26*c,P+14*c),a.stroke())}a.globalAlpha=1,a.shadowBlur=0}function x0(a,t,o,i,n,l,r={}){const u=l.lut,f=l.arrow||{},S=f.w??1,d=f.glow??1,h=o/256*(r.scale??1);a.clearRect(0,0,t,o);const e=i.map(([m,w])=>[m*t,w*o]);if(e.length<2)return;const c=48,s=[],v=m=>{if(e.length===2)return[e[0][0]+(e[1][0]-e[0][0])*m,e[0][1]+(e[1][1]-e[0][1])*m];const w=m*(e.length-1),x=Math.min(e.length-2,Math.floor(w)),b=w-x,_=e[Math.max(0,x-1)],T=e[x],C=e[x+1],L=e[Math.min(e.length-1,x+2)],D=(P,B,E,H)=>.5*(2*B+(-P+E)*b+(2*P-5*B+4*E-H)*b*b+(-P+3*B-3*E+H)*b*b*b);return[D(_[0],T[0],C[0],L[0]),D(_[1],T[1],C[1],L[1])]};for(let m=0;m<=c;m++)s.push(v(m/c));const y=Math.max(0,Math.min(1,r.prog!=null?r.prog:n*.55%1)),A=Math.max(1,Math.round(c*y)),p=r.alpha??1,M=r.tail??.22;a.lineCap="round";const k=42*h*(.7+.3*S);let R=A;if(y>.28){let m=0;const w=k*.42*Math.min(1,(y-.28)/.22);for(;R>1&&m<w;)m+=Math.hypot(s[R][0]-s[R-1][0],s[R][1]-s[R-1][1]),R--}for(const m of[0,1])for(let w=1;w<=R;w++){const x=w/A,b=Math.min(1,x/M),_=b*b*(3-2*b)*p;a.globalAlpha=m?_:_*.16*x,a.strokeStyle=u(.55+.42*x),a.lineWidth=(1.1+11.9*x)*h*S*(m?1:1.9),a.beginPath(),a.moveTo(s[w-1][0],s[w-1][1]),a.lineTo(s[w][0],s[w][1]),a.stroke()}if(y>.28){const m=s[Math.max(0,A-2)][0],w=s[Math.max(0,A-2)][1],x=Math.atan2(s[A][1]-w,s[A][0]-m)+Math.PI/2,b=s[A][0]-Math.sin(x)*k*.3,_=s[A][1]+Math.cos(x)*k*.3;a.save(),a.translate(b,_),a.rotate(x),a.globalAlpha=Math.min(1,(y-.28)/.22)*p;const T={color:u(.95),glowColor:u(.85),glow:12*d};l.glyph&&(l.glyph(a,"LIFT_TIP",0,0,k,T)||l.glyph(a,"TIP_TRI",0,0,k*.93,T))||(a.strokeStyle=u(.95),a.lineWidth=9*h*S,a.lineJoin="round",a.lineCap="round",a.beginPath(),a.moveTo(-18*h,12*h),a.lineTo(0,-14*h),a.lineTo(18*h,12*h),a.stroke()),a.restore()}a.globalAlpha=1}function S0(a,t,o,i,n,l){n=n||{};const r=l.lut,u=n.style||l.arrow.line,f=!!n.closed,S=[0];for(let c=1;c<t.length;c++)S.push(S[c-1]+Math.hypot(t[c][0]-t[c-1][0],t[c][1]-t[c-1][1]));const d=S[S.length-1]||1,h=c=>{c=(c%d+d)%d;let s=1;for(;s<S.length-1&&S[s]<c;)s++;const v=(c-S[s-1])/Math.max(1e-4,S[s]-S[s-1]);return[t[s-1][0]+(t[s][0]-t[s-1][0])*v,t[s-1][1]+(t[s][1]-t[s-1][1])*v,Math.atan2(t[s][1]-t[s-1][1],t[s][0]-t[s-1][0])]},e=l.arrow;if(u==="chevron"){const c=(26*i+8)*e.gap,s=Math.max(2,Math.floor(d/c));a.shadowColor=r(Math.min(1,e.heat+.2)),a.shadowBlur=8*i*e.glow;for(let v=0;v<s;v++){const y=v*c+o*42*e.speed%c;if(!f&&y>d-4)continue;const[A,p,M]=h(y),k=7.5*i,R=8.5*i,m=.45+.4*Math.sin(y/d*6.283-o*2.2*e.speed);a.strokeStyle=r(e.heat-.05+m*.3),a.lineWidth=3.2*i,a.lineJoin="round",a.lineCap="round",a.save(),a.translate(A,p),a.rotate(M),a.beginPath(),a.moveTo(-R*.5,-k),a.lineTo(R*.5,0),a.lineTo(-R*.5,k),a.stroke(),a.restore()}return!0}if(u==="comet"){const c=o*.35*e.speed%1*d,s=d*e.tail,v=Math.max(24,t.length*2);a.lineCap="round";for(let p=0;p<v;p++){const M=c-p/v*s,k=c-(p+1)/v*s;if(!f&&k<0)break;const R=1-p/v,[m,w]=h(M),[x,b]=h(k);!f&&Math.hypot(x-m,b-w)>d*.4||(a.globalAlpha=Math.pow(R,1.6),a.strokeStyle=r(Math.max(.05,e.heat-.2)+R*.55),a.lineWidth=(1.5+R*4.5)*i,R>.72?(a.shadowColor=r(Math.min(1,e.heat+.3)),a.shadowBlur=R*12*i*e.glow):a.shadowBlur=0,a.beginPath(),a.moveTo(m,w),a.lineTo(x,b),a.stroke())}a.globalAlpha=1,a.lineCap="butt",a.shadowBlur=0;const[y,A]=h(c);return a.fillStyle=rgba(W.ink,.95),a.shadowColor=r(.9),a.shadowBlur=16*i,a.beginPath(),a.arc(y,A,2.6*i,0,7),a.fill(),a.shadowBlur=0,!0}if(a.strokeStyle=n.color||r(e.heat),a.shadowColor=r(Math.min(1,e.heat+.15)),a.shadowBlur=(n.glow??8)*i*e.glow,u==="taper")for(let c=1;c<t.length;c++)a.lineWidth=(.5+c/t.length*4.5)*i,a.beginPath(),a.moveTo(t[c-1][0],t[c-1][1]),a.lineTo(t[c][0],t[c][1]),a.stroke();else w0(a,i,o,l),a.beginPath(),t.forEach(([c,s],v)=>v?a.lineTo(c,s):a.moveTo(c,s)),f&&a.closePath(),a.stroke();return a.setLineDash([]),a.lineCap="butt",a.lineDashOffset=0,a.shadowBlur=0,!0}function G0(a,t,o,i,n,l){const r=13*i.halo,u=l.lut;a.clearRect(0,0,t,t),a.lineJoin="round";const f=t/220,S=t/2,d=18*o.round*f,h=40*f,e=48*f,c=t-80*f,s=t-96*f,v=[],y=(p,M,k,R)=>{for(let m=0;m<=1;m+=.12)v.push([p+(k-p)*m,M+(R-M)*m])};y(h+d,e,h+c-d,e),y(h+c,e+d,h+c,e+s-d),y(h+c-d,e+s,h+d,e+s),y(h,e+s-d,h,e+d),a.shadowColor=u(.6),a.shadowBlur=r*.8;const A=4*l.arrow.w*f;if(l.arrow.line==="solid"?(a.setLineDash([10*o.dash*f,8*f]),a.lineDashOffset=-n*22*f,a.strokeStyle=u(.45),a.lineWidth=A,a.beginPath(),a.roundRect(h,e,c,s,d),a.stroke(),a.setLineDash([]),a.lineDashOffset=0):S0(a,v,n,l.arrow.w*f,{color:u(.45),closed:!0},l),o.prog!=null&&o.prog>.001){const p=[],M=(C,L,D,P,B)=>{for(let E=1;E<=B;E++)p.push([C+(D-C)*E/B,L+(P-L)*E/B])},k=(C,L,D,P,B)=>{for(let E=1;E<=B;E++){const H=D+(P-D)*E/B;p.push([C+d*Math.cos(H),L+d*Math.sin(H)])}},R=Math.PI/2,m=h+c,w=e+s,x=h+c/2;p.push([x,e]),M(x,e,m-d,e,8),k(m-d,e+d,-R,0,6),M(m,e+d,m,w-d,10),k(m-d,w-d,0,R,6),M(m-d,w,h+d,w,14),k(h+d,w-d,R,Math.PI,6),M(h,w-d,h,e+d,10),k(h+d,e+d,Math.PI,Math.PI+R,6),M(h+d,e,x,e,8);let b=0;for(let C=1;C<p.length;C++)b+=Math.hypot(p[C][0]-p[C-1][0],p[C][1]-p[C-1][1]);const _=b*Math.min(1,o.prog);a.save(),a.setLineDash([10*o.dash*f,8*f]),a.lineDashOffset=-(c/2-d)-n*22*f,a.strokeStyle=u(.9),a.lineWidth=A*1.3,a.lineCap="round",a.shadowColor=u(.92),a.shadowBlur=r*1.3,a.beginPath(),a.moveTo(p[0][0],p[0][1]);let T=0;for(let C=1;C<p.length&&T<_;C++){const L=Math.hypot(p[C][0]-p[C-1][0],p[C][1]-p[C-1][1]);if(T+L<=_)a.lineTo(p[C][0],p[C][1]),T+=L;else{const D=(_-T)/L;a.lineTo(p[C-1][0]+(p[C][0]-p[C-1][0])*D,p[C-1][1]+(p[C][1]-p[C-1][1])*D),T=_}}a.stroke(),a.setLineDash([]),a.restore()}o.feet>.05&&l.foot&&(l.foot(a,!1,S-16*o.feet*f,S+6*f,26*o.feet*f),l.foot(a,!0,S+16*o.feet*f,S+6*f,26*o.feet*f)),a.shadowBlur=0}function O0(a,t,o,i,n,l,r,u){const f=13*i.halo,S=4*l.arrow.w*(t/220),d=l.lut;a.clearRect(0,0,t,t),a.lineJoin="round";const h=t/220,e=r||[[45*h,130*h],[110*h,60*h],[175*h,110*h]],c=u??n*.5%1,s=Math.min(1,c*1.25)*(e.length-1),v=Math.min(e.length-1,Math.floor(s+.35));a.shadowColor=d(.7),a.shadowBlur=f;const y=w=>{const x=Math.max(0,Math.min(e.length-2,Math.floor(w))),b=w-x;return[e[x][0]+(e[x+1][0]-e[x][0])*b,e[x][1]+(e[x+1][1]-e[x][1])*b]},A=o.comet!=null?o.comet:1,p=o.tailLen!=null?o.tailLen:.5;if(a.save(),a.shadowBlur=0,a.globalAlpha=o.rail!=null?o.rail:.22,a.strokeStyle=d(.6),a.lineWidth=2.2*h,a.lineCap="round",a.setLineDash([.01,8*h]),a.beginPath(),e.forEach(([w,x],b)=>b?a.lineTo(w,x):a.moveTo(w,x)),a.stroke(),a.setLineDash([]),a.restore(),s>.02){const w=(x,b)=>d(x).replace("rgb(","rgba(").replace(")",`,${b.toFixed(3)})`);for(let x=0;x<26;x++){const b=x/25,_=s-p*b;if(_<=0)break;const T=y(_),C=4.7*h*A*(1-b*.75),L=C*2.1,D=(1-b)*(1-b)*.8,P=a.createRadialGradient(T[0],T[1],0,T[0],T[1],L);P.addColorStop(0,w(.55-.18*b,D)),P.addColorStop(.5,w(.5-.18*b,D*.35)),P.addColorStop(1,w(.45-.18*b,0)),a.save(),a.shadowBlur=0,a.globalAlpha=1,a.fillStyle=P,a.beginPath(),a.arc(T[0],T[1],L,0,Math.PI*2),a.fill(),a.restore()}if(s<e.length-1-.001&&s-Math.floor(s)>.03){const x=y(s);a.save(),a.fillStyle=d(.62),a.shadowColor=d(.8),a.shadowBlur=11*h*A,a.beginPath(),a.arc(x[0],x[1],5.6*h*A,0,Math.PI*2),a.fill(),a.fillStyle=d(.95),a.globalAlpha=.95,a.beginPath(),a.arc(x[0],x[1],2.1*h*A,0,Math.PI*2),a.fill(),a.restore()}}a.globalAlpha=1,a.lineCap="butt";const M=o.done!=null,k=o.done||0,R=o.vd||[],m=o.acc!=null?o.acc:e.length-1;e.forEach(([w,x],b)=>{const _=b<k,T=R[b]||null,C=!_&&b===v&&!M,L=C?1+Math.sin(n*6)*.14:1,D=12*o.node*L*h*(b===m?1.34:1);_&&T!=="miss"&&(a.shadowBlur=f*1.4,a.shadowColor=d(.5),a.fillStyle=d(T==="near"?.5:.36),a.beginPath(),a.arc(w,x,D*.88,0,Math.PI*2),a.fill()),_||(a.save(),a.translate(w,x),c0(a,d,D,C?.8:.5,C?.9:.5,S*.9,f),a.restore()),a.strokeStyle=T==="hit"?G.prism:T==="near"?G.sand:T==="miss"?W.lo:d(_?.62:C?.8:.45),a.lineWidth=S*(C?1.3:T?1.15:.9),a.shadowBlur=C?f*1.6:T&&T!=="miss"?f*1.3:f*.6,T&&T!=="miss"&&(a.shadowColor=T==="hit"?G.prism:G.sand),a.beginPath(),a.arc(w,x,D,0,Math.PI*2),a.stroke(),l.num&&(_&&T!=="miss"?(a.save(),a.globalCompositeOperation="destination-out",a.shadowBlur=0,l.num(a,String(b+1),w,x,16*o.numS*L*h,Math.round(14*o.numS*h)),a.restore()):(a.globalAlpha=b<=v?1:.6,l.num(a,String(b+1),w,x,16*o.numS*L*h,Math.round(14*o.numS*h)),a.globalAlpha=1))}),a.shadowBlur=0}function c0(a,t,o,i,n,l,r,u=1){if(o<=.6)return;const f=(c,s)=>t(c).replace("rgb(","rgba(").replace(")",`,${s})`),S=l*2.6*u,d=Math.max(.1,o-S),h=o+S,e=a.createRadialGradient(0,0,d,0,0,h);e.addColorStop(0,f(i-.05,0)),e.addColorStop(.5,f(i,n*.85)),e.addColorStop(1,f(i-.05,0)),a.globalAlpha=1,a.fillStyle=e,a.shadowBlur=0,a.beginPath(),a.arc(0,0,h,0,Math.PI*2),a.fill(),a.globalAlpha=Math.min(1,n*1.1),a.lineWidth=l*.85,a.strokeStyle=t(Math.min(.98,i+.12)),a.shadowColor=t(.88),a.shadowBlur=r*.6,a.beginPath(),a.arc(0,0,o,0,Math.PI*2),a.stroke(),a.shadowBlur=0}function U0(a,t,o,i,n,l,r){const u=l.lut,f=13*i.halo,S=t/220,d=t/2,h=l.arrow&&l.arrow.w||1,e=(m,w)=>u(m).replace("rgb(","rgba(").replace(")",`,${w})`);a.clearRect(0,0,t,t),a.lineJoin="round",a.lineCap="round";const c=(o.r!=null?o.r:.42)*t,s=c*(o.rt!=null?o.rt:.36),v=3.4*h*S,y=r!=null?Math.max(0,Math.min(1,r)):n*(o.tempo||.6)%1,A=Math.pow(y,1.6),p=Math.max(0,(y-.9)/.1);a.save(),a.translate(d,d);const M=(m,w,x,b=1)=>c0(a,u,m,w,x,v,f,b),k=a.createRadialGradient(0,0,0,0,0,s*1.08);k.addColorStop(0,e(.6,.1+.18*p)),k.addColorStop(.65,e(.5,.05+.08*p)),k.addColorStop(1,e(.5,0)),a.globalAlpha=1,a.fillStyle=k,a.beginPath(),a.arc(0,0,s*1.08,0,Math.PI*2),a.fill();const R=1+.02*Math.sin(n*2.6);M(s*R,.55+.4*p,.5+.45*p,.9);for(let m=2;m>=0;m--){const w=Math.pow(Math.max(0,y-m*.05),1.6),x=c-(c-s)*w,b=m===0?.6+.4*A:.18/m*(1-p);M(x,.55+.4*A,b*(1-p*.45),1.15-.35*A)}p>.01&&M(s*(1+1.4*p),.9,(1-p)*.8,1.1),a.globalAlpha=.6+.3*p,a.shadowColor=u(.85),a.shadowBlur=f*(.9+p),a.fillStyle=u(.62+.3*p),a.beginPath(),a.arc(0,0,v*.85+3*S*p,0,Math.PI*2),a.fill(),a.restore(),a.globalAlpha=1,a.shadowBlur=0}function H0(a,t,o,i,n,l,r,u){const f=l.lut,S=13*i.halo,d=t/220,h=t/2,e=l.arrow&&l.arrow.w||1,c=e*d,s=(I,F)=>f(I).replace("rgb(","rgba(").replace(")",","+F+")");a.clearRect(0,0,t,t),a.lineJoin="round",a.lineCap="round";const v=t*.42*(o.spread!=null?o.spread:1),A=(u||[[-.95,.5],[-.48,-.42],[0,-.8],[.48,-.42],[.95,.5]]).map(([I,F])=>[h+I*v,h+F*v]),p=80,M=[];for(let I=0;I<=p;I++){const F=I/p*(A.length-1),O=Math.min(A.length-2,Math.floor(F)),U=F-O,q=A[Math.max(0,O-1)],$=A[O],N=A[O+1],a0=A[Math.min(A.length-1,O+2)],t0=(X,J,Y,o0)=>.5*(2*J+(-X+Y)*U+(2*X-5*J+4*Y-o0)*U*U+(-X+3*J-3*Y+o0)*U*U*U);M.push([t0(q[0],$[0],N[0],a0[0]),t0(q[1],$[1],N[1],a0[1])])}const k=I=>{const F=Math.max(0,Math.min(p,I*p)),O=Math.floor(F),U=F-O,q=M[O],$=M[Math.min(p,O+1)];return[q[0]+($[0]-q[0])*U,q[1]+($[1]-q[1])*U]},R=.68;let m,w,x;if(r!=null)m=Math.max(0,Math.min(1,r)),w=1,x=0;else{const I=n*(o.tempo||.42)%1;if(I<R)m=I/R,w=1,x=0;else{const F=(I-R)/(1-R);m=1,w=1-F*F,x=F}}if(w<=.012)return;const b=m*m*m*(m*(6*m-15)+10),_=Math.min(1,16*m*m*(1-m)*(1-m));o.taper!=null&&o.taper;const T=.36*(o.tail!=null?o.tail:1),C=o.width!=null?o.width:1;{const I=a.createLinearGradient(M[0][0],M[0][1],M[p][0],M[p][1]);I.addColorStop(0,s(.46,0)),I.addColorStop(.3,s(.46,.03*w)),I.addColorStop(.8,s(.46,.045*w)),I.addColorStop(1,s(.46,0)),a.globalAlpha=1,a.strokeStyle=I,a.lineWidth=9*c,a.shadowColor=f(.6),a.shadowBlur=S*2,a.beginPath(),M.forEach(([F,O],U)=>U?a.lineTo(F,O):a.moveTo(F,O)),a.stroke(),a.shadowBlur=0}const L=40,D=Math.max(0,b-T*(1-x)),P=[];for(let I=0;I<=L;I++)P.push(k(D+(b-D)*(I/L)));const B=()=>{a.beginPath(),P.forEach(([I,F],O)=>O?a.lineTo(I,F):a.moveTo(I,F)),a.stroke()},E=()=>{const I=a.createLinearGradient(P[0][0],P[0][1],P[L][0],P[L][1]);return I.addColorStop(0,s(.55,0)),I.addColorStop(.4,s(.56,0)),I.addColorStop(.68,s(.6,.09)),I.addColorStop(.88,s(.64,.24)),I.addColorStop(1,s(.68,.44)),I},H=1+.5*_;a.globalAlpha=w,a.strokeStyle=E(),a.lineWidth=(20+10*_)*c*C,a.shadowColor=f(.72),a.shadowBlur=S*2.2,B(),a.strokeStyle=E(),a.lineWidth=(10+5*_)*c*C,a.shadowBlur=S*1,B(),a.shadowBlur=0;for(let I=1;I<=L;I++){const F=I/L;a.globalAlpha=Math.pow(F,2.2)*.95*w,a.strokeStyle=f(.55+.38*F),a.lineWidth=(1.6+6.5*Math.pow(F,.7))*c*C*H,a.beginPath(),a.moveTo(P[I-1][0],P[I-1][1]),a.lineTo(P[I][0],P[I][1]),a.stroke()}const V=P[L][0],g=P[L][1];a.globalAlpha=.8*w,a.fillStyle=f(.6),a.shadowColor=f(.8),a.shadowBlur=S*1.6,a.beginPath(),a.arc(V,g,(9+5*_)*c*C,0,Math.PI*2),a.fill(),a.globalAlpha=w,a.fillStyle=f(.93),a.shadowBlur=S*.6,a.beginPath(),a.arc(V,g,(3.4+1.8*_)*c*C,0,Math.PI*2),a.fill(),a.globalAlpha=1,a.shadowBlur=0}function q0(a,t,o,i,n,l,r){const u=o.r!=null?o.r:.3,f=o.dir!=null?o.dir:1,S=(o.sweep!=null?o.sweep:.66)*Math.PI*2,d=r??n*(o.tempo!=null?o.tempo:.5)%1,h=-Math.PI/2+f*d*Math.PI*2,e=16,c=[];for(let s=0;s<=e;s++){const v=h-f*(1-s/e)*S;c.push([.5+Math.cos(v)*u,.5+Math.sin(v)*u])}x0(a,t,t,c,n,l,{prog:1,tail:o.tail!=null?o.tail:.68,scale:(o.scale!=null?o.scale:1)*(o.width!=null?o.width:1)})}export{D0 as C,p0 as G,P0 as M,W as N,G as P,M0 as Q,_0 as R,T0 as S,R0 as Z,j as a,h0 as b,U0 as c,y0 as d,q0 as e,E0 as f,H0 as g,O0 as h,L0 as i,A0 as j,I0 as k,G0 as l,k0 as m,B0 as n,z as o,S0 as p,C0 as q,x0 as r,i0 as s,b0 as t,v0 as u,F0 as v,u0 as w,d0 as x};
