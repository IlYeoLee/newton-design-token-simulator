...
vec3 personLook(float thick, float lumS, float lumB, float mIn, float face, float vTop){
  ...
  lumS *= kExp; lumB *= kExp;
  if (uPForm > 0.5) {
    // ★ 깊이(두께장) 기반은 **폐기**했다. thick 은 블러된 실루엣이라 몸통 안쪽이 전부 1.0 에
    //   포화한다 — 아래 uPSweep 주석에 이미 적혀 있던 사실인데 두 번 무시했다. 깊이를 T 로 쓰면
    //   전신이 한 색으로 뭉개진 **평면 빨강 실루엣**이 된다(유저 스샷 08-01). 방향을 뒤집어도
    //   같다: 포화한 필드에는 뒤집을 계조 자체가 없다.
    //   레퍼런스(public/refs/person)의 계조는 깊이가 아니라 **소스 휘도**에서 온다 —
    //   밝은 머리카락·팔뚝이 뽀얗게 뜨고 어두운 옷이 진한 코랄로 앉는다. 그러니 듀오톤의
    //   축은 휘도다. 현행 경로와의 차이는 P_ABS(0.26)로 억눌러 둔 **절대 휘도를 전면에 쓴다**는
    //   것이고, 클립별 노출차는 위 kExp(uPExp)가 이미 상쇄한다.
    float lum = mix(mix(lumS, lumB, 0.35), lumB, face);
    vec3 c = personColor(clamp(smoothstep(0.06, 0.95, lum), 0.0, 1.0));
    // 경계의 부드러움 = 바깥 얇은 층만 우유빛으로 태운다(어둡게가 아니라 밝게).
    //   thick 이 포화하지 **않는** 유일한 구간이 여기라 가장자리에만 걸린다.
    float e = 1.0 - smoothstep(0.15, 0.78, clamp(thick, 0.0, 1.0));
    return clamp(mix(c, vec3(1.0, 0.97, 0.94), clamp(e * 0.8 + face * 0.55, 0.0, 1.0)), 0.0, 1.0);
  }
...
vec4 markState(vec2 uv, float state, float prog, float strong, float t){
  ...
  return A;
}`;function w0(t,a,o,i){t.lineWidth=4*a;const s=i.arrow;s.line==="dash"?t.setLineDash([12*a*s.gap,10*s.gap]):s.line==="dot"?(t.setLineDash([.5,12*s.gap]),t.lineCap="round",t.lineWidth=5*a):t.setLineDash([]),o!=null&&s.line!=="solid"&&s.line!=="taper"&&(t.lineDashOffset=-o*40*s.speed)}function F0(t,a,o,i,s,l={}){const c=s.lut,r=s.arrow||{},d=r.w??1,m=r.speed??1,f=r.glow??1,p=l.pulse??1,e=o/256,h=a/2,u=i*.9*m%1,C=l.prog!=null?Math.max(0,Math.min(1,l.prog)):Math.min(1,u/.55),v=l.prog!=null?1:u>.88?(1-u)/.12:1;t.clearRect(0,0,a,o);const w=v*(.45+.55*p),n=o-24*e,S=58*e,M=n+(S-n)*C,T=(_,k)=>c(_).replace("rgb(","rgba(").replace(")",`,${k.toFixed(3)})`),x=1.1*e*d,I=13*e*d,R=t.createLinearGradient(0,n,0,M);if(R.addColorStop(0,T(.55,0)),R.addColorStop(.1,T(.64,.45*w)),R.addColorStop(.32,T(.76,.85*w)),R.addColorStop(.62,T(.88,.98*w)),R.addColorStop(1,T(.97,w)),t.globalAlpha=1,t.fillStyle=R,t.beginPath(),t.moveTo(h-x/2,n),t.lineTo(h+x/2,n),t.lineTo(h+I/2,M),t.lineTo(h-I/2,M),t.closePath(),t.fill(),t.globalAlpha=w,C>.28&&!l.noTip){const _=34*e*(.7+.3*d),k=Math.min(1,(C-.28)/.22)*w,y=M+_*.3;t.globalAlpha=k;const A={color:c(.95),glowColor:c(.85),glow:12*f};s.glyph&&(s.glyph(t,"LIFT_TIP",h,y,_,A)||s.glyph(t,"TIP_TRI",h,y,_*.93,A))||(t.strokeStyle=c(.95),t.lineWidth=13*e*d,t.lineCap="round",t.lineJoin="round",t.shadowColor=c(.9),t.shadowBlur=18*e*f,t.beginPath(),t.moveTo(h-26*e,y+14*e),t.lineTo(h,y-16*e),t.lineTo(h+26*e,y+14*e),t.stroke())}t.globalAlpha=1,t.shadowBlur=0}function B0(t,a,o,i,s,l,c={}){const r=l.lut,d=l.arrow||{},m=d.w??1,f=d.glow??1,p=o/256;t.clearRect(0,0,a,o);const e=i.map(([n,S])=>[n*a,S*o]);if(e.length<2)return;const h=48,u=[],C=n=>{if(e.length===2)return[e[0][0]+(e[1][0]-e[0][0])*n,e[0][1]+(e[1][1]-e[0][1])*n];const S=n*(e.length-1),M=Math.min(e.length-2,Math.floor(S)),T=S-M,x=e[Math.max(0,M-1)],I=e[M],R=e[M+1],_=e[Math.min(e.length-1,M+2)],k=(y,A,P,B)=>.5*(2*A+(-y+P)*T+(2*y-5*A+4*P-B)*T*T+(-y+3*A-3*P+B)*T*T*T);return[k(x[0],I[0],R[0],_[0]),k(x[1],I[1],R[1],_[1])]};for(let n=0;n<=h;n++)u.push(C(n/h));const v=Math.max(0,Math.min(1,c.prog!=null?c.prog:s*.55%1)),w=Math.max(1,Math.round(h*v));t.lineCap="round";for(let n=1;n<=w;n++){const S=n/w;t.globalAlpha=Math.pow(S,1.5),t.strokeStyle=r(.45+.5*S),t.lineWidth=(1.6+3.2*S)*p*m,t.beginPath(),t.moveTo(u[n-1][0],u[n-1][1]),t.lineTo(u[n][0],u[n][1]),t.stroke()}if(v>.25){const n=u[w][0],S=u[w][1],M=u[Math.max(0,w-2)][0],T=u[Math.max(0,w-2)][1],x=Math.atan2(S-T,n-M)+Math.PI/2,I=30*p*(.7+.3*m);t.save(),t.translate(n,S),t.rotate(x),t.globalAlpha=Math.min(1,(v-.25)/.2);const R={color:r(.95),glowColor:r(.85),glow:12*f};l.glyph&&(l.glyph(t,"LIFT_TIP",0,0,I,R)||l.glyph(t,"TIP_TRI",0,0,I*.93,R))||(t.strokeStyle=r(.95),t.lineWidth=9*p*m,t.lineJoin="round",t.lineCap="round",t.beginPath(),t.moveTo(-18*p,12*p),t.lineTo(0,-14*p),t.lineTo(18*p,12*p),t.stroke()),t.restore()}t.globalAlpha=1}function c0(t,a,o,i,s,l){s=s||{};const c=l.lut,r=s.style||l.arrow.line,d=!!s.closed,m=[0];for(let h=1;h<a.length;h++)m.push(m[h-1]+Math.hypot(a[h][0]-a[h-1][0],a[h][1]-a[h-1][1]));const f=m[m.length-1]||1,p=h=>{h=(h%f+f)%f;let u=1;for(;u<m.length-1&&m[u]<h;)u++;const C=(h-m[u-1])/Math.max(1e-4,m[u]-m[u-1]);return[a[u-1][0]+(a[u][0]-a[u-1][0])*C,a[u-1][1]+(a[u][1]-a[u-1][1])*C,Math.atan2(a[u][1]-a[u-1][1],a[u][0]-a[u-1][0])]},e=l.arrow;if(r==="chevron"){const h=(26*i+8)*e.gap,u=Math.max(2,Math.floor(f/h));t.shadowColor=c(Math.min(1,e.heat+.2)),t.shadowBlur=8*i*e.glow;for(let C=0;C<u;C++){const v=C*h+o*42*e.speed%h;if(!d&&v>f-4)continue;const[w,n,S]=p(v),M=7.5*i,T=8.5*i,x=.45+.4*Math.sin(v/f*6.283-o*2.2*e.speed);t.strokeStyle=c(e.heat-.05+x*.3),t.lineWidth=3.2*i,t.lineJoin="round",t.lineCap="round",t.save(),t.translate(w,n),t.rotate(S),t.beginPath(),t.moveTo(-T*.5,-M),t.lineTo(T*.5,0),t.lineTo(-T*.5,M),t.stroke(),t.restore()}return!0}if(r==="comet"){const h=o*.35*e.speed%1*f,u=f*e.tail,C=Math.max(24,a.length*2);t.lineCap="round";for(let n=0;n<C;n++){const S=h-n/C*u,M=h-(n+1)/C*u;if(!d&&M<0)break;const T=1-n/C,[x,I]=p(S),[R,_]=p(M);!d&&Math.hypot(R-x,_-I)>f*.4||(t.globalAlpha=Math.pow(T,1.6),t.strokeStyle=c(Math.max(.05,e.heat-.2)+T*.55),t.lineWidth=(1.5+T*4.5)*i,T>.72?(t.shadowColor=c(Math.min(1,e.heat+.3)),t.shadowBlur=T*12*i*e.glow):t.shadowBlur=0,t.beginPath(),t.moveTo(x,I),t.lineTo(R,_),t.stroke())}t.globalAlpha=1,t.lineCap="butt",t.shadowBlur=0;const[v,w]=p(h);return t.fillStyle=rgba(W.ink,.95),t.shadowColor=c(.9),t.shadowBlur=16*i,t.beginPath(),t.arc(v,w,2.6*i,0,7),t.fill(),t.shadowBlur=0,!0}if(t.strokeStyle=s.color||c(e.heat),t.shadowColor=c(Math.min(1,e.heat+.15)),t.shadowBlur=(s.glow??8)*i*e.glow,r==="taper")for(let h=1;h<a.length;h++)t.lineWidth=(.5+h/a.length*4.5)*i,t.beginPath(),t.moveTo(a[h-1][0],a[h-1][1]),t.lineTo(a[h][0],a[h][1]),t.stroke();else w0(t,i,o,l),t.beginPath(),a.forEach(([h,u],C)=>C?t.lineTo(h,u):t.moveTo(h,u)),d&&t.closePath(),t.stroke();return t.setLineDash([]),t.lineCap="butt",t.lineDashOffset=0,t.shadowBlur=0,!0}function E0(t,a,o,i,s,l){const c=13*i.halo,r=l.lut;t.clearRect(0,0,a,a),t.lineJoin="round";const d=a/220,m=a/2,f=18*o.round*d,p=40*d,e=48*d,h=a-80*d,u=a-96*d,C=[],v=(n,S,M,T)=>{for(let x=0;x<=1;x+=.12)C.push([n+(M-n)*x,S+(T-S)*x])};v(p+f,e,p+h-f,e),v(p+h,e+f,p+h,e+u-f),v(p+h-f,e+u,p+f,e+u),v(p,e+u-f,p,e+f),t.shadowColor=r(.6),t.shadowBlur=c*.8;const w=4*l.arrow.w*d;if(l.arrow.line==="solid"?(t.setLineDash([10*o.dash*d,8*d]),t.lineDashOffset=-s*22*d,t.strokeStyle=r(.45),t.lineWidth=w,t.beginPath(),t.roundRect(p,e,h,u,f),t.stroke(),t.setLineDash([]),t.lineDashOffset=0):c0(t,C,s,l.arrow.w*d,{color:r(.45),closed:!0},l),o.prog!=null&&o.prog>.001){const n=[],S=(A,P,B,D,G)=>{for(let O=1;O<=G;O++)n.push([A+(B-A)*O/G,P+(D-P)*O/G])},M=(A,P,B,D,G)=>{for(let O=1;O<=G;O++){const j=B+(D-B)*O/G;n.push([A+f*Math.cos(j),P+f*Math.sin(j)])}},T=Math.PI/2,x=p+h,I=e+u,R=p+h/2;n.push([R,e]),S(R,e,x-f,e,8),M(x-f,e+f,-T,0,6),S(x,e+f,x,I-f,10),M(x-f,I-f,0,T,6),S(x-f,I,p+f,I,14),M(p+f,I-f,T,Math.PI,6),S(p,I-f,p,e+f,10),M(p+f,e+f,Math.PI,Math.PI+T,6),S(p+f,e,R,e,8);let _=0;for(let A=1;A<n.length;A++)_+=Math.hypot(n[A][0]-n[A-1][0],n[A][1]-n[A-1][1]);const k=_*Math.min(1,o.prog);t.save(),t.setLineDash([10*o.dash*d,8*d]),t.lineDashOffset=-(h/2-f)-s*22*d,t.strokeStyle=r(.9),t.lineWidth=w*1.3,t.lineCap="round",t.shadowColor=r(.92),t.shadowBlur=c*1.3,t.beginPath(),t.moveTo(n[0][0],n[0][1]);let y=0;for(let A=1;A<n.length&&y<k;A++){const P=Math.hypot(n[A][0]-n[A-1][0],n[A][1]-n[A-1][1]);if(y+P<=k)t.lineTo(n[A][0],n[A][1]),y+=P;else{const B=(k-y)/P;t.lineTo(n[A-1][0]+(n[A][0]-n[A-1][0])*B,n[A-1][1]+(n[A][1]-n[A-1][1])*B),y=k}}t.stroke(),t.setLineDash([]),t.restore()}o.feet>.05&&l.foot&&(l.foot(t,!1,m-16*o.feet*d,m+6*d,26*o.feet*d),l.foot(t,!0,m+16*o.feet*d,m+6*d,26*o.feet*d)),t.shadowBlur=0}function O0(t,a,o,i,s,l,c,r){const d=13*i.halo,m=4*l.arrow.w*(a/220),f=l.lut;t.clearRect(0,0,a,a),t.lineJoin="round";const p=a/220,e=c||[[45*p,130*p],[110*p,60*p],[175*p,110*p]],h=r??s*.5%1,u=Math.min(1,h*1.25)*(e.length-1),C=Math.min(e.length-1,Math.floor(u+.35));t.shadowColor=f(.7),t.shadowBlur=d;const v=[[e[0][0],e[0][1]]];for(let w=1;w<=e.length-1;w++){const n=Math.max(0,Math.min(1,u-(w-1)));if(n<=0)break;v.push([e[w-1][0]+(e[w][0]-e[w-1][0])*n,e[w-1][1]+(e[w][1]-e[w-1][1])*n])}v.length>1&&c0(t,v,s,l.arrow.w*p,{color:f(.62)},l),t.setLineDash([4*p,7*p]),t.lineDashOffset=0,t.globalAlpha=.3,t.strokeStyle=f(.45),t.lineWidth=m,t.beginPath(),e.forEach(([w,n],S)=>S?t.lineTo(w,n):t.moveTo(w,n)),t.stroke(),t.globalAlpha=1,t.setLineDash([]),t.lineCap="butt",t.lineDashOffset=0,e.forEach(([w,n],S)=>{const M=S===C,T=M?1+Math.sin(s*6)*.14:1;t.strokeStyle=f(M?.8:.45),t.lineWidth=m*(M?1.3:.9),t.shadowBlur=M?d*1.6:d*.6,t.beginPath(),t.arc(w,n,12*o.node*T*p,0,Math.PI*2),t.stroke(),l.num&&(t.globalAlpha=S<=C?1:.45,l.num(t,String(S+1),w,n,16*o.numS*T*p,Math.round(14*o.numS*p)),t.globalAlpha=1)}),t.shadowBlur=0}function U0(t,a,o,i,s,l,c){const r=l.lut,d=13*i.halo,m=a/220,f=a/2,p=l.arrow&&l.arrow.w||1,e=(x,I)=>r(x).replace("rgb(","rgba(").replace(")",`,${I})`);t.clearRect(0,0,a,a),t.lineJoin="round",t.lineCap="round";const h=(o.r!=null?o.r:.42)*a,u=h*(o.rt!=null?o.rt:.36),C=3.4*p*m,v=c!=null?Math.max(0,Math.min(1,c)):s*(o.tempo||.6)%1,w=Math.pow(v,1.6),n=Math.max(0,(v-.9)/.1);t.save(),t.translate(f,f);const S=(x,I,R,_=1)=>{if(x<=.6)return;const k=C*2.6*_,y=Math.max(.1,x-k),A=x+k,P=t.createRadialGradient(0,0,y,0,0,A);P.addColorStop(0,e(I-.05,0)),P.addColorStop(.5,e(I,R*.85)),P.addColorStop(1,e(I-.05,0)),t.globalAlpha=1,t.fillStyle=P,t.shadowBlur=0,t.beginPath(),t.arc(0,0,A,0,Math.PI*2),t.fill(),t.globalAlpha=Math.min(1,R*1.1),t.lineWidth=C*.85,t.strokeStyle=r(Math.min(.98,I+.12)),t.shadowColor=r(.88),t.shadowBlur=d*.6,t.beginPath(),t.arc(0,0,x,0,Math.PI*2),t.stroke(),t.shadowBlur=0},M=t.createRadialGradient(0,0,0,0,0,u*1.08);M.addColorStop(0,e(.6,.1+.18*n)),M.addColorStop(.65,e(.5,.05+.08*n)),M.addColorStop(1,e(.5,0)),t.globalAlpha=1,t.fillStyle=M,t.beginPath(),t.arc(0,0,u*1.08,0,Math.PI*2),t.fill();const T=1+.02*Math.sin(s*2.6);S(u*T,.55+.4*n,.5+.45*n,.9);for(let x=2;x>=0;x--){const I=Math.pow(Math.max(0,v-x*.05),1.6),R=h-(h-u)*I,_=x===0?.6+.4*w:.18/x*(1-n);S(R,.55+.4*w,_*(1-n*.45),1.15-.35*w)}n>.01&&S(u*(1+1.4*n),.9,(1-n)*.8,1.1),t.globalAlpha=.6+.3*n,t.shadowColor=r(.85),t.shadowBlur=d*(.9+n),t.fillStyle=r(.62+.3*n),t.beginPath(),t.arc(0,0,C*.85+3*m*n,0,Math.PI*2),t.fill(),t.restore(),t.globalAlpha=1,t.shadowBlur=0}function G0(t,a,o,i,s,l,c,r){const d=l.lut,m=13*i.halo,f=a/220,p=a/2,e=l.arrow&&l.arrow.w||1,h=e*f,u=(b,L)=>d(b).replace("rgb(","rgba(").replace(")",","+L+")");t.clearRect(0,0,a,a),t.lineJoin="round",t.lineCap="round";const C=a*.42*(o.spread!=null?o.spread:1),w=(r||[[-.95,.5],[-.48,-.42],[0,-.8],[.48,-.42],[.95,.5]]).map(([b,L])=>[p+b*C,p+L*C]),n=80,S=[];for(let b=0;b<=n;b++){const L=b/n*(w.length-1),F=Math.min(w.length-2,Math.floor(L)),U=L-F,H=w[Math.max(0,F-1)],K=w[F],N=w[F+1],t0=w[Math.min(w.length-1,F+2)],a0=(X,J,Y,o0)=>.5*(2*J+(-X+Y)*U+(2*X-5*J+4*Y-o0)*U*U+(-X+3*J-3*Y+o0)*U*U*U);S.push([a0(H[0],K[0],N[0],t0[0]),a0(H[1],K[1],N[1],t0[1])])}const M=b=>{const L=Math.max(0,Math.min(n,b*n)),F=Math.floor(L),U=L-F,H=S[F],K=S[Math.min(n,F+1)];return[H[0]+(K[0]-H[0])*U,H[1]+(K[1]-H[1])*U]},T=.68;let x,I,R;if(c!=null)x=Math.max(0,Math.min(1,c)),I=1,R=0;else{const b=s*(o.tempo||.42)%1;if(b<T)x=b/T,I=1,R=0;else{const L=(b-T)/(1-T);x=1,I=1-L*L,R=L}}if(I<=.012)return;const _=x*x*x*(x*(6*x-15)+10),k=Math.min(1,16*x*x*(1-x)*(1-x));o.taper!=null&&o.taper;const y=.36*(o.tail!=null?o.tail:1),A=o.width!=null?o.width:1;{const b=t.createLinearGradient(S[0][0],S[0][1],S[n][0],S[n][1]);b.addColorStop(0,u(.46,0)),b.addColorStop(.3,u(.46,.03*I)),b.addColorStop(.8,u(.46,.045*I)),b.addColorStop(1,u(.46,0)),t.globalAlpha=1,t.strokeStyle=b,t.lineWidth=9*h,t.shadowColor=d(.6),t.shadowBlur=m*2,t.beginPath(),S.forEach(([L,F],U)=>U?t.lineTo(L,F):t.moveTo(L,F)),t.stroke(),t.shadowBlur=0}const P=40,B=Math.max(0,_-y*(1-R)),D=[];for(let b=0;b<=P;b++)D.push(M(B+(_-B)*(b/P)));const G=()=>{t.beginPath(),D.forEach(([b,L],F)=>F?t.lineTo(b,L):t.moveTo(b,L)),t.stroke()},O=()=>{const b=t.createLinearGradient(D[0][0],D[0][1],D[P][0],D[P][1]);return b.addColorStop(0,u(.55,0)),b.addColorStop(.4,u(.56,0)),b.addColorStop(.68,u(.6,.09)),b.addColorStop(.88,u(.64,.24)),b.addColorStop(1,u(.68,.44)),b},j=1+.5*k;t.globalAlpha=I,t.strokeStyle=O(),t.lineWidth=(20+10*k)*h*A,t.shadowColor=d(.72),t.shadowBlur=m*2.2,G(),t.strokeStyle=O(),t.lineWidth=(10+5*k)*h*A,t.shadowBlur=m*1,G(),t.shadowBlur=0;for(let b=1;b<=P;b++){const L=b/P;t.globalAlpha=Math.pow(L,2.2)*.95*I,t.strokeStyle=d(.55+.38*L),t.lineWidth=(1.6+6.5*Math.pow(L,.7))*h*A*j,t.beginPath(),t.moveTo(D[b-1][0],D[b-1][1]),t.lineTo(D[b][0],D[b][1]),t.stroke()}const V=D[P][0],g=D[P][1];t.globalAlpha=.8*I,t.fillStyle=d(.6),t.shadowColor=d(.8),t.shadowBlur=m*1.6,t.beginPath(),t.arc(V,g,(9+5*k)*h*A,0,Math.PI*2),t.fill(),t.globalAlpha=I,t.fillStyle=d(.93),t.shadowBlur=m*.6,t.beginPath(),t.arc(V,g,(3.4+1.8*k)*h*A,0,Math.PI*2),t.fill(),t.globalAlpha=1,t.shadowBlur=0}function H0(t,a,o,i,s,l,c){const r=l.lut,d=13*i.halo,m=a/220,f=a/2,p=l.arrow&&l.arrow.w||1;t.clearRect(0,0,a,a),t.lineJoin="round",t.lineCap="round";const e=(o.r!=null?o.r:.3)*a,h=o.width!=null?o.width:1,u=4.2*p*m*h,C=o.dir!=null?o.dir:1,v=(o.sweep!=null?o.sweep:.66)*Math.PI*2,w=c!=null?Math.max(0,Math.min(1,c)):s*(o.tempo||.5)%1,n=-Math.PI/2+C*w*Math.PI*2;t.save(),t.translate(f,f),t.globalAlpha=.16,t.lineWidth=u*.7,t.strokeStyle=r(.44),t.shadowColor=r(.6),t.shadowBlur=d*.4,t.beginPath(),t.arc(0,0,e,0,Math.PI*2),t.stroke(),t.shadowBlur=0;const S=16;for(let k=0;k<S;k++){const y=k/(S-1),A=n-C*y*v,P=n-C*(y+1.2/S)*v;t.globalAlpha=(1-y)*.9,t.strokeStyle=r(.55+.35*(1-y)),t.lineWidth=u*(.55+.55*(1-y)),t.shadowColor=r(.8),t.shadowBlur=d*(.4+.5*(1-y)),t.beginPath(),t.arc(0,0,e,Math.min(A,P),Math.max(A,P),!1),t.stroke()}t.shadowBlur=0;const M=Math.cos(n)*e,T=Math.sin(n)*e,x=n+C*Math.PI/2,I=8*m*h;t.save(),t.translate(M,T),t.rotate(x+Math.PI/2),t.globalAlpha=1;const R=3.4*I*(.7+.3*p),_={color:r(.96),glowColor:r(.9),glow:d*1.2};l.glyph&&(l.glyph(t,"LIFT_TIP",0,0,R,_)||l.glyph(t,"TIP_TRI",0,0,R*.93,_))||(t.rotate(-Math.PI/2),t.strokeStyle=r(.96),t.lineWidth=u*.9,t.shadowColor=r(.9),t.shadowBlur=d*1.2,t.beginPath(),t.moveTo(-I,-I*.9),t.lineTo(I*.5,0),t.lineTo(-I,I*.9),t.stroke()),t.restore(),t.globalAlpha=.62,t.shadowColor=r(.75),t.shadowBlur=d*.6,t.fillStyle=r(.6),t.beginPath(),t.arc(0,0,u*.6,0,Math.PI*2),t.fill(),t.restore(),t.globalAlpha=1,t.shadowBlur=0}export{k0 as C,p0 as G,P0 as M,W as N,E as P,b0 as Q,_0 as R,T0 as S,M0 as Z,$ as a,f0 as b,A0 as c,v0 as d,I0 as e,E0 as f,O0 as g,U0 as h,R0 as i,G0 as j,H0 as k,y0 as l,F0 as m,D0 as n,z as o,c0 as p,x0 as q,B0 as r,i0 as s,S0 as t,C0 as u,L0 as v,h0 as w,u0 as x};
