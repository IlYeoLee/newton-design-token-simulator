...
vec4 markState(vec2 uv, float state, float prog, float strong, float t){
  ...
  return A;
}`;function w0(a,t,o,i){a.lineWidth=4*t;const s=i.arrow;s.line==="dash"?a.setLineDash([12*t*s.gap,10*s.gap]):s.line==="dot"?(a.setLineDash([.5,12*s.gap]),a.lineCap="round",a.lineWidth=5*t):a.setLineDash([]),o!=null&&s.line!=="solid"&&s.line!=="taper"&&(a.lineDashOffset=-o*40*s.speed)}function B0(a,t,o,i,s,e={}){const c=s.lut,r=s.arrow||{},f=r.w??1,m=r.speed??1,d=r.glow??1,p=e.pulse??1,l=o/256,u=t/2,h=i*.9*m%1,C=e.prog!=null?Math.max(0,Math.min(1,e.prog)):Math.min(1,h/.55),M=e.prog!=null?1:h>.88?(1-h)/.12:1;a.clearRect(0,0,t,o);const w=M*(.45+.55*p),n=o-24*l,S=58*l,T=n+(S-n)*C,v=(k,L)=>c(k).replace("rgb(","rgba(").replace(")",`,${L.toFixed(3)})`),x=1.1*l*f,b=13*l*f,P=a.createLinearGradient(0,n,0,T);if(P.addColorStop(0,v(.55,0)),P.addColorStop(.1,v(.64,.45*w)),P.addColorStop(.32,v(.76,.85*w)),P.addColorStop(.62,v(.88,.98*w)),P.addColorStop(1,v(.97,w)),a.globalAlpha=1,a.fillStyle=P,a.beginPath(),a.moveTo(u-x/2,n),a.lineTo(u+x/2,n),a.lineTo(u+b/2,T),a.lineTo(u-b/2,T),a.closePath(),a.fill(),a.globalAlpha=w,C>.28&&!e.noTip){const k=34*l*(.7+.3*f),L=Math.min(1,(C-.28)/.22)*w,R=T+k*.3;a.globalAlpha=L;const I={color:c(.95),glowColor:c(.85),glow:12*d};s.glyph&&(s.glyph(a,"LIFT_TIP",u,R,k,I)||s.glyph(a,"TIP_TRI",u,R,k*.93,I))||(a.strokeStyle=c(.95),a.lineWidth=13*l*f,a.lineCap="round",a.lineJoin="round",a.shadowColor=c(.9),a.shadowBlur=18*l*d,a.beginPath(),a.moveTo(u-26*l,R+14*l),a.lineTo(u,R-16*l),a.lineTo(u+26*l,R+14*l),a.stroke())}a.globalAlpha=1,a.shadowBlur=0}function F0(a,t,o,i,s,e,c={}){const r=e.lut,f=e.arrow||{},m=f.w??1,d=f.glow??1,p=o/256;a.clearRect(0,0,t,o);const l=i.map(([n,S])=>[n*t,S*o]);if(l.length<2)return;const u=48,h=[],C=n=>{if(l.length===2)return[l[0][0]+(l[1][0]-l[0][0])*n,l[0][1]+(l[1][1]-l[0][1])*n];const S=n*(l.length-1),T=Math.min(l.length-2,Math.floor(S)),v=S-T,x=l[Math.max(0,T-1)],b=l[T],P=l[T+1],k=l[Math.min(l.length-1,T+2)],L=(R,I,y,F)=>.5*(2*I+(-R+y)*v+(2*R-5*I+4*y-F)*v*v+(-R+3*I-3*y+F)*v*v*v);return[L(x[0],b[0],P[0],k[0]),L(x[1],b[1],P[1],k[1])]};for(let n=0;n<=u;n++)h.push(C(n/u));const M=Math.max(0,Math.min(1,c.prog!=null?c.prog:s*.55%1)),w=Math.max(1,Math.round(u*M));a.lineCap="round";for(let n=1;n<=w;n++){const S=n/w;a.globalAlpha=Math.pow(S,1.5),a.strokeStyle=r(.45+.5*S),a.lineWidth=(1.6+3.2*S)*p*m,a.beginPath(),a.moveTo(h[n-1][0],h[n-1][1]),a.lineTo(h[n][0],h[n][1]),a.stroke()}if(M>.25){const n=h[w][0],S=h[w][1],T=h[Math.max(0,w-2)][0],v=h[Math.max(0,w-2)][1],x=Math.atan2(S-v,n-T)+Math.PI/2,b=30*p*(.7+.3*m);a.save(),a.translate(n,S),a.rotate(x),a.globalAlpha=Math.min(1,(M-.25)/.2);const P={color:r(.95),glowColor:r(.85),glow:12*d};e.glyph&&(e.glyph(a,"LIFT_TIP",0,0,b,P)||e.glyph(a,"TIP_TRI",0,0,b*.93,P))||(a.strokeStyle=r(.95),a.lineWidth=9*p*m,a.lineJoin="round",a.lineCap="round",a.beginPath(),a.moveTo(-18*p,12*p),a.lineTo(0,-14*p),a.lineTo(18*p,12*p),a.stroke()),a.restore()}a.globalAlpha=1}function c0(a,t,o,i,s,e){s=s||{};const c=e.lut,r=s.style||e.arrow.line,f=!!s.closed,m=[0];for(let u=1;u<t.length;u++)m.push(m[u-1]+Math.hypot(t[u][0]-t[u-1][0],t[u][1]-t[u-1][1]));const d=m[m.length-1]||1,p=u=>{u=(u%d+d)%d;let h=1;for(;h<m.length-1&&m[h]<u;)h++;const C=(u-m[h-1])/Math.max(1e-4,m[h]-m[h-1]);return[t[h-1][0]+(t[h][0]-t[h-1][0])*C,t[h-1][1]+(t[h][1]-t[h-1][1])*C,Math.atan2(t[h][1]-t[h-1][1],t[h][0]-t[h-1][0])]},l=e.arrow;if(r==="chevron"){const u=(26*i+8)*l.gap,h=Math.max(2,Math.floor(d/u));a.shadowColor=c(Math.min(1,l.heat+.2)),a.shadowBlur=8*i*l.glow;for(let C=0;C<h;C++){const M=C*u+o*42*l.speed%u;if(!f&&M>d-4)continue;const[w,n,S]=p(M),T=7.5*i,v=8.5*i,x=.45+.4*Math.sin(M/d*6.283-o*2.2*l.speed);a.strokeStyle=c(l.heat-.05+x*.3),a.lineWidth=3.2*i,a.lineJoin="round",a.lineCap="round",a.save(),a.translate(w,n),a.rotate(S),a.beginPath(),a.moveTo(-v*.5,-T),a.lineTo(v*.5,0),a.lineTo(-v*.5,T),a.stroke(),a.restore()}return!0}if(r==="comet"){const u=o*.35*l.speed%1*d,h=d*l.tail,C=Math.max(24,t.length*2);a.lineCap="round";for(let n=0;n<C;n++){const S=u-n/C*h,T=u-(n+1)/C*h;if(!f&&T<0)break;const v=1-n/C,[x,b]=p(S),[P,k]=p(T);!f&&Math.hypot(P-x,k-b)>d*.4||(a.globalAlpha=Math.pow(v,1.6),a.strokeStyle=c(Math.max(.05,l.heat-.2)+v*.55),a.lineWidth=(1.5+v*4.5)*i,v>.72?(a.shadowColor=c(Math.min(1,l.heat+.3)),a.shadowBlur=v*12*i*l.glow):a.shadowBlur=0,a.beginPath(),a.moveTo(x,b),a.lineTo(P,k),a.stroke())}a.globalAlpha=1,a.lineCap="butt",a.shadowBlur=0;const[M,w]=p(u);return a.fillStyle=rgba(W.ink,.95),a.shadowColor=c(.9),a.shadowBlur=16*i,a.beginPath(),a.arc(M,w,2.6*i,0,7),a.fill(),a.shadowBlur=0,!0}if(a.strokeStyle=s.color||c(l.heat),a.shadowColor=c(Math.min(1,l.heat+.15)),a.shadowBlur=(s.glow??8)*i*l.glow,r==="taper")for(let u=1;u<t.length;u++)a.lineWidth=(.5+u/t.length*4.5)*i,a.beginPath(),a.moveTo(t[u-1][0],t[u-1][1]),a.lineTo(t[u][0],t[u][1]),a.stroke();else w0(a,i,o,e),a.beginPath(),t.forEach(([u,h],C)=>C?a.lineTo(u,h):a.moveTo(u,h)),f&&a.closePath(),a.stroke();return a.setLineDash([]),a.lineCap="butt",a.lineDashOffset=0,a.shadowBlur=0,!0}function E0(a,t,o,i,s,e){const c=13*i.halo,r=e.lut;a.clearRect(0,0,t,t),a.lineJoin="round";const f=t/220,m=t/2,d=18*o.round*f,p=40*f,l=48*f,u=t-80*f,h=t-96*f,C=[],M=(n,S,T,v)=>{for(let x=0;x<=1;x+=.12)C.push([n+(T-n)*x,S+(v-S)*x])};M(p+d,l,p+u-d,l),M(p+u,l+d,p+u,l+h-d),M(p+u-d,l+h,p+d,l+h),M(p,l+h-d,p,l+d),a.shadowColor=r(.6),a.shadowBlur=c*.8;const w=4*e.arrow.w*f;if(e.arrow.line==="solid"?(a.setLineDash([10*o.dash*f,8*f]),a.lineDashOffset=-s*22*f,a.strokeStyle=r(.45),a.lineWidth=w,a.beginPath(),a.roundRect(p,l,u,h,d),a.stroke(),a.setLineDash([]),a.lineDashOffset=0):c0(a,C,s,e.arrow.w*f,{color:r(.45),closed:!0},e),o.prog!=null&&o.prog>.001){const n=[],S=(I,y,F,D,U)=>{for(let O=1;O<=U;O++)n.push([I+(F-I)*O/U,y+(D-y)*O/U])},T=(I,y,F,D,U)=>{for(let O=1;O<=U;O++){const $=F+(D-F)*O/U;n.push([I+d*Math.cos($),y+d*Math.sin($)])}},v=Math.PI/2,x=p+u,b=l+h,P=p+u/2;n.push([P,l]),S(P,l,x-d,l,8),T(x-d,l+d,-v,0,6),S(x,l+d,x,b-d,10),T(x-d,b-d,0,v,6),S(x-d,b,p+d,b,14),T(p+d,b-d,v,Math.PI,6),S(p,b-d,p,l+d,10),T(p+d,l+d,Math.PI,Math.PI+v,6),S(p+d,l,P,l,8);let k=0;for(let I=1;I<n.length;I++)k+=Math.hypot(n[I][0]-n[I-1][0],n[I][1]-n[I-1][1]);const L=k*Math.min(1,o.prog);a.save(),a.setLineDash([10*o.dash*f,8*f]),a.lineDashOffset=-(u/2-d)-s*22*f,a.strokeStyle=r(.9),a.lineWidth=w*1.3,a.lineCap="round",a.shadowColor=r(.92),a.shadowBlur=c*1.3,a.beginPath(),a.moveTo(n[0][0],n[0][1]);let R=0;for(let I=1;I<n.length&&R<L;I++){const y=Math.hypot(n[I][0]-n[I-1][0],n[I][1]-n[I-1][1]);if(R+y<=L)a.lineTo(n[I][0],n[I][1]),R+=y;else{const F=(L-R)/y;a.lineTo(n[I-1][0]+(n[I][0]-n[I-1][0])*F,n[I-1][1]+(n[I][1]-n[I-1][1])*F),R=L}}a.stroke(),a.setLineDash([]),a.restore()}o.feet>.05&&e.foot&&(e.foot(a,!1,m-16*o.feet*f,m+6*f,26*o.feet*f),e.foot(a,!0,m+16*o.feet*f,m+6*f,26*o.feet*f)),a.shadowBlur=0}function O0(a,t,o,i,s,e,c,r){const f=13*i.halo,m=4*e.arrow.w*(t/220),d=e.lut;a.clearRect(0,0,t,t),a.lineJoin="round";const p=t/220,l=c||[[45*p,130*p],[110*p,60*p],[175*p,110*p]],u=r??s*.5%1,h=Math.min(1,u*1.25)*(l.length-1),C=Math.min(l.length-1,Math.floor(h+.35));a.shadowColor=d(.7),a.shadowBlur=f;const M=[[l[0][0],l[0][1]]];for(let w=1;w<=l.length-1;w++){const n=Math.max(0,Math.min(1,h-(w-1)));if(n<=0)break;M.push([l[w-1][0]+(l[w][0]-l[w-1][0])*n,l[w-1][1]+(l[w][1]-l[w-1][1])*n])}M.length>1&&c0(a,M,s,e.arrow.w*p,{color:d(.62)},e),a.setLineDash([4*p,7*p]),a.lineDashOffset=0,a.globalAlpha=.3,a.strokeStyle=d(.45),a.lineWidth=m,a.beginPath(),l.forEach(([w,n],S)=>S?a.lineTo(w,n):a.moveTo(w,n)),a.stroke(),a.globalAlpha=1,a.setLineDash([]),a.lineCap="butt",a.lineDashOffset=0,l.forEach(([w,n],S)=>{const T=S===C,v=T?1+Math.sin(s*6)*.14:1;a.strokeStyle=d(T?.8:.45),a.lineWidth=m*(T?1.3:.9),a.shadowBlur=T?f*1.6:f*.6,a.beginPath(),a.arc(w,n,12*o.node*v*p,0,Math.PI*2),a.stroke(),e.num&&(a.globalAlpha=S<=C?1:.45,e.num(a,String(S+1),w,n,16*o.numS*v*p,Math.round(14*o.numS*p)),a.globalAlpha=1)}),a.shadowBlur=0}function G0(a,t,o,i,s,e,c){const r=e.lut,f=13*i.halo,m=t/220,d=t/2,p=e.arrow&&e.arrow.w||1,l=(x,b)=>r(x).replace("rgb(","rgba(").replace(")",`,${b})`);a.clearRect(0,0,t,t),a.lineJoin="round",a.lineCap="round";const u=(o.r!=null?o.r:.42)*t,h=u*(o.rt!=null?o.rt:.36),C=3.4*p*m,M=c!=null?Math.max(0,Math.min(1,c)):s*(o.tempo||.6)%1,w=Math.pow(M,1.6),n=Math.max(0,(M-.9)/.1);a.save(),a.translate(d,d);const S=(x,b,P,k=1)=>{if(x<=.6)return;const L=C*2.6*k,R=Math.max(.1,x-L),I=x+L,y=a.createRadialGradient(0,0,R,0,0,I);y.addColorStop(0,l(b-.05,0)),y.addColorStop(.5,l(b,P*.85)),y.addColorStop(1,l(b-.05,0)),a.globalAlpha=1,a.fillStyle=y,a.shadowBlur=0,a.beginPath(),a.arc(0,0,I,0,Math.PI*2),a.fill(),a.globalAlpha=Math.min(1,P*1.1),a.lineWidth=C*.85,a.strokeStyle=r(Math.min(.98,b+.12)),a.shadowColor=r(.88),a.shadowBlur=f*.6,a.beginPath(),a.arc(0,0,x,0,Math.PI*2),a.stroke(),a.shadowBlur=0},T=a.createRadialGradient(0,0,0,0,0,h*1.08);T.addColorStop(0,l(.6,.1+.18*n)),T.addColorStop(.65,l(.5,.05+.08*n)),T.addColorStop(1,l(.5,0)),a.globalAlpha=1,a.fillStyle=T,a.beginPath(),a.arc(0,0,h*1.08,0,Math.PI*2),a.fill();const v=1+.02*Math.sin(s*2.6);S(h*v,.55+.4*n,.5+.45*n,.9);for(let x=2;x>=0;x--){const b=Math.pow(Math.max(0,M-x*.05),1.6),P=u-(u-h)*b,k=x===0?.6+.4*w:.18/x*(1-n);S(P,.55+.4*w,k*(1-n*.45),1.15-.35*w)}n>.01&&S(h*(1+1.4*n),.9,(1-n)*.8,1.1),a.globalAlpha=.6+.3*n,a.shadowColor=r(.85),a.shadowBlur=f*(.9+n),a.fillStyle=r(.62+.3*n),a.beginPath(),a.arc(0,0,C*.85+3*m*n,0,Math.PI*2),a.fill(),a.restore(),a.globalAlpha=1,a.shadowBlur=0}function U0(a,t,o,i,s,e,c,r){const f=e.lut,m=13*i.halo,d=t/220,p=t/2,l=e.arrow&&e.arrow.w||1,u=l*d,h=(A,_)=>f(A).replace("rgb(","rgba(").replace(")",","+_+")");a.clearRect(0,0,t,t),a.lineJoin="round",a.lineCap="round";const C=t*.42*(o.spread!=null?o.spread:1),w=(r||[[-.95,.5],[-.48,-.42],[0,-.8],[.48,-.42],[.95,.5]]).map(([A,_])=>[p+A*C,p+_*C]),n=80,S=[];for(let A=0;A<=n;A++){const _=A/n*(w.length-1),B=Math.min(w.length-2,Math.floor(_)),G=_-B,H=w[Math.max(0,B-1)],K=w[B],N=w[B+1],a0=w[Math.min(w.length-1,B+2)],t0=(X,J,Y,o0)=>.5*(2*J+(-X+Y)*G+(2*X-5*J+4*Y-o0)*G*G+(-X+3*J-3*Y+o0)*G*G*G);S.push([t0(H[0],K[0],N[0],a0[0]),t0(H[1],K[1],N[1],a0[1])])}const T=A=>{const _=Math.max(0,Math.min(n,A*n)),B=Math.floor(_),G=_-B,H=S[B],K=S[Math.min(n,B+1)];return[H[0]+(K[0]-H[0])*G,H[1]+(K[1]-H[1])*G]},v=.68;let x,b,P;if(c!=null)x=Math.max(0,Math.min(1,c)),b=1,P=0;else{const A=s*(o.tempo||.42)%1;if(A<v)x=A/v,b=1,P=0;else{const _=(A-v)/(1-v);x=1,b=1-_*_,P=_}}if(b<=.012)return;const k=x*x*x*(x*(6*x-15)+10),L=Math.min(1,16*x*x*(1-x)*(1-x));o.taper!=null&&o.taper;const R=.36*(o.tail!=null?o.tail:1),I=o.width!=null?o.width:1;{const A=a.createLinearGradient(S[0][0],S[0][1],S[n][0],S[n][1]);A.addColorStop(0,h(.46,0)),A.addColorStop(.3,h(.46,.03*b)),A.addColorStop(.8,h(.46,.045*b)),A.addColorStop(1,h(.46,0)),a.globalAlpha=1,a.strokeStyle=A,a.lineWidth=9*u,a.shadowColor=f(.6),a.shadowBlur=m*2,a.beginPath(),S.forEach(([_,B],G)=>G?a.lineTo(_,B):a.moveTo(_,B)),a.stroke(),a.shadowBlur=0}const y=40,F=Math.max(0,k-R*(1-P)),D=[];for(let A=0;A<=y;A++)D.push(T(F+(k-F)*(A/y)));const U=()=>{a.beginPath(),D.forEach(([A,_],B)=>B?a.lineTo(A,_):a.moveTo(A,_)),a.stroke()},O=()=>{const A=a.createLinearGradient(D[0][0],D[0][1],D[y][0],D[y][1]);return A.addColorStop(0,h(.55,0)),A.addColorStop(.4,h(.56,0)),A.addColorStop(.68,h(.6,.09)),A.addColorStop(.88,h(.64,.24)),A.addColorStop(1,h(.68,.44)),A},$=1+.5*L;a.globalAlpha=b,a.strokeStyle=O(),a.lineWidth=(20+10*L)*u*I,a.shadowColor=f(.72),a.shadowBlur=m*2.2,U(),a.strokeStyle=O(),a.lineWidth=(10+5*L)*u*I,a.shadowBlur=m*1,U(),a.shadowBlur=0;for(let A=1;A<=y;A++){const _=A/y;a.globalAlpha=Math.pow(_,2.2)*.95*b,a.strokeStyle=f(.55+.38*_),a.lineWidth=(1.6+6.5*Math.pow(_,.7))*u*I*$,a.beginPath(),a.moveTo(D[A-1][0],D[A-1][1]),a.lineTo(D[A][0],D[A][1]),a.stroke()}const V=D[y][0],g=D[y][1];a.globalAlpha=.8*b,a.fillStyle=f(.6),a.shadowColor=f(.8),a.shadowBlur=m*1.6,a.beginPath(),a.arc(V,g,(9+5*L)*u*I,0,Math.PI*2),a.fill(),a.globalAlpha=b,a.fillStyle=f(.93),a.shadowBlur=m*.6,a.beginPath(),a.arc(V,g,(3.4+1.8*L)*u*I,0,Math.PI*2),a.fill(),a.globalAlpha=1,a.shadowBlur=0}function H0(a,t,o,i,s,e,c){const r=e.lut,f=13*i.halo,m=t/220,d=t/2,p=e.arrow&&e.arrow.w||1;a.clearRect(0,0,t,t),a.lineJoin="round",a.lineCap="round";const l=(o.r!=null?o.r:.3)*t,u=o.width!=null?o.width:1,h=4.2*p*m*u,C=o.dir!=null?o.dir:1,M=(o.sweep!=null?o.sweep:.66)*Math.PI*2,w=c!=null?Math.max(0,Math.min(1,c)):s*(o.tempo||.5)%1,n=-Math.PI/2+C*w*Math.PI*2;a.save(),a.translate(d,d),a.globalAlpha=.16,a.lineWidth=h*.7,a.strokeStyle=r(.44),a.shadowColor=r(.6),a.shadowBlur=f*.4,a.beginPath(),a.arc(0,0,l,0,Math.PI*2),a.stroke(),a.shadowBlur=0;const S=16;for(let L=0;L<S;L++){const R=L/(S-1),I=n-C*R*M,y=n-C*(R+1.2/S)*M;a.globalAlpha=(1-R)*.9,a.strokeStyle=r(.55+.35*(1-R)),a.lineWidth=h*(.55+.55*(1-R)),a.shadowColor=r(.8),a.shadowBlur=f*(.4+.5*(1-R)),a.beginPath(),a.arc(0,0,l,Math.min(I,y),Math.max(I,y),!1),a.stroke()}a.shadowBlur=0;const T=Math.cos(n)*l,v=Math.sin(n)*l,x=n+C*Math.PI/2,b=8*m*u;a.save(),a.translate(T,v),a.rotate(x+Math.PI/2),a.globalAlpha=1;const P=3.4*b*(.7+.3*p),k={color:r(.96),glowColor:r(.9),glow:f*1.2};e.glyph&&(e.glyph(a,"LIFT_TIP",0,0,P,k)||e.glyph(a,"TIP_TRI",0,0,P*.93,k))||(a.rotate(-Math.PI/2),a.strokeStyle=r(.96),a.lineWidth=h*.9,a.shadowColor=r(.9),a.shadowBlur=f*1.2,a.beginPath(),a.moveTo(-b,-b*.9),a.lineTo(b*.5,0),a.lineTo(-b,b*.9),a.stroke()),a.restore(),a.globalAlpha=.62,a.shadowColor=r(.75),a.shadowBlur=f*.6,a.fillStyle=r(.6),a.beginPath(),a.arc(0,0,h*.6,0,Math.PI*2),a.fill(),a.restore(),a.globalAlpha=1,a.shadowBlur=0}export{L0 as C,p0 as G,y0 as M,W as N,E as P,A0 as Q,k0 as R,v0 as S,T0 as Z,j as a,d0 as b,I0 as c,M0 as d,b0 as e,E0 as f,O0 as g,G0 as h,P0 as i,U0 as j,H0 as k,R0 as l,B0 as m,D0 as n,z as o,c0 as p,x0 as q,F0 as r,i0 as s,S0 as t,C0 as u,_0 as v,u0 as w,h0 as x};
