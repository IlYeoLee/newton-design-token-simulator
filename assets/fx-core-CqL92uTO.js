...
vec4 markState(vec2 uv, float state, float prog, float strong, float t){
  ...
  return A;
}`;function Ut(t,a,o,d){t.lineWidth=4*a;const i=d.arrow;i.line==="dash"?t.setLineDash([12*a*i.gap,10*i.gap]):i.line==="dot"?(t.setLineDash([.5,12*i.gap]),t.lineCap="round",t.lineWidth=5*a):t.setLineDash([]),o!=null&&i.line!=="solid"&&i.line!=="taper"&&(t.lineDashOffset=-o*40*i.speed)}function so(t,a,o,d,i,l={}){const e=i.lut,c=i.arrow||{},h=c.w??1,v=c.speed??1,u=c.glow??1,p=l.pulse??1,r=o/256,n=r*(l.scale??1),s=a/2,A=d*.9*v%1,B=l.prog!=null?Math.max(0,Math.min(1,l.prog)):Math.min(1,A/.55),D=l.prog!=null?1:A>.88?(1-A)/.12:1;t.clearRect(0,0,a,o);const m=D*(.45+.55*p),I=o-24*r,L=58*r,R=I+(L-I)*B,M=(y,W)=>e(y).replace("rgb(","rgba(").replace(")",`,${W.toFixed(3)})`),w=1.1*n*h,x=13*n*h,b=42*n*(.7+.3*h),F=!l.noTip&&B>.28?Math.min(1,(B-.28)/.22)*b*.42:0,T=R+F,C=t.createLinearGradient(0,I,0,T),_=y=>Math.max(.02,Math.min(.99,y+((c.heat??.5)-.5)*1.1));C.addColorStop(0,M(_(.55),0)),C.addColorStop(.18,M(_(.64),.3*m)),C.addColorStop(.4,M(_(.76),.8*m)),C.addColorStop(.65,M(_(.88),.98*m)),C.addColorStop(1,M(_(.97),m));const G=l.dots?(()=>{const y=Math.abs(T-I),W=Math.max(3,Math.round(y/(9.5*n))),q=d*1.15*v%1,Y=(P,f,k)=>{const E=Math.max(0,Math.min(1,(k-P)/(f-P)));return E*E*(3-2*E)},S=[];for(let P=0;P<W;P++){const f=((P+.5)/W+q/W)%1,k=1+.22*Math.sin((f*3-q*2)*Math.PI*2);S.push({y:I+(T-I)*f,r:(w/2+(x/2-w/2)*f)*k,a:(1-.35*Y(.72,.92,f))*(1-Y(.92,1,f))})}return S})():null;if(t.save(),t.filter=`blur(${7*n}px)`,t.globalAlpha=l.dots?.3:.55,t.fillStyle=C,l.dots){const y=t.globalAlpha;for(const W of G)t.globalAlpha=y*W.a,t.beginPath(),t.arc(s,W.y,Math.max(1.4*n,W.r*1.5),0,Math.PI*2),t.fill();t.globalAlpha=y}else t.beginPath(),t.moveTo(s-w,I),t.lineTo(s+w,I),t.lineTo(s+x*.95,T),t.lineTo(s-x*.95,T),t.closePath(),t.fill();if(t.restore(),t.globalAlpha=1,t.fillStyle=C,l.dots){for(const y of G)t.globalAlpha=y.a,t.beginPath(),t.arc(s,y.y,Math.max(.9*n,y.r*.92),0,Math.PI*2),t.fill();t.globalAlpha=1}else t.beginPath(),t.moveTo(s-w/2,I),t.lineTo(s+w/2,I),t.lineTo(s+x/2,T),t.lineTo(s-x/2,T),t.closePath(),t.fill();if(t.fillStyle="rgba(255,255,255,0.94)",l.dots){for(const y of G)t.globalAlpha=y.a*m,t.beginPath(),t.arc(s,y.y,Math.max(.6*n,y.r*.46),0,Math.PI*2),t.fill();t.globalAlpha=1}else{t.save(),t.globalAlpha=.9*m;const y=w*.2,W=x*.3;t.beginPath(),t.moveTo(s-y,I),t.lineTo(s+y,I),t.lineTo(s+W,T),t.lineTo(s-W,T),t.closePath(),t.fill(),t.restore()}if(t.globalAlpha=m,B>.28&&!l.noTip){const y=b,W=Math.min(1,(B-.28)/.22)*m,q=R+y*.3;t.globalAlpha=W;const Y={color:e(_(.95)),glowColor:e(_(.85)),glow:12*u};i.glyph&&(i.glyph(t,"LIFT_TIP",s,q,y,Y)||i.glyph(t,"TIP_TRI",s,q,y*.93,Y))||(t.strokeStyle=e(.95),t.lineWidth=13*n*h,t.lineCap="round",t.lineJoin="round",t.shadowColor=e(.9),t.shadowBlur=18*n*u,t.beginPath(),t.moveTo(s-26*n,q+14*n),t.lineTo(s,q-16*n),t.lineTo(s+26*n,q+14*n),t.stroke())}t.globalAlpha=1,t.shadowBlur=0}function Wt(t,a,o,d,i,l,e={}){const c=l.lut,h=l.arrow||{},v=h.w??1,u=h.glow??1,p=w=>Math.max(.02,Math.min(.99,w+((h.heat??.5)-.5)*1.1)),r=o/256*(e.scale??1);t.clearRect(0,0,a,o);const n=d.map(([w,x])=>[w*a,x*o]);if(n.length<2)return;const s=48,A=[],B=w=>{if(n.length===2)return[n[0][0]+(n[1][0]-n[0][0])*w,n[0][1]+(n[1][1]-n[0][1])*w];const x=w*(n.length-1),b=Math.min(n.length-2,Math.floor(x)),F=x-b,T=n[Math.max(0,b-1)],C=n[b],_=n[b+1],G=n[Math.min(n.length-1,b+2)],y=(W,q,Y,S)=>.5*(2*q+(-W+Y)*F+(2*W-5*q+4*Y-S)*F*F+(-W+3*q-3*Y+S)*F*F*F);return[y(T[0],C[0],_[0],G[0]),y(T[1],C[1],_[1],G[1])]};for(let w=0;w<=s;w++)A.push(B(w/s));const D=Math.max(0,Math.min(1,e.prog!=null?e.prog:i*.55%1)),m=Math.max(1,Math.round(s*D)),I=e.alpha??1,L=e.tail??.22;t.lineCap="round";const R=42*r*(.7+.3*v);let M=m;if(D>.28){let w=0;const x=R*.42*Math.min(1,(D-.28)/.22);for(;M>1&&w<x;)w+=Math.hypot(A[M][0]-A[M-1][0],A[M][1]-A[M-1][1]),M--}for(const w of[0,1])for(let x=1;x<=M;x++){const b=x/m,F=Math.min(1,b/L),T=F*F*(3-2*F)*I;t.globalAlpha=w?T:T*.16*b,t.strokeStyle=c(.55+.42*b),t.lineWidth=(1.1+11.9*b)*r*v*(w?1:1.9),t.beginPath(),t.moveTo(A[x-1][0],A[x-1][1]),t.lineTo(A[x][0],A[x][1]),t.stroke()}if(D>.28){const w=A[Math.max(0,m-2)][0],x=A[Math.max(0,m-2)][1],b=Math.atan2(A[m][1]-x,A[m][0]-w)+Math.PI/2,F=A[m][0]-Math.sin(b)*R*.3,T=A[m][1]+Math.cos(b)*R*.3;t.save(),t.translate(F,T),t.rotate(b),t.globalAlpha=Math.min(1,(D-.28)/.22)*I;const C={color:c(p(.95)),glowColor:c(p(.85)),glow:12*u};l.glyph&&(l.glyph(t,"LIFT_TIP",0,0,R,C)||l.glyph(t,"TIP_TRI",0,0,R*.93,C))||(t.strokeStyle=c(.95),t.lineWidth=9*r*v,t.lineJoin="round",t.lineCap="round",t.beginPath(),t.moveTo(-18*r,12*r),t.lineTo(0,-14*r),t.lineTo(18*r,12*r),t.stroke()),t.restore()}t.globalAlpha=1}function qt(t,a,o,d,i,l){i=i||{};const e=l.lut,c=i.style||l.arrow.line,h=!!i.closed,v=[0];for(let n=1;n<a.length;n++)v.push(v[n-1]+Math.hypot(a[n][0]-a[n-1][0],a[n][1]-a[n-1][1]));const u=v[v.length-1]||1,p=n=>{n=(n%u+u)%u;let s=1;for(;s<v.length-1&&v[s]<n;)s++;const A=(n-v[s-1])/Math.max(1e-4,v[s]-v[s-1]);return[a[s-1][0]+(a[s][0]-a[s-1][0])*A,a[s-1][1]+(a[s][1]-a[s-1][1])*A,Math.atan2(a[s][1]-a[s-1][1],a[s][0]-a[s-1][0])]},r=l.arrow;if(c==="chevron"){const n=(26*d+8)*r.gap,s=Math.max(2,Math.floor(u/n));t.shadowColor=e(Math.min(1,r.heat+.2)),t.shadowBlur=8*d*r.glow;for(let A=0;A<s;A++){const B=A*n+o*42*r.speed%n;if(!h&&B>u-4)continue;const[D,m,I]=p(B),L=7.5*d,R=8.5*d,M=.45+.4*Math.sin(B/u*6.283-o*2.2*r.speed);t.strokeStyle=e(r.heat-.05+M*.3),t.lineWidth=3.2*d,t.lineJoin="round",t.lineCap="round",t.save(),t.translate(D,m),t.rotate(I),t.beginPath(),t.moveTo(-R*.5,-L),t.lineTo(R*.5,0),t.lineTo(-R*.5,L),t.stroke(),t.restore()}return!0}if(c==="comet"){const n=o*.35*r.speed%1*u,s=u*r.tail,A=Math.max(24,a.length*2);t.lineCap="round";for(let m=0;m<A;m++){const I=n-m/A*s,L=n-(m+1)/A*s;if(!h&&L<0)break;const R=1-m/A,[M,w]=p(I),[x,b]=p(L);!h&&Math.hypot(x-M,b-w)>u*.4||(t.globalAlpha=Math.pow(R,1.6),t.strokeStyle=e(Math.max(.05,r.heat-.2)+R*.55),t.lineWidth=(1.5+R*4.5)*d,R>.72?(t.shadowColor=e(Math.min(1,r.heat+.3)),t.shadowBlur=R*12*d*r.glow):t.shadowBlur=0,t.beginPath(),t.moveTo(M,w),t.lineTo(x,b),t.stroke())}t.globalAlpha=1,t.lineCap="butt",t.shadowBlur=0;const[B,D]=p(n);return t.fillStyle=rgba(t0.ink,.95),t.shadowColor=e(.9),t.shadowBlur=16*d,t.beginPath(),t.arc(B,D,2.6*d,0,7),t.fill(),t.shadowBlur=0,!0}if(t.strokeStyle=i.color||e(r.heat),t.shadowColor=e(Math.min(1,r.heat+.15)),t.shadowBlur=(i.glow??8)*d*r.glow,c==="taper")for(let n=1;n<a.length;n++)t.lineWidth=(.5+n/a.length*4.5)*d,t.beginPath(),t.moveTo(a[n-1][0],a[n-1][1]),t.lineTo(a[n][0],a[n][1]),t.stroke();else Ut(t,d,o,l),t.beginPath(),a.forEach(([n,s],A)=>A?t.lineTo(n,s):t.moveTo(n,s)),h&&t.closePath(),t.stroke();return t.setLineDash([]),t.lineCap="butt",t.lineDashOffset=0,t.shadowBlur=0,!0}function Kt(t,a,o,d,i,l){const e=l.lut,c=a/220,h=a/2,v=13*d.halo*(o.glow??1),u=h-26*c,p=32,r=Math.max(0,Math.min(1,o.prog??0)),n=r*p,s=2.9*c*(o.dash??1),A=1+.05*Math.sin(i*2.2);t.save(),t.translate(h,h);for(let B=0;B<p;B++){const D=-Math.PI/2+B/p*Math.PI*2,m=u*Math.cos(D),I=u*Math.sin(D),L=Math.max(0,Math.min(1,n-B));L>0?(t.shadowColor=e(.8),t.shadowBlur=v*(.8+.6*L),t.fillStyle=e(.55+.35*L),t.globalAlpha=.35+.65*L,t.beginPath(),t.arc(m,I,s*(.85+.45*L),0,Math.PI*2),t.fill()):(t.shadowColor=e(.45),t.shadowBlur=v*.35,t.fillStyle=e(.42),t.globalAlpha=.3*A,t.beginPath(),t.arc(m,I,s*.72,0,Math.PI*2),t.fill())}if(r>.995){const B=i*1.6%1;t.globalAlpha=(1-B)*.5,t.shadowBlur=v*.6,t.shadowColor=e(.85),t.strokeStyle=e(.8),t.lineWidth=2.2*c,t.beginPath(),t.arc(0,0,u*(1+.22*B),0,Math.PI*2),t.stroke()}t.globalAlpha=1,t.shadowBlur=0,t.restore()}function ro(t,a,o,d,i,l){const e=13*d.halo,c=l.lut;t.clearRect(0,0,a,a),t.lineJoin="round";const h=a/220,v=a/2,u=18*o.round*h,p=40*h,r=48*h,n=a-80*h,s=a-96*h;if(u>=Math.min(n,s)/2-1){Kt(t,a,o,d,i,l);return}const A=[],B=(m,I,L,R)=>{for(let M=0;M<=1;M+=.12)A.push([m+(L-m)*M,I+(R-I)*M])};B(p+u,r,p+n-u,r),B(p+n,r+u,p+n,r+s-u),B(p+n-u,r+s,p+u,r+s),B(p,r+s-u,p,r+u),t.shadowColor=c(.6),t.shadowBlur=e*.8;const D=4*l.arrow.w*h;if(l.arrow.line==="solid"?(t.setLineDash([10*o.dash*h,8*h]),t.lineDashOffset=-i*22*h,t.strokeStyle=c(.45),t.lineWidth=D,t.beginPath(),t.roundRect(p,r,n,s,u),t.stroke(),t.setLineDash([]),t.lineDashOffset=0):qt(t,A,i,l.arrow.w*h,{color:c(.45),closed:!0},l),o.prog!=null&&o.prog>.001){const m=[],I=(C,_,G,y,W)=>{for(let q=1;q<=W;q++)m.push([C+(G-C)*q/W,_+(y-_)*q/W])},L=(C,_,G,y,W)=>{for(let q=1;q<=W;q++){const Y=G+(y-G)*q/W;m.push([C+u*Math.cos(Y),_+u*Math.sin(Y)])}},R=Math.PI/2,M=p+n,w=r+s,x=p+n/2;m.push([x,r]),I(x,r,M-u,r,8),L(M-u,r+u,-R,0,6),I(M,r+u,M,w-u,10),L(M-u,w-u,0,R,6),I(M-u,w,p+u,w,14),L(p+u,w-u,R,Math.PI,6),I(p,w-u,p,r+u,10),L(p+u,r+u,Math.PI,Math.PI+R,6),I(p+u,r,x,r,8);let b=0;for(let C=1;C<m.length;C++)b+=Math.hypot(m[C][0]-m[C-1][0],m[C][1]-m[C-1][1]);const F=b*Math.min(1,o.prog);t.save(),t.setLineDash([10*o.dash*h,8*h]),t.lineDashOffset=-(n/2-u)-i*22*h,t.strokeStyle=c(.9),t.lineWidth=D*1.3,t.lineCap="round",t.shadowColor=c(.92),t.shadowBlur=e*1.3,t.beginPath(),t.moveTo(m[0][0],m[0][1]);let T=0;for(let C=1;C<m.length&&T<F;C++){const _=Math.hypot(m[C][0]-m[C-1][0],m[C][1]-m[C-1][1]);if(T+_<=F)t.lineTo(m[C][0],m[C][1]),T+=_;else{const G=(F-T)/_;t.lineTo(m[C-1][0]+(m[C][0]-m[C-1][0])*G,m[C-1][1]+(m[C][1]-m[C-1][1])*G),T=F}}t.stroke(),t.setLineDash([]),t.restore()}o.feet>.05&&l.foot&&(l.foot(t,!1,v-16*o.feet*h,v+6*h,26*o.feet*h),l.foot(t,!0,v+16*o.feet*h,v+6*h,26*o.feet*h)),t.shadowBlur=0}function io(t,a,o,d,i,l,e,c){const h=13*d.halo,v=4*l.arrow.w*(a/220),u=l.lut;t.clearRect(0,0,a,a),t.lineJoin="round";const p=a/220,r=e||[[45*p,130*p],[110*p,60*p],[175*p,110*p]],n=c??i*.5%1,s=Math.min(1,n*1.25)*(r.length-1),A=Math.min(r.length-1,Math.floor(s+.35));t.shadowColor=u(.7),t.shadowBlur=h;const B=w=>{const x=Math.max(0,Math.min(r.length-2,Math.floor(w))),b=w-x;return[r[x][0]+(r[x+1][0]-r[x][0])*b,r[x][1]+(r[x+1][1]-r[x][1])*b]},D=o.comet!=null?o.comet:1,m=o.tailLen!=null?o.tailLen:.5;if(t.save(),t.shadowBlur=0,t.globalAlpha=o.rail!=null?o.rail:.22,t.strokeStyle=u(.6),t.lineWidth=2.2*p,t.lineCap="round",t.setLineDash([.01,8*p]),t.beginPath(),r.forEach(([w,x],b)=>b?t.lineTo(w,x):t.moveTo(w,x)),t.stroke(),t.setLineDash([]),t.restore(),s>.02){const w=(x,b)=>u(x).replace("rgb(","rgba(").replace(")",`,${b.toFixed(3)})`);for(let x=0;x<26;x++){const b=x/25,F=s-m*b;if(F<=0)break;const T=B(F),C=4.7*p*D*(1-b*.75),_=C*2.1,G=(1-b)*(1-b)*.8,y=t.createRadialGradient(T[0],T[1],0,T[0],T[1],_);y.addColorStop(0,w(.55-.18*b,G)),y.addColorStop(.5,w(.5-.18*b,G*.35)),y.addColorStop(1,w(.45-.18*b,0)),t.save(),t.shadowBlur=0,t.globalAlpha=1,t.fillStyle=y,t.beginPath(),t.arc(T[0],T[1],_,0,Math.PI*2),t.fill(),t.restore()}if(s<r.length-1-.001&&s-Math.floor(s)>.03){const x=B(s);t.save(),t.fillStyle=u(.62),t.shadowColor=u(.8),t.shadowBlur=11*p*D,t.beginPath(),t.arc(x[0],x[1],5.6*p*D,0,Math.PI*2),t.fill(),t.fillStyle=u(.95),t.globalAlpha=.95,t.beginPath(),t.arc(x[0],x[1],2.1*p*D,0,Math.PI*2),t.fill(),t.restore()}}t.globalAlpha=1,t.lineCap="butt";const I=o.done!=null,L=o.done||0,R=o.vd||[],M=o.acc!=null?o.acc:r.length-1;if(r.forEach(([w,x],b)=>{const F=b<L,T=R[b]||null,C=!F&&b===A&&!I,_=C?1+Math.sin(i*6)*.14:1,G=o.pop!=null&&b===L-1&&o.pop<.3?1+.35*(1-o.pop/.3):1,y=12*o.node*_*G*p*(b===M?1.34:1);F&&T!=="miss"&&(t.shadowBlur=h*1.4,t.shadowColor=u(.5),t.fillStyle=u(T==="near"?.5:.36),t.beginPath(),t.arc(w,x,y*.88,0,Math.PI*2),t.fill()),F||(t.save(),t.translate(w,x),o0(t,u,y,C?.8:.5,C?.9:.5,v*.9,h),t.restore()),t.strokeStyle=T==="hit"?K.prism:T==="near"?K.sand:T==="miss"?t0.lo:u(F?.62:C?.8:.45),t.lineWidth=v*(C?1.3:T?1.15:.9),t.shadowBlur=C?h*1.6:T&&T!=="miss"?h*1.3:h*.6,T&&T!=="miss"&&(t.shadowColor=T==="hit"?K.prism:K.sand),t.beginPath(),t.arc(w,x,y,0,Math.PI*2),t.stroke(),l.num&&(F&&T!=="miss"?(t.save(),t.globalCompositeOperation="destination-out",t.shadowBlur=0,l.num(t,String(b+1),w,x,16*o.numS*_*p,Math.round(14*o.numS*p)),t.restore()):(t.globalAlpha=b<=A?1:.6,l.num(t,String(b+1),w,x,16*o.numS*_*p,Math.round(14*o.numS*p)),t.globalAlpha=1))}),o.pop!=null&&o.pop<.38&&L>0&&r[L-1]){const[w,x]=r[L-1],b=1-o.pop/.38,F=b*b,T=12*o.node*p*(L-1===(o.acc!=null?o.acc:r.length-1)?1.34:1);t.save();const C=T*(1.6+1.9*(1-b)),_=t.createRadialGradient(w,x,0,w,x,C);_.addColorStop(0,`rgba(255,255,255,${Math.min(1,1.2*F).toFixed(3)})`),_.addColorStop(.35,u(.92).replace("rgb(","rgba(").replace(")",`,${(.8*F).toFixed(3)})`)),_.addColorStop(1,u(.7).replace("rgb(","rgba(").replace(")",",0)")),t.fillStyle=_,t.beginPath(),t.arc(w,x,C,0,Math.PI*2),t.fill(),t.strokeStyle=u(.92),t.globalAlpha=F,t.lineWidth=Math.max(1,6*p*b),t.shadowColor=u(.85),t.shadowBlur=24*p*F,t.beginPath(),t.arc(w,x,T*(1.1+3.4*(1-b)),0,Math.PI*2),t.stroke();const G=Math.max(0,b-.35)/.65;G>0&&(t.globalAlpha=G*G*.6,t.lineWidth=Math.max(1,3.5*p*G),t.beginPath(),t.arc(w,x,T*(1.1+2*(1-G)),0,Math.PI*2),t.stroke()),t.restore()}t.shadowBlur=0}function o0(t,a,o,d,i,l,e,c=1){if(o<=.6)return;const h=(n,s)=>a(n).replace("rgb(","rgba(").replace(")",`,${s})`),v=l*2.6*c,u=Math.max(.1,o-v),p=o+v,r=t.createRadialGradient(0,0,u,0,0,p);r.addColorStop(0,h(d-.05,0)),r.addColorStop(.5,h(d,i*.85)),r.addColorStop(1,h(d-.05,0)),t.globalAlpha=1,t.fillStyle=r,t.shadowBlur=0,t.beginPath(),t.arc(0,0,p,0,Math.PI*2),t.fill(),t.globalAlpha=Math.min(1,i*1.1),t.lineWidth=l*.85,t.strokeStyle=a(Math.min(.98,d+.12)),t.shadowColor=a(.88),t.shadowBlur=e*.6,t.beginPath(),t.arc(0,0,o,0,Math.PI*2),t.stroke(),t.shadowBlur=0}const e0=t=>1-Math.pow(1-Math.max(0,Math.min(1,t)),5);function co(t,a,o,d,i,l){const e=l.lut,c=a/512,h=l?.day?1:0,v=l.arrow&&l.arrow.w||1,u=(S,P)=>e(S).replace("rgb(","rgba(").replace(")",","+P+")"),p=h?.95:.86,r=(S,P)=>u(p,S),n=4*v*c;t.clearRect(0,0,a,a),t.lineJoin="round",t.lineCap="round";const s=o.mat||{nx:.86,fx:.86,ny:-.86,fy:.86},A=o.in==null?1:Math.max(0,Math.min(1,o.in)),B=e0(A/.45),D=S=>e0((A-.28-S*.09)/.42),m=o.hit==null?9:o.hit,I=m<.34?Math.sin(Math.PI*(m/.34)):0,L=S=>a/2+S*a/2,R=S=>a/2-S*a/2,M=(S,P)=>e(S).replace("rgb(","rgba(").replace(")",","+P+")"),w=(S,P,f,k,E,O,U)=>{t.font="700 "+k+'px "Supreme", sans-serif',t.textAlign="center",t.textBaseline="middle";const H=Array.from(String(S)),z=k*O;let Q=-z;for(const j of H)Q+=t.measureText(j).width+z;let J=P-Q/2;t.fillStyle=E;for(const j of H){const $=t.measureText(j).width;t.fillText(j,J+$/2,f),J+=$+z}},x=.05*(o.round!=null?o.round/.35:1),b=(S,P,f,k)=>{};b(-s.fx+x,s.fy-x),b(-s.nx+x*.6,s.ny+x),b(s.nx-x*.6,s.ny+x),b(s.fx-x,s.fy-x);const F=o.center?o.center.x:0,T=o.center?o.center.y:0,C=(o.center&&o.center.r!=null?o.center.r:.26)*a/2*B*(1+.055*I+.008*Math.sin(i*2.4)),_=(o.targets||[]).slice().sort((S,P)=>(S.n||0)-(P.n||0));if(_.length){t.save(),t.shadowBlur=0,t.globalAlpha=(h?.55:.22)*e0(A/.9),t.strokeStyle=e(h?.3:.6),t.lineWidth=(h?3.6:2.2)*c,t.lineCap="round",t.setLineDash([.01,(h?6:8)*c]);const S=o.travel||{};for(const P of _){const f=L(P.x),k=R(P.y),E=L(F)-f,O=R(T)-k,U=S.k>0&&S.k<1&&(P.n===S.from||P.n===S.to),H=Math.hypot(E,O)||1,z=(P.r!=null?P.r:.2)*a/2*1.16,Q=f+E/H*z,J=k+O/H*z,j=C*(o.center&&o.center.live?1.34:1),$=L(F)-E/H*j*1.12,X=R(T)-O/H*j*1.12;if(t.beginPath(),t.moveTo(Q,J),t.lineTo($,X),t.stroke(),I>.01){const Z=1-I,V=.34,i0=t.createLinearGradient($,X,Q,J),l0=(x0,w0)=>i0.addColorStop(Math.max(0,Math.min(1,x0)),e(.85).replace("rgb(","rgba(").replace(")",","+w0+")"));l0(Z-V,0),l0(Z,.9*I),l0(Z+V,0),t.save(),t.setLineDash([]),t.globalAlpha=1,t.strokeStyle=i0,t.lineWidth=3.4*c,t.stroke(),t.restore()}if(U){const Z=t.createLinearGradient(Q,J,$,X),V=Math.sin(Math.PI*S.k)*.85;Z.addColorStop(0,e(.5).replace("rgb(","rgba(").replace(")",",0)")),Z.addColorStop(.5,e(.9).replace("rgb(","rgba(").replace(")",","+V+")")),Z.addColorStop(1,e(.5).replace("rgb(","rgba(").replace(")",",0)")),t.save(),t.setLineDash([]),t.globalAlpha=1,t.strokeStyle=Z,t.lineWidth=3*c,t.stroke(),t.restore()}}t.setLineDash([]),t.restore()}const G=o.travel;if(G&&G.k>.001&&G.k<1){const S=_.find(f=>f.n===G.from),P=_.find(f=>f.n===G.to);if(S&&P){const f=L(S.x),k=R(S.y),E=L(P.x),O=R(P.y),U=L(F),H=R(T),z=e0(G.k),Q=V=>V<.5?[f+(U-f)*(V/.5),k+(H-k)*(V/.5)]:[U+(E-U)*((V-.5)/.5),H+(O-H)*((V-.5)/.5)],[J,j]=Q(z),[$,X]=Q(Math.max(0,z-.28)),Z=t.createLinearGradient($,X,J,j);Z.addColorStop(0,e(.5).replace("rgb(","rgba(").replace(")",",0)")),Z.addColorStop(1,e(.85).replace("rgb(","rgba(").replace(")",",0.85)")),t.strokeStyle=Z,t.lineWidth=3.2*c,t.lineCap="round",t.beginPath(),t.moveTo($,X),t.lineTo(J,j),t.stroke(),t.fillStyle=e(.62),t.shadowColor=e(.8),t.shadowBlur=11*c,t.beginPath(),t.arc(J,j,5.6*c,0,Math.PI*2),t.fill(),t.fillStyle=e(.95),t.globalAlpha=.95,t.shadowBlur=0,t.beginPath(),t.arc(J,j,2.1*c,0,Math.PI*2),t.fill(),t.globalAlpha=1}}const y=13*d.halo*(h?.45:1);for(const S of o.targets||[]){const P=S.on!==!1,f=!!S.live,k=D(S.n?S.n-1:0);if(k<=.001)continue;const E=L(S.x),O=R(S.y),U=(S.r!=null?S.r:.2)*a/2*(f?1.34:1)*(.9+.1*k);t.globalAlpha=k*(P?1:.45),t.shadowBlur=y*1.4,t.shadowColor=e(.5),P||f?(t.fillStyle=e(h?f?.26:.62:f?.5:.36),t.beginPath(),t.arc(E,O,U*.88,0,Math.PI*2),t.fill(),t.shadowBlur=0,t.save(),t.translate(E,O),o0(t,e,U,f?.8:.5,f?.9:.5,n*.9,y),t.restore()):(t.shadowBlur=y*.5,t.strokeStyle=e(.55),t.lineWidth=n*.9,t.setLineDash([n*1.1,n*1.35]),t.beginPath(),t.arc(E,O,U*.94,0,Math.PI*2),t.stroke(),t.setLineDash([]));const H=S.v;t.strokeStyle=H==="hit"?K.prism:H==="near"?K.sand:H==="miss"?t0.lo:e(f?.8:.45),t.lineWidth=n*(f?1.3:H?1.15:.9),t.shadowBlur=f?y*1.6:H&&H!=="miss"?y*1.3:y*.6,t.shadowColor=H==="hit"?K.prism:H==="near"?K.sand:e(.5),t.beginPath(),t.arc(E,O,U,0,Math.PI*2),t.stroke(),t.shadowBlur=0,l.num(t,S.n,E,O,U*.9,Math.round(U*.78)),t.globalAlpha=1}const W=(S,P,f,k)=>{const E=13*d.halo,O=o.per||.4,U=Math.max(0,Math.min(1,k/O)),H=Math.min(.16,O*.42),z=Math.max(0,Math.min(1,1-k/H)),Q=f*1.9;for(let $=2;$>=0;$--){const X=Math.pow(Math.max(0,U-$*.05),1.6),Z=Q-(Q-f)*X,V=$===0?.55+.35*X:.16/$;t.save(),t.translate(S,P),o0(t,e,Z,.5+.35*X,V,n*.45,E,1.15-.35*X),t.restore()}z>.01&&(t.save(),t.translate(S,P),o0(t,e,f*(1+1.5*(1-z)),.92,z*.9,n*1.7,E,1.1),t.restore());const J=Math.max(0,Math.min(1,1-k/.22)),j=J*J;if(j>.01){const $=Math.max(1,f*(1.5+1.8*(1-J))),X=t.createRadialGradient(S,P,0,S,P,$);X.addColorStop(0,`rgba(255,255,255,${Math.min(1,1.15*j).toFixed(3)})`),X.addColorStop(.35,M(.92,(.8*j).toFixed(3))),X.addColorStop(1,M(.7,0)),t.save(),t.shadowBlur=0,t.fillStyle=X,t.beginPath(),t.arc(S,P,$,0,Math.PI*2),t.fill(),t.restore()}},q=(o.targets||[]).filter(S=>S&&S.hit!=null&&S.hit<1.2);for(const S of q){const P=(S.r!=null?S.r:.2)*a/2*(S.live?1.34:1);W(L(S.x),R(S.y),P,S.hit)}const Y=o.center?o.center.hit!=null?o.center.hit:o.hit!=null?m:null:null;if(Y!=null&&Y<1.2&&W(L(F),R(T),C,Y),o.center){const S=L(F),P=R(T),f=13*d.halo*(h?.45:1),k=t.createRadialGradient(S,P,C*.1,S,P,C*1.6);k.addColorStop(0,M(.42,.3)),k.addColorStop(.6,M(.35,.12)),k.addColorStop(1,M(.3,0)),t.fillStyle=k,t.beginPath(),t.arc(S,P,C*1.6,0,Math.PI*2),t.fill();const E=!!o.center.live,O=C*(E?1.34:1);t.shadowBlur=f*1.4,t.shadowColor=e(.5),t.fillStyle=e(h?E?.26:.62:E?.5:.36),t.beginPath(),t.arc(S,P,O*.88,0,Math.PI*2),t.fill(),t.shadowBlur=0,t.save(),t.translate(S,P),o0(t,e,O,E?.8:.5,E?.9:.5,n*.9,f),t.restore();const U=o.center.v;if(t.strokeStyle=U==="hit"?K.prism:U==="near"?K.sand:U==="miss"?t0.lo:e(E?.8:.45),t.lineWidth=n*(E?1.3:U?1.15:.9),t.shadowBlur=E?f*1.6:U&&U!=="miss"?f*1.3:f*.6,t.shadowColor=U==="hit"?K.prism:U==="near"?K.sand:e(.5),t.beginPath(),t.arc(S,P,O,0,Math.PI*2),t.stroke(),t.shadowBlur=0,o.brand&&l.logo&&l.logo.complete&&l.logo.naturalWidth){const H=C*1.05,z=H*l.logo.naturalHeight/l.logo.naturalWidth;t.globalAlpha=.5*B,t.drawImage(l.logo,S-H/2,P-z/2,H,z),t.globalAlpha=1}if(o.center.label){const H=String(o.center.label).split(`
`);H.forEach((z,Q)=>w(z,S,P+(Q-(H.length-1)/2)*15*c,12*c,r(.9),.14))}}if(o.prog>.001&&o.center){const S=L(F),P=R(T),f=I;t.strokeStyle=M(.34,.9+.1*f),t.lineWidth=n*(.85+.5*f),t.lineCap="round",t.shadowColor=e(.42),t.shadowBlur=(10+16*f)*c,t.beginPath(),t.arc(S,P,C,-Math.PI/2,-Math.PI/2+Math.PI*2*Math.min(1,o.prog)),t.stroke(),t.shadowBlur=0}}function ho(t,a,o,d,i,l,e){const c=l.lut,h=13*d.halo,v=a/220,u=a/2,p=l.arrow&&l.arrow.w||1,r=(M,w)=>c(M).replace("rgb(","rgba(").replace(")",`,${w})`);t.clearRect(0,0,a,a),t.lineJoin="round",t.lineCap="round";const n=(o.r!=null?o.r:.42)*a,s=n*(o.rt!=null?o.rt:.36),A=3.4*p*v,B=e!=null?Math.max(0,Math.min(1,e)):i*(o.tempo||.6)%1,D=Math.pow(B,1.6),m=Math.max(0,(B-.9)/.1);t.save(),t.translate(u,u);const I=(M,w,x,b=1)=>o0(t,c,M,w,x,A,h,b),L=t.createRadialGradient(0,0,0,0,0,s*1.08);L.addColorStop(0,r(.6,.1+.18*m)),L.addColorStop(.65,r(.5,.05+.08*m)),L.addColorStop(1,r(.5,0)),t.globalAlpha=1,t.fillStyle=L,t.beginPath(),t.arc(0,0,s*1.08,0,Math.PI*2),t.fill();const R=1+.02*Math.sin(i*2.6);I(s*R,.55+.4*m,.5+.45*m,.9);for(let M=2;M>=0;M--){const w=Math.pow(Math.max(0,B-M*.05),1.6),x=n-(n-s)*w,b=M===0?.6+.4*D:.18/M*(1-m);I(x,.55+.4*D,b*(1-m*.45),1.15-.35*D)}m>.01&&I(s*(1+1.4*m),.9,(1-m)*.8,1.1),t.globalAlpha=.6+.3*m,t.shadowColor=c(.85),t.shadowBlur=h*(.9+m),t.fillStyle=c(.62+.3*m),t.beginPath(),t.arc(0,0,A*.85+3*v*m,0,Math.PI*2),t.fill(),t.restore(),t.globalAlpha=1,t.shadowBlur=0}function uo(t,a,o,d,i,l,e,c){const h=l.lut,v=13*d.halo,u=a/220,p=a/2,r=l.arrow&&l.arrow.w||1,n=r*u,s=(f,k)=>h(f).replace("rgb(","rgba(").replace(")",","+k+")");t.clearRect(0,0,a,a),t.lineJoin="round",t.lineCap="round";const A=a*.42*(o.spread!=null?o.spread:1),D=(c||[[-.95,.5],[-.48,-.42],[0,-.8],[.48,-.42],[.95,.5]]).map(([f,k])=>[p+f*A,p+k*A]),m=80,I=[];for(let f=0;f<=m;f++){const k=f/m*(D.length-1),E=Math.min(D.length-2,Math.floor(k)),O=k-E,U=D[Math.max(0,E-1)],H=D[E],z=D[E+1],Q=D[Math.min(D.length-1,E+2)],J=(j,$,X,Z)=>.5*(2*$+(-j+X)*O+(2*j-5*$+4*X-Z)*O*O+(-j+3*$-3*X+Z)*O*O*O);I.push([J(U[0],H[0],z[0],Q[0]),J(U[1],H[1],z[1],Q[1])])}const L=f=>{const k=Math.max(0,Math.min(m,f*m)),E=Math.floor(k),O=k-E,U=I[E],H=I[Math.min(m,E+1)];return[U[0]+(H[0]-U[0])*O,U[1]+(H[1]-U[1])*O]},R=.68;let M,w,x;if(e!=null)M=Math.max(0,Math.min(1,e)),w=1,x=0;else{const f=i*(o.tempo||.42)%1;if(f<R)M=f/R,w=1,x=0;else{const k=(f-R)/(1-R);M=1,w=1-k*k,x=k}}if(w<=.012)return;const b=M*M*M*(M*(6*M-15)+10),F=Math.min(1,16*M*M*(1-M)*(1-M));o.taper!=null&&o.taper;const T=.36*(o.tail!=null?o.tail:1),C=o.width!=null?o.width:1;{const f=t.createLinearGradient(I[0][0],I[0][1],I[m][0],I[m][1]);f.addColorStop(0,s(.46,0)),f.addColorStop(.3,s(.46,.03*w)),f.addColorStop(.8,s(.46,.045*w)),f.addColorStop(1,s(.46,0)),t.globalAlpha=1,t.strokeStyle=f,t.lineWidth=9*n,t.shadowColor=h(.6),t.shadowBlur=v*2,t.beginPath(),I.forEach(([k,E],O)=>O?t.lineTo(k,E):t.moveTo(k,E)),t.stroke(),t.shadowBlur=0}const _=40,G=Math.max(0,b-T*(1-x)),y=[];for(let f=0;f<=_;f++)y.push(L(G+(b-G)*(f/_)));const W=()=>{t.beginPath(),y.forEach(([f,k],E)=>E?t.lineTo(f,k):t.moveTo(f,k)),t.stroke()},q=()=>{const f=t.createLinearGradient(y[0][0],y[0][1],y[_][0],y[_][1]);return f.addColorStop(0,s(.55,0)),f.addColorStop(.4,s(.56,0)),f.addColorStop(.68,s(.6,.09)),f.addColorStop(.88,s(.64,.24)),f.addColorStop(1,s(.68,.44)),f},Y=1+.5*F;t.globalAlpha=w,t.strokeStyle=q(),t.lineWidth=(20+10*F)*n*C,t.shadowColor=h(.72),t.shadowBlur=v*2.2,W(),t.strokeStyle=q(),t.lineWidth=(10+5*F)*n*C,t.shadowBlur=v*1,W(),t.shadowBlur=0;for(let f=1;f<=_;f++){const k=f/_;t.globalAlpha=Math.pow(k,2.2)*.95*w,t.strokeStyle=h(.55+.38*k),t.lineWidth=(1.6+6.5*Math.pow(k,.7))*n*C*Y,t.beginPath(),t.moveTo(y[f-1][0],y[f-1][1]),t.lineTo(y[f][0],y[f][1]),t.stroke()}const S=y[_][0],P=y[_][1];t.globalAlpha=.8*w,t.fillStyle=h(.6),t.shadowColor=h(.8),t.shadowBlur=v*1.6,t.beginPath(),t.arc(S,P,(9+5*F)*n*C,0,Math.PI*2),t.fill(),t.globalAlpha=w,t.fillStyle=h(.93),t.shadowBlur=v*.6,t.beginPath(),t.arc(S,P,(3.4+1.8*F)*n*C,0,Math.PI*2),t.fill(),t.globalAlpha=1,t.shadowBlur=0}function po(t,a,o,d,i,l,e){const c=o.r!=null?o.r:.3,h=o.dir!=null?o.dir:1,v=(o.sweep!=null?o.sweep:.66)*Math.PI*2,u=e??i*(o.tempo!=null?o.tempo:.5)%1,p=-Math.PI/2+h*u*Math.PI*2,r=16,n=[];for(let s=0;s<=r;s++){const A=p-h*(1-s/r)*v;n.push([.5+Math.cos(A)*c,.5+Math.sin(A)*c])}Wt(t,a,a,n,i,l,{prog:1,tail:o.tail!=null?o.tail:.68,scale:(o.scale!=null?o.scale:1)*(o.width!=null?o.width:1)})}function fo(t,a,o,d,i,l,e={}){const c=Math.max(2,Math.round(e.count??7)),h=e.r??4.4,v=e.flow??.35,u=e.hair??.26,p=e.style||"chain",r=e.color||"255,246,234",n=e.inset??.12,s=e.alpha??1,A=d-a,B=i-o,D=Math.hypot(A,B);if(D<.001||s<=.004)return;const m=A/D,I=B/D,L=a+A*n,R=o+B*n,M=D*(1-n*2);if(M<=0)return;t.save(),p==="chain"&&u>0&&(t.strokeStyle=`rgba(${r},${(u*s).toFixed(3)})`,t.lineWidth=Math.max(1,h*.34),t.beginPath(),t.moveTo(L,R),t.lineTo(L+m*M,R+I*M),t.stroke());const w=l*v%1;for(let x=0;x<c;x++){let b=(x+.5)/c;if(p==="pulse"||p==="dots"||p==="chain"){const y=b-.5;if(b=.5+y+Math.sign(y)*(w/c%(1/c)),b<0||b>1)continue}const F=L+m*M*b,T=R+I*M*b,C=Math.abs(b-.5)*2;let _=h,G=s;if(p==="taper"&&(_=h*(.55+.75*C)),p==="pulse"){const y=((C-w)%1+1)%1;G=s*(.35+.65*Math.max(0,1-Math.abs(y-0)*3))}t.fillStyle=`rgba(${r},${(G*.96).toFixed(3)})`,t.beginPath(),t.arc(F,T,_,0,Math.PI*2),t.fill()}t.restore()}export{b0 as A,eo as C,Gt as F,m0 as G,Et as L,to as M,t0 as N,K as P,Qt as Q,ao as R,Jt as S,Zt as Z,a0 as a,Ht as b,ho as c,Vt as d,po as e,so as f,uo as g,co as h,Nt as i,fo as j,io as k,Yt as l,Xt as m,ro as n,oo as o,no as p,r0 as q,qt as r,f0 as s,zt as t,jt as u,lo as v,$t as w,Wt as x,gt as y,S0 as z};
