...
vec4 markState(vec2 uv, float state, float prog, float strong, float t){
  ...
  return A;
}`;function Dt(t,a,o,d){t.lineWidth=4*a;const i=d.arrow;i.line==="dash"?t.setLineDash([12*a*i.gap,10*i.gap]):i.line==="dot"?(t.setLineDash([.5,12*i.gap]),t.lineCap="round",t.lineWidth=5*a):t.setLineDash([]),o!=null&&i.line!=="solid"&&i.line!=="taper"&&(t.lineDashOffset=-o*40*i.speed)}function Nt(t,a,o,d,i,e={}){const l=i.lut,r=i.arrow||{},p=r.w??1,v=r.speed??1,h=r.glow??1,u=e.pulse??1,s=o/256,c=s*(e.scale??1),n=a/2,R=d*.9*v%1,G=e.prog!=null?Math.max(0,Math.min(1,e.prog)):Math.min(1,R/.55),F=e.prog!=null?1:R>.88?(1-R)/.12:1;t.clearRect(0,0,a,o);const w=F*(.45+.55*u),I=o-24*s,L=58*s,T=I+(L-I)*G,m=(k,P)=>l(k).replace("rgb(","rgba(").replace(")",`,${P.toFixed(3)})`),x=1.1*c*p,f=13*c*p,b=42*c*(.7+.3*p),D=!e.noTip&&G>.28?Math.min(1,(G-.28)/.22)*b*.42:0,M=T+D,C=t.createLinearGradient(0,I,0,M);C.addColorStop(0,m(.55,0)),C.addColorStop(.18,m(.64,.3*w)),C.addColorStop(.4,m(.76,.8*w)),C.addColorStop(.65,m(.88,.98*w)),C.addColorStop(1,m(.97,w));const E=e.dots?(()=>{const k=Math.abs(M-I),P=Math.max(3,Math.round(k/(9.5*c))),S=d*1.15*v%1,y=(H,U,A)=>{const _=Math.max(0,Math.min(1,(A-H)/(U-H)));return _*_*(3-2*_)},B=[];for(let H=0;H<P;H++){const U=((H+.5)/P+S/P)%1,A=1+.22*Math.sin((U*3-S*2)*Math.PI*2);B.push({y:I+(M-I)*U,r:(x/2+(f/2-x/2)*U)*A,a:(1-.35*y(.72,.92,U))*(1-y(.92,1,U))})}return B})():null;if(t.save(),t.filter=`blur(${7*c}px)`,t.globalAlpha=e.dots?.3:.55,t.fillStyle=C,e.dots){const k=t.globalAlpha;for(const P of E)t.globalAlpha=k*P.a,t.beginPath(),t.arc(n,P.y,Math.max(1.4*c,P.r*1.5),0,Math.PI*2),t.fill();t.globalAlpha=k}else t.beginPath(),t.moveTo(n-x,I),t.lineTo(n+x,I),t.lineTo(n+f*.95,M),t.lineTo(n-f*.95,M),t.closePath(),t.fill();if(t.restore(),t.globalAlpha=1,t.fillStyle=C,e.dots){for(const k of E)t.globalAlpha=k.a,t.beginPath(),t.arc(n,k.y,Math.max(.9*c,k.r*.92),0,Math.PI*2),t.fill();t.globalAlpha=1}else t.beginPath(),t.moveTo(n-x/2,I),t.lineTo(n+x/2,I),t.lineTo(n+f/2,M),t.lineTo(n-f/2,M),t.closePath(),t.fill();if(t.globalAlpha=w,G>.28&&!e.noTip){const k=b,P=Math.min(1,(G-.28)/.22)*w,S=T+k*.3;t.globalAlpha=P;const y={color:l(.95),glowColor:l(.85),glow:12*h};i.glyph&&(i.glyph(t,"LIFT_TIP",n,S,k,y)||i.glyph(t,"TIP_TRI",n,S,k*.93,y))||(t.strokeStyle=l(.95),t.lineWidth=13*c*p,t.lineCap="round",t.lineJoin="round",t.shadowColor=l(.9),t.shadowBlur=18*c*h,t.beginPath(),t.moveTo(n-26*c,S+14*c),t.lineTo(n,S-16*c),t.lineTo(n+26*c,S+14*c),t.stroke())}t.globalAlpha=1,t.shadowBlur=0}function Bt(t,a,o,d,i,e,l={}){const r=e.lut,p=e.arrow||{},v=p.w??1,h=p.glow??1,u=o/256*(l.scale??1);t.clearRect(0,0,a,o);const s=d.map(([m,x])=>[m*a,x*o]);if(s.length<2)return;const c=48,n=[],R=m=>{if(s.length===2)return[s[0][0]+(s[1][0]-s[0][0])*m,s[0][1]+(s[1][1]-s[0][1])*m];const x=m*(s.length-1),f=Math.min(s.length-2,Math.floor(x)),b=x-f,D=s[Math.max(0,f-1)],M=s[f],C=s[f+1],E=s[Math.min(s.length-1,f+2)],k=(P,S,y,B)=>.5*(2*S+(-P+y)*b+(2*P-5*S+4*y-B)*b*b+(-P+3*S-3*y+B)*b*b*b);return[k(D[0],M[0],C[0],E[0]),k(D[1],M[1],C[1],E[1])]};for(let m=0;m<=c;m++)n.push(R(m/c));const G=Math.max(0,Math.min(1,l.prog!=null?l.prog:i*.55%1)),F=Math.max(1,Math.round(c*G)),w=l.alpha??1,I=l.tail??.22;t.lineCap="round";const L=42*u*(.7+.3*v);let T=F;if(G>.28){let m=0;const x=L*.42*Math.min(1,(G-.28)/.22);for(;T>1&&m<x;)m+=Math.hypot(n[T][0]-n[T-1][0],n[T][1]-n[T-1][1]),T--}for(const m of[0,1])for(let x=1;x<=T;x++){const f=x/F,b=Math.min(1,f/I),D=b*b*(3-2*b)*w;t.globalAlpha=m?D:D*.16*f,t.strokeStyle=r(.55+.42*f),t.lineWidth=(1.1+11.9*f)*u*v*(m?1:1.9),t.beginPath(),t.moveTo(n[x-1][0],n[x-1][1]),t.lineTo(n[x][0],n[x][1]),t.stroke()}if(G>.28){const m=n[Math.max(0,F-2)][0],x=n[Math.max(0,F-2)][1],f=Math.atan2(n[F][1]-x,n[F][0]-m)+Math.PI/2,b=n[F][0]-Math.sin(f)*L*.3,D=n[F][1]+Math.cos(f)*L*.3;t.save(),t.translate(b,D),t.rotate(f),t.globalAlpha=Math.min(1,(G-.28)/.22)*w;const M={color:r(.95),glowColor:r(.85),glow:12*h};e.glyph&&(e.glyph(t,"LIFT_TIP",0,0,L,M)||e.glyph(t,"TIP_TRI",0,0,L*.93,M))||(t.strokeStyle=r(.95),t.lineWidth=9*u*v,t.lineJoin="round",t.lineCap="round",t.beginPath(),t.moveTo(-18*u,12*u),t.lineTo(0,-14*u),t.lineTo(18*u,12*u),t.stroke()),t.restore()}t.globalAlpha=1}function Et(t,a,o,d,i,e){i=i||{};const l=e.lut,r=i.style||e.arrow.line,p=!!i.closed,v=[0];for(let c=1;c<a.length;c++)v.push(v[c-1]+Math.hypot(a[c][0]-a[c-1][0],a[c][1]-a[c-1][1]));const h=v[v.length-1]||1,u=c=>{c=(c%h+h)%h;let n=1;for(;n<v.length-1&&v[n]<c;)n++;const R=(c-v[n-1])/Math.max(1e-4,v[n]-v[n-1]);return[a[n-1][0]+(a[n][0]-a[n-1][0])*R,a[n-1][1]+(a[n][1]-a[n-1][1])*R,Math.atan2(a[n][1]-a[n-1][1],a[n][0]-a[n-1][0])]},s=e.arrow;if(r==="chevron"){const c=(26*d+8)*s.gap,n=Math.max(2,Math.floor(h/c));t.shadowColor=l(Math.min(1,s.heat+.2)),t.shadowBlur=8*d*s.glow;for(let R=0;R<n;R++){const G=R*c+o*42*s.speed%c;if(!p&&G>h-4)continue;const[F,w,I]=u(G),L=7.5*d,T=8.5*d,m=.45+.4*Math.sin(G/h*6.283-o*2.2*s.speed);t.strokeStyle=l(s.heat-.05+m*.3),t.lineWidth=3.2*d,t.lineJoin="round",t.lineCap="round",t.save(),t.translate(F,w),t.rotate(I),t.beginPath(),t.moveTo(-T*.5,-L),t.lineTo(T*.5,0),t.lineTo(-T*.5,L),t.stroke(),t.restore()}return!0}if(r==="comet"){const c=o*.35*s.speed%1*h,n=h*s.tail,R=Math.max(24,a.length*2);t.lineCap="round";for(let w=0;w<R;w++){const I=c-w/R*n,L=c-(w+1)/R*n;if(!p&&L<0)break;const T=1-w/R,[m,x]=u(I),[f,b]=u(L);!p&&Math.hypot(f-m,b-x)>h*.4||(t.globalAlpha=Math.pow(T,1.6),t.strokeStyle=l(Math.max(.05,s.heat-.2)+T*.55),t.lineWidth=(1.5+T*4.5)*d,T>.72?(t.shadowColor=l(Math.min(1,s.heat+.3)),t.shadowBlur=T*12*d*s.glow):t.shadowBlur=0,t.beginPath(),t.moveTo(m,x),t.lineTo(f,b),t.stroke())}t.globalAlpha=1,t.lineCap="butt",t.shadowBlur=0;const[G,F]=u(c);return t.fillStyle=rgba(V.ink,.95),t.shadowColor=l(.9),t.shadowBlur=16*d,t.beginPath(),t.arc(G,F,2.6*d,0,7),t.fill(),t.shadowBlur=0,!0}if(t.strokeStyle=i.color||l(s.heat),t.shadowColor=l(Math.min(1,s.heat+.15)),t.shadowBlur=(i.glow??8)*d*s.glow,r==="taper")for(let c=1;c<a.length;c++)t.lineWidth=(.5+c/a.length*4.5)*d,t.beginPath(),t.moveTo(a[c-1][0],a[c-1][1]),t.lineTo(a[c][0],a[c][1]),t.stroke();else Dt(t,d,o,e),t.beginPath(),a.forEach(([c,n],R)=>R?t.lineTo(c,n):t.moveTo(c,n)),p&&t.closePath(),t.stroke();return t.setLineDash([]),t.lineCap="butt",t.lineDashOffset=0,t.shadowBlur=0,!0}function to(t,a,o,d,i,e){const l=13*d.halo,r=e.lut;t.clearRect(0,0,a,a),t.lineJoin="round";const p=a/220,v=a/2,h=18*o.round*p,u=40*p,s=48*p,c=a-80*p,n=a-96*p,R=[],G=(w,I,L,T)=>{for(let m=0;m<=1;m+=.12)R.push([w+(L-w)*m,I+(T-I)*m])};G(u+h,s,u+c-h,s),G(u+c,s+h,u+c,s+n-h),G(u+c-h,s+n,u+h,s+n),G(u,s+n-h,u,s+h),t.shadowColor=r(.6),t.shadowBlur=l*.8;const F=4*e.arrow.w*p;if(e.arrow.line==="solid"?(t.setLineDash([10*o.dash*p,8*p]),t.lineDashOffset=-i*22*p,t.strokeStyle=r(.45),t.lineWidth=F,t.beginPath(),t.roundRect(u,s,c,n,h),t.stroke(),t.setLineDash([]),t.lineDashOffset=0):Et(t,R,i,e.arrow.w*p,{color:r(.45),closed:!0},e),o.prog!=null&&o.prog>.001){const w=[],I=(C,E,k,P,S)=>{for(let y=1;y<=S;y++)w.push([C+(k-C)*y/S,E+(P-E)*y/S])},L=(C,E,k,P,S)=>{for(let y=1;y<=S;y++){const B=k+(P-k)*y/S;w.push([C+h*Math.cos(B),E+h*Math.sin(B)])}},T=Math.PI/2,m=u+c,x=s+n,f=u+c/2;w.push([f,s]),I(f,s,m-h,s,8),L(m-h,s+h,-T,0,6),I(m,s+h,m,x-h,10),L(m-h,x-h,0,T,6),I(m-h,x,u+h,x,14),L(u+h,x-h,T,Math.PI,6),I(u,x-h,u,s+h,10),L(u+h,s+h,Math.PI,Math.PI+T,6),I(u+h,s,f,s,8);let b=0;for(let C=1;C<w.length;C++)b+=Math.hypot(w[C][0]-w[C-1][0],w[C][1]-w[C-1][1]);const D=b*Math.min(1,o.prog);t.save(),t.setLineDash([10*o.dash*p,8*p]),t.lineDashOffset=-(c/2-h)-i*22*p,t.strokeStyle=r(.9),t.lineWidth=F*1.3,t.lineCap="round",t.shadowColor=r(.92),t.shadowBlur=l*1.3,t.beginPath(),t.moveTo(w[0][0],w[0][1]);let M=0;for(let C=1;C<w.length&&M<D;C++){const E=Math.hypot(w[C][0]-w[C-1][0],w[C][1]-w[C-1][1]);if(M+E<=D)t.lineTo(w[C][0],w[C][1]),M+=E;else{const k=(D-M)/E;t.lineTo(w[C-1][0]+(w[C][0]-w[C-1][0])*k,w[C-1][1]+(w[C][1]-w[C-1][1])*k),M=D}}t.stroke(),t.setLineDash([]),t.restore()}o.feet>.05&&e.foot&&(e.foot(t,!1,v-16*o.feet*p,v+6*p,26*o.feet*p),e.foot(t,!0,v+16*o.feet*p,v+6*p,26*o.feet*p)),t.shadowBlur=0}function oo(t,a,o,d,i,e,l,r){const p=13*d.halo,v=4*e.arrow.w*(a/220),h=e.lut;t.clearRect(0,0,a,a),t.lineJoin="round";const u=a/220,s=l||[[45*u,130*u],[110*u,60*u],[175*u,110*u]],c=r??i*.5%1,n=Math.min(1,c*1.25)*(s.length-1),R=Math.min(s.length-1,Math.floor(n+.35));t.shadowColor=h(.7),t.shadowBlur=p;const G=x=>{const f=Math.max(0,Math.min(s.length-2,Math.floor(x))),b=x-f;return[s[f][0]+(s[f+1][0]-s[f][0])*b,s[f][1]+(s[f+1][1]-s[f][1])*b]},F=o.comet!=null?o.comet:1,w=o.tailLen!=null?o.tailLen:.5;if(t.save(),t.shadowBlur=0,t.globalAlpha=o.rail!=null?o.rail:.22,t.strokeStyle=h(.6),t.lineWidth=2.2*u,t.lineCap="round",t.setLineDash([.01,8*u]),t.beginPath(),s.forEach(([x,f],b)=>b?t.lineTo(x,f):t.moveTo(x,f)),t.stroke(),t.setLineDash([]),t.restore(),n>.02){const x=(f,b)=>h(f).replace("rgb(","rgba(").replace(")",`,${b.toFixed(3)})`);for(let f=0;f<26;f++){const b=f/25,D=n-w*b;if(D<=0)break;const M=G(D),C=4.7*u*F*(1-b*.75),E=C*2.1,k=(1-b)*(1-b)*.8,P=t.createRadialGradient(M[0],M[1],0,M[0],M[1],E);P.addColorStop(0,x(.55-.18*b,k)),P.addColorStop(.5,x(.5-.18*b,k*.35)),P.addColorStop(1,x(.45-.18*b,0)),t.save(),t.shadowBlur=0,t.globalAlpha=1,t.fillStyle=P,t.beginPath(),t.arc(M[0],M[1],E,0,Math.PI*2),t.fill(),t.restore()}if(n<s.length-1-.001&&n-Math.floor(n)>.03){const f=G(n);t.save(),t.fillStyle=h(.62),t.shadowColor=h(.8),t.shadowBlur=11*u*F,t.beginPath(),t.arc(f[0],f[1],5.6*u*F,0,Math.PI*2),t.fill(),t.fillStyle=h(.95),t.globalAlpha=.95,t.beginPath(),t.arc(f[0],f[1],2.1*u*F,0,Math.PI*2),t.fill(),t.restore()}}t.globalAlpha=1,t.lineCap="butt";const I=o.done!=null,L=o.done||0,T=o.vd||[],m=o.acc!=null?o.acc:s.length-1;if(s.forEach(([x,f],b)=>{const D=b<L,M=T[b]||null,C=!D&&b===R&&!I,E=C?1+Math.sin(i*6)*.14:1,k=o.pop!=null&&b===L-1&&o.pop<.3?1+.35*(1-o.pop/.3):1,P=12*o.node*E*k*u*(b===m?1.34:1);D&&M!=="miss"&&(t.shadowBlur=p*1.4,t.shadowColor=h(.5),t.fillStyle=h(M==="near"?.5:.36),t.beginPath(),t.arc(x,f,P*.88,0,Math.PI*2),t.fill()),D||(t.save(),t.translate(x,f),N(t,h,P,C?.8:.5,C?.9:.5,v*.9,p),t.restore()),t.strokeStyle=M==="hit"?$.prism:M==="near"?$.sand:M==="miss"?V.lo:h(D?.62:C?.8:.45),t.lineWidth=v*(C?1.3:M?1.15:.9),t.shadowBlur=C?p*1.6:M&&M!=="miss"?p*1.3:p*.6,M&&M!=="miss"&&(t.shadowColor=M==="hit"?$.prism:$.sand),t.beginPath(),t.arc(x,f,P,0,Math.PI*2),t.stroke(),e.num&&(D&&M!=="miss"?(t.save(),t.globalCompositeOperation="destination-out",t.shadowBlur=0,e.num(t,String(b+1),x,f,16*o.numS*E*u,Math.round(14*o.numS*u)),t.restore()):(t.globalAlpha=b<=R?1:.6,e.num(t,String(b+1),x,f,16*o.numS*E*u,Math.round(14*o.numS*u)),t.globalAlpha=1))}),o.pop!=null&&o.pop<.38&&L>0&&s[L-1]){const[x,f]=s[L-1],b=1-o.pop/.38,D=b*b,M=12*o.node*u*(L-1===(o.acc!=null?o.acc:s.length-1)?1.34:1);t.save();const C=M*(1.6+1.9*(1-b)),E=t.createRadialGradient(x,f,0,x,f,C);E.addColorStop(0,`rgba(255,255,255,${Math.min(1,1.2*D).toFixed(3)})`),E.addColorStop(.35,h(.92).replace("rgb(","rgba(").replace(")",`,${(.8*D).toFixed(3)})`)),E.addColorStop(1,h(.7).replace("rgb(","rgba(").replace(")",",0)")),t.fillStyle=E,t.beginPath(),t.arc(x,f,C,0,Math.PI*2),t.fill(),t.strokeStyle=h(.92),t.globalAlpha=D,t.lineWidth=Math.max(1,6*u*b),t.shadowColor=h(.85),t.shadowBlur=24*u*D,t.beginPath(),t.arc(x,f,M*(1.1+3.4*(1-b)),0,Math.PI*2),t.stroke();const k=Math.max(0,b-.35)/.65;k>0&&(t.globalAlpha=k*k*.6,t.lineWidth=Math.max(1,3.5*u*k),t.beginPath(),t.arc(x,f,M*(1.1+2*(1-k)),0,Math.PI*2),t.stroke()),t.restore()}t.shadowBlur=0}function N(t,a,o,d,i,e,l,r=1){if(o<=.6)return;const p=(c,n)=>a(c).replace("rgb(","rgba(").replace(")",`,${n})`),v=e*2.6*r,h=Math.max(.1,o-v),u=o+v,s=t.createRadialGradient(0,0,h,0,0,u);s.addColorStop(0,p(d-.05,0)),s.addColorStop(.5,p(d,i*.85)),s.addColorStop(1,p(d-.05,0)),t.globalAlpha=1,t.fillStyle=s,t.shadowBlur=0,t.beginPath(),t.arc(0,0,u,0,Math.PI*2),t.fill(),t.globalAlpha=Math.min(1,i*1.1),t.lineWidth=e*.85,t.strokeStyle=a(Math.min(.98,d+.12)),t.shadowColor=a(.88),t.shadowBlur=l*.6,t.beginPath(),t.arc(0,0,o,0,Math.PI*2),t.stroke(),t.shadowBlur=0}const a0=t=>1-Math.pow(1-Math.max(0,Math.min(1,t)),5);function ao(t,a,o,d,i,e){const l=e.lut,r=a/512,p=e?.day?1:0,v=e.arrow&&e.arrow.w||1,h=(S,y)=>l(S).replace("rgb(","rgba(").replace(")",","+y+")"),u=p?.95:.86,s=(S,y)=>h(u,S),c=4*v*r;t.clearRect(0,0,a,a),t.lineJoin="round",t.lineCap="round";const n=o.mat||{nx:.86,fx:.86,ny:-.86,fy:.86},R=o.in==null?1:Math.max(0,Math.min(1,o.in)),G=a0(R/.45),F=S=>a0((R-.28-S*.09)/.42),w=o.hit==null?9:o.hit,I=w<.34?Math.sin(Math.PI*(w/.34)):0,L=S=>a/2+S*a/2,T=S=>a/2-S*a/2,m=(S,y)=>l(S).replace("rgb(","rgba(").replace(")",","+y+")"),x=(S,y,B,H,U,A,_)=>{t.font="700 "+H+'px "Supreme", sans-serif',t.textAlign="center",t.textBaseline="middle";const O=Array.from(String(S)),W=H*A;let q=-W;for(const g of O)q+=t.measureText(g).width+W;let K=y-q/2;t.fillStyle=U;for(const g of O){const X=t.measureText(g).width;t.fillText(g,K+X/2,B),K+=X+W}},f=.05*(o.round!=null?o.round/.35:1),b=(S,y,B,H)=>{};b(-n.fx+f,n.fy-f),b(-n.nx+f*.6,n.ny+f),b(n.nx-f*.6,n.ny+f),b(n.fx-f,n.fy-f);const D=o.center?o.center.x:0,M=o.center?o.center.y:0,C=(o.center&&o.center.r!=null?o.center.r:.26)*a/2*G*(1+.055*I+.008*Math.sin(i*2.4)),E=(o.targets||[]).slice().sort((S,y)=>(S.n||0)-(y.n||0));if(E.length){t.save(),t.shadowBlur=0,t.globalAlpha=.22*a0(R/.9),t.strokeStyle=l(.6),t.lineWidth=2.2*r,t.lineCap="round",t.setLineDash([.01,8*r]);const S=o.travel||{};for(const y of E){const B=L(y.x),H=T(y.y),U=L(D)-B,A=T(M)-H,_=S.k>0&&S.k<1&&(y.n===S.from||y.n===S.to),O=Math.hypot(U,A)||1,W=(y.r!=null?y.r:.2)*a/2*1.16,q=B+U/O*W,K=H+A/O*W,g=L(D)-U/O*C*1.12,X=T(M)-A/O*C*1.12;if(t.beginPath(),t.moveTo(q,K),t.lineTo(g,X),t.stroke(),I>.01){const z=1-I,J=.34,Y=t.createLinearGradient(g,X,q,K),Z=(o0,p0)=>Y.addColorStop(Math.max(0,Math.min(1,o0)),l(.85).replace("rgb(","rgba(").replace(")",","+p0+")"));Z(z-J,0),Z(z,.9*I),Z(z+J,0),t.save(),t.setLineDash([]),t.globalAlpha=1,t.strokeStyle=Y,t.lineWidth=3.4*r,t.stroke(),t.restore()}if(_){const z=t.createLinearGradient(q,K,g,X),J=Math.sin(Math.PI*S.k)*.85;z.addColorStop(0,l(.5).replace("rgb(","rgba(").replace(")",",0)")),z.addColorStop(.5,l(.9).replace("rgb(","rgba(").replace(")",","+J+")")),z.addColorStop(1,l(.5).replace("rgb(","rgba(").replace(")",",0)")),t.save(),t.setLineDash([]),t.globalAlpha=1,t.strokeStyle=z,t.lineWidth=3*r,t.stroke(),t.restore()}}t.setLineDash([]),t.restore()}const k=o.travel;if(k&&k.k>.001&&k.k<1){const S=E.find(B=>B.n===k.from),y=E.find(B=>B.n===k.to);if(S&&y){const B=L(S.x),H=T(S.y),U=L(y.x),A=T(y.y),_=L(D),O=T(M),W=a0(k.k),q=Y=>Y<.5?[B+(_-B)*(Y/.5),H+(O-H)*(Y/.5)]:[_+(U-_)*((Y-.5)/.5),O+(A-O)*((Y-.5)/.5)],[K,g]=q(W),[X,z]=q(Math.max(0,W-.28)),J=t.createLinearGradient(X,z,K,g);J.addColorStop(0,l(.5).replace("rgb(","rgba(").replace(")",",0)")),J.addColorStop(1,l(.85).replace("rgb(","rgba(").replace(")",",0.85)")),t.strokeStyle=J,t.lineWidth=3.2*r,t.lineCap="round",t.beginPath(),t.moveTo(X,z),t.lineTo(K,g),t.stroke(),t.fillStyle=l(.62),t.shadowColor=l(.8),t.shadowBlur=11*r,t.beginPath(),t.arc(K,g,5.6*r,0,Math.PI*2),t.fill(),t.fillStyle=l(.95),t.globalAlpha=.95,t.shadowBlur=0,t.beginPath(),t.arc(K,g,2.1*r,0,Math.PI*2),t.fill(),t.globalAlpha=1}}const P=13*d.halo*(p?.45:1);for(const S of o.targets||[]){const y=S.on!==!1,B=!!S.live,H=F(S.n?S.n-1:0);if(H<=.001)continue;const U=L(S.x),A=T(S.y),_=(S.r!=null?S.r:.2)*a/2*(B?1.34:1)*(.9+.1*H);t.globalAlpha=H*(y?1:.45),t.shadowBlur=P*1.4,t.shadowColor=l(.5),t.fillStyle=l(p?B?.26:.62:B?.5:.36),t.beginPath(),t.arc(U,A,_*.88,0,Math.PI*2),t.fill(),t.shadowBlur=0,t.save(),t.translate(U,A),N(t,l,_,B?.8:.5,B?.9:.5,c*.9,P),t.restore(),e.num(t,S.n,U,A,_*.9,Math.round(_*.78)),t.globalAlpha=1}if(o.center&&o.hit!=null){const S=L(D),y=T(M),B=13*d.halo,H=o.per||.4,U=Math.max(0,Math.min(1,w/H)),A=C*1.9;for(let _=2;_>=0;_--){const O=Math.pow(Math.max(0,U-_*.05),1.6),W=A-(A-C)*O,q=_===0?.55+.35*O:.16/_;t.save(),t.translate(S,y),N(t,l,W,.5+.35*O,q,c*.7,B,1.15-.35*O),t.restore()}I>.01&&(t.save(),t.translate(S,y),N(t,l,C*(1+1.5*(1-I)),.92,I*.85,c*.8,B,1.1),t.restore())}if(o.center){const S=L(D),y=T(M),B=t.createRadialGradient(S,y,C*.1,S,y,C*1.6);if(B.addColorStop(0,m(.42,.3)),B.addColorStop(.6,m(.35,.12)),B.addColorStop(1,m(.3,0)),t.fillStyle=B,t.beginPath(),t.arc(S,y,C*1.6,0,Math.PI*2),t.fill(),o.center.ring!==0&&(t.strokeStyle=m(.3,.3),t.lineWidth=c*.85,t.shadowColor=l(.42),t.shadowBlur=18*r,t.beginPath(),t.arc(S,y,C,0,Math.PI*2),t.stroke(),t.shadowBlur=0),o.brand&&e.logo&&e.logo.complete&&e.logo.naturalWidth){const H=C*1.05,U=H*e.logo.naturalHeight/e.logo.naturalWidth;t.globalAlpha=.5*G,t.drawImage(e.logo,S-H/2,y-U/2,H,U),t.globalAlpha=1}if(o.center.label){const H=String(o.center.label).split(`
`);H.forEach((U,A)=>x(U,S,y+(A-(H.length-1)/2)*15*r,12*r,s(.9),.14))}}if(o.chev){const S=(y,B)=>{for(let H=0;H<2;H++){const U=H*9*r,A=11*r,_=7*r,O=y+B*U;t.beginPath(),t.moveTo(L(D)-A,O-B*_),t.lineTo(L(D),O),t.lineTo(L(D)+A,O-B*_),t.stroke()}};t.strokeStyle=s(.45+.3*Math.sin(i*2.4)),t.lineWidth=2.2*v*r,S(T(M)-C*2,-1),S(T(M)+C*2,1)}if(o.prog>.001&&o.center){const S=L(D),y=T(M),B=I;t.strokeStyle=m(.34,.9+.1*B),t.lineWidth=c*(.85+.5*B),t.lineCap="round",t.shadowColor=l(.42),t.shadowBlur=(10+16*B)*r,t.beginPath(),t.arc(S,y,C,-Math.PI/2,-Math.PI/2+Math.PI*2*Math.min(1,o.prog)),t.stroke(),t.shadowBlur=0}}function eo(t,a,o,d,i,e,l){const r=e.lut,p=13*d.halo,v=a/220,h=a/2,u=e.arrow&&e.arrow.w||1,s=(m,x)=>r(m).replace("rgb(","rgba(").replace(")",`,${x})`);t.clearRect(0,0,a,a),t.lineJoin="round",t.lineCap="round";const c=(o.r!=null?o.r:.42)*a,n=c*(o.rt!=null?o.rt:.36),R=3.4*u*v,G=l!=null?Math.max(0,Math.min(1,l)):i*(o.tempo||.6)%1,F=Math.pow(G,1.6),w=Math.max(0,(G-.9)/.1);t.save(),t.translate(h,h);const I=(m,x,f,b=1)=>N(t,r,m,x,f,R,p,b),L=t.createRadialGradient(0,0,0,0,0,n*1.08);L.addColorStop(0,s(.6,.1+.18*w)),L.addColorStop(.65,s(.5,.05+.08*w)),L.addColorStop(1,s(.5,0)),t.globalAlpha=1,t.fillStyle=L,t.beginPath(),t.arc(0,0,n*1.08,0,Math.PI*2),t.fill();const T=1+.02*Math.sin(i*2.6);I(n*T,.55+.4*w,.5+.45*w,.9);for(let m=2;m>=0;m--){const x=Math.pow(Math.max(0,G-m*.05),1.6),f=c-(c-n)*x,b=m===0?.6+.4*F:.18/m*(1-w);I(f,.55+.4*F,b*(1-w*.45),1.15-.35*F)}w>.01&&I(n*(1+1.4*w),.9,(1-w)*.8,1.1),t.globalAlpha=.6+.3*w,t.shadowColor=r(.85),t.shadowBlur=p*(.9+w),t.fillStyle=r(.62+.3*w),t.beginPath(),t.arc(0,0,R*.85+3*v*w,0,Math.PI*2),t.fill(),t.restore(),t.globalAlpha=1,t.shadowBlur=0}function lo(t,a,o,d,i,e,l,r){const p=e.lut,v=13*d.halo,h=a/220,u=a/2,s=e.arrow&&e.arrow.w||1,c=s*h,n=(A,_)=>p(A).replace("rgb(","rgba(").replace(")",","+_+")");t.clearRect(0,0,a,a),t.lineJoin="round",t.lineCap="round";const R=a*.42*(o.spread!=null?o.spread:1),F=(r||[[-.95,.5],[-.48,-.42],[0,-.8],[.48,-.42],[.95,.5]]).map(([A,_])=>[u+A*R,u+_*R]),w=80,I=[];for(let A=0;A<=w;A++){const _=A/w*(F.length-1),O=Math.min(F.length-2,Math.floor(_)),W=_-O,q=F[Math.max(0,O-1)],K=F[O],g=F[O+1],X=F[Math.min(F.length-1,O+2)],z=(J,Y,Z,o0)=>.5*(2*Y+(-J+Z)*W+(2*J-5*Y+4*Z-o0)*W*W+(-J+3*Y-3*Z+o0)*W*W*W);I.push([z(q[0],K[0],g[0],X[0]),z(q[1],K[1],g[1],X[1])])}const L=A=>{const _=Math.max(0,Math.min(w,A*w)),O=Math.floor(_),W=_-O,q=I[O],K=I[Math.min(w,O+1)];return[q[0]+(K[0]-q[0])*W,q[1]+(K[1]-q[1])*W]},T=.68;let m,x,f;if(l!=null)m=Math.max(0,Math.min(1,l)),x=1,f=0;else{const A=i*(o.tempo||.42)%1;if(A<T)m=A/T,x=1,f=0;else{const _=(A-T)/(1-T);m=1,x=1-_*_,f=_}}if(x<=.012)return;const b=m*m*m*(m*(6*m-15)+10),D=Math.min(1,16*m*m*(1-m)*(1-m));o.taper!=null&&o.taper;const M=.36*(o.tail!=null?o.tail:1),C=o.width!=null?o.width:1;{const A=t.createLinearGradient(I[0][0],I[0][1],I[w][0],I[w][1]);A.addColorStop(0,n(.46,0)),A.addColorStop(.3,n(.46,.03*x)),A.addColorStop(.8,n(.46,.045*x)),A.addColorStop(1,n(.46,0)),t.globalAlpha=1,t.strokeStyle=A,t.lineWidth=9*c,t.shadowColor=p(.6),t.shadowBlur=v*2,t.beginPath(),I.forEach(([_,O],W)=>W?t.lineTo(_,O):t.moveTo(_,O)),t.stroke(),t.shadowBlur=0}const E=40,k=Math.max(0,b-M*(1-f)),P=[];for(let A=0;A<=E;A++)P.push(L(k+(b-k)*(A/E)));const S=()=>{t.beginPath(),P.forEach(([A,_],O)=>O?t.lineTo(A,_):t.moveTo(A,_)),t.stroke()},y=()=>{const A=t.createLinearGradient(P[0][0],P[0][1],P[E][0],P[E][1]);return A.addColorStop(0,n(.55,0)),A.addColorStop(.4,n(.56,0)),A.addColorStop(.68,n(.6,.09)),A.addColorStop(.88,n(.64,.24)),A.addColorStop(1,n(.68,.44)),A},B=1+.5*D;t.globalAlpha=x,t.strokeStyle=y(),t.lineWidth=(20+10*D)*c*C,t.shadowColor=p(.72),t.shadowBlur=v*2.2,S(),t.strokeStyle=y(),t.lineWidth=(10+5*D)*c*C,t.shadowBlur=v*1,S(),t.shadowBlur=0;for(let A=1;A<=E;A++){const _=A/E;t.globalAlpha=Math.pow(_,2.2)*.95*x,t.strokeStyle=p(.55+.38*_),t.lineWidth=(1.6+6.5*Math.pow(_,.7))*c*C*B,t.beginPath(),t.moveTo(P[A-1][0],P[A-1][1]),t.lineTo(P[A][0],P[A][1]),t.stroke()}const H=P[E][0],U=P[E][1];t.globalAlpha=.8*x,t.fillStyle=p(.6),t.shadowColor=p(.8),t.shadowBlur=v*1.6,t.beginPath(),t.arc(H,U,(9+5*D)*c*C,0,Math.PI*2),t.fill(),t.globalAlpha=x,t.fillStyle=p(.93),t.shadowBlur=v*.6,t.beginPath(),t.arc(H,U,(3.4+1.8*D)*c*C,0,Math.PI*2),t.fill(),t.globalAlpha=1,t.shadowBlur=0}function no(t,a,o,d,i,e,l){const r=o.r!=null?o.r:.3,p=o.dir!=null?o.dir:1,v=(o.sweep!=null?o.sweep:.66)*Math.PI*2,h=l??i*(o.tempo!=null?o.tempo:.5)%1,u=-Math.PI/2+p*h*Math.PI*2,s=16,c=[];for(let n=0;n<=s;n++){const R=u-p*(1-n/s)*v;c.push([.5+Math.cos(R)*r,.5+Math.sin(R)*r])}Bt(t,a,a,c,i,e,{prog:1,tail:o.tail!=null?o.tail:.68,scale:(o.scale!=null?o.scale:1)*(o.width!=null?o.width:1)})}function so(t,a,o,d,i,e,l={}){const r=Math.max(2,Math.round(l.count??7)),p=l.r??4.4,v=l.flow??.35,h=l.hair??.26,u=l.style||"chain",s=l.color||"255,246,234",c=l.inset??.12,n=l.alpha??1,R=d-a,G=i-o,F=Math.hypot(R,G);if(F<.001||n<=.004)return;const w=R/F,I=G/F,L=a+R*c,T=o+G*c,m=F*(1-c*2);if(m<=0)return;t.save(),u==="chain"&&h>0&&(t.strokeStyle=`rgba(${s},${(h*n).toFixed(3)})`,t.lineWidth=Math.max(1,p*.34),t.beginPath(),t.moveTo(L,T),t.lineTo(L+w*m,T+I*m),t.stroke());const x=e*v%1;for(let f=0;f<r;f++){let b=(f+.5)/r;if(u==="pulse"||u==="dots"||u==="chain"){const P=b-.5;if(b=.5+P+Math.sign(P)*(x/r%(1/r)),b<0||b>1)continue}const D=L+w*m*b,M=T+I*m*b,C=Math.abs(b-.5)*2;let E=p,k=n;if(u==="taper"&&(E=p*(.55+.75*C)),u==="pulse"){const P=((C-x)%1+1)%1;k=n*(.35+.65*Math.max(0,1-Math.abs(P-0)*3))}t.fillStyle=`rgba(${s},${(k*.96).toFixed(3)})`,t.beginPath(),t.arc(D,M,E,0,Math.PI*2),t.fill()}t.restore()}export{f0 as A,m0 as B,Qt as C,h0 as G,Pt as M,V as N,$ as P,Kt as Q,Yt as R,qt as S,$t as Z,t0 as a,_t as b,Xt as c,zt as d,eo as e,no as f,Nt as g,lo as h,jt as i,ao as j,so as k,oo as l,Wt as m,Ut as n,to as o,Jt as p,Vt as q,n0 as r,u0 as s,Et as t,Ht as u,Ot as v,Zt as w,Gt as x,Bt as y,gt as z};
