...
vec4 markState(vec2 uv, float state, float prog, float strong, float t){
  ...
  return A;
}`;function ut(t,o,a,i){t.lineWidth=4*o;const n=i.arrow;n.line==="dash"?t.setLineDash([12*o*n.gap,10*n.gap]):n.line==="dot"?(t.setLineDash([.5,12*n.gap]),t.lineCap="round",t.lineWidth=5*o):t.setLineDash([]),a!=null&&n.line!=="solid"&&n.line!=="taper"&&(t.lineDashOffset=-a*40*n.speed)}function At(t,o,a,i,n,e={}){const s=n.lut,r=n.arrow||{},f=r.w??1,p=r.speed??1,m=r.glow??1,u=e.pulse??1,l=a/256,h=o/2,d=i*.9*p%1,y=e.prog!=null?Math.max(0,Math.min(1,e.prog)):Math.min(1,d/.55),v=e.prog!=null?1:d>.88?(1-d)/.12:1;t.clearRect(0,0,o,a);const w=v*(.45+.55*u),c=a-24*l,M=58*l,S=c+(M-c)*y,x=(R,T)=>s(R).replace("rgb(","rgba(").replace(")",`,${T.toFixed(3)})`),C=1.1*l*f,A=13*l*f,k=t.createLinearGradient(0,c,0,S);if(k.addColorStop(0,x(.55,0)),k.addColorStop(.1,x(.64,.45*w)),k.addColorStop(.32,x(.76,.85*w)),k.addColorStop(.62,x(.88,.98*w)),k.addColorStop(1,x(.97,w)),t.globalAlpha=1,t.fillStyle=k,t.beginPath(),t.moveTo(h-C/2,c),t.lineTo(h+C/2,c),t.lineTo(h+A/2,S),t.lineTo(h-A/2,S),t.closePath(),t.fill(),t.globalAlpha=w,y>.28&&!e.noTip){const R=34*l*(.7+.3*f),T=Math.min(1,(y-.28)/.22)*w,I=S+R*.3;t.globalAlpha=T;const B={color:s(.95),glowColor:s(.85),glow:12*m};n.glyph&&(n.glyph(t,"LIFT_TIP",h,I,R,B)||n.glyph(t,"TIP_TRI",h,I,R*.93,B))||(t.strokeStyle=s(.95),t.lineWidth=13*l*f,t.lineCap="round",t.lineJoin="round",t.shadowColor=s(.9),t.shadowBlur=18*l*m,t.beginPath(),t.moveTo(h-26*l,I+14*l),t.lineTo(h,I-16*l),t.lineTo(h+26*l,I+14*l),t.stroke())}t.globalAlpha=1,t.shadowBlur=0}function yt(t,o,a,i,n,e,s={}){const r=e.lut,f=e.arrow||{},p=f.w??1,m=f.glow??1,u=a/256;t.clearRect(0,0,o,a);const l=i.map(([c,M])=>[c*o,M*a]);if(l.length<2)return;const h=48,d=[],y=c=>{if(l.length===2)return[l[0][0]+(l[1][0]-l[0][0])*c,l[0][1]+(l[1][1]-l[0][1])*c];const M=c*(l.length-1),S=Math.min(l.length-2,Math.floor(M)),x=M-S,C=l[Math.max(0,S-1)],A=l[S],k=l[S+1],R=l[Math.min(l.length-1,S+2)],T=(I,B,D,U)=>.5*(2*B+(-I+D)*x+(2*I-5*B+4*D-U)*x*x+(-I+3*B-3*D+U)*x*x*x);return[T(C[0],A[0],k[0],R[0]),T(C[1],A[1],k[1],R[1])]};for(let c=0;c<=h;c++)d.push(y(c/h));const v=Math.max(0,Math.min(1,s.prog!=null?s.prog:n*.55%1)),w=Math.max(1,Math.round(h*v));t.lineCap="round";for(let c=1;c<=w;c++){const M=c/w;t.globalAlpha=Math.pow(M,1.5),t.strokeStyle=r(.45+.5*M),t.lineWidth=(1.6+3.2*M)*u*p,t.beginPath(),t.moveTo(d[c-1][0],d[c-1][1]),t.lineTo(d[c][0],d[c][1]),t.stroke()}if(v>.25){const c=d[w][0],M=d[w][1],S=d[Math.max(0,w-2)][0],x=d[Math.max(0,w-2)][1],C=Math.atan2(M-x,c-S)+Math.PI/2,A=30*u*(.7+.3*p);t.save(),t.translate(c,M),t.rotate(C),t.globalAlpha=Math.min(1,(v-.25)/.2);const k={color:r(.95),glowColor:r(.85),glow:12*m};e.glyph&&(e.glyph(t,"LIFT_TIP",0,0,A,k)||e.glyph(t,"TIP_TRI",0,0,A*.93,k))||(t.strokeStyle=r(.95),t.lineWidth=9*u*p,t.lineJoin="round",t.lineCap="round",t.beginPath(),t.moveTo(-18*u,12*u),t.lineTo(0,-14*u),t.lineTo(18*u,12*u),t.stroke()),t.restore()}t.globalAlpha=1}function nt(t,o,a,i,n,e){n=n||{};const s=e.lut,r=n.style||e.arrow.line,f=!!n.closed,p=[0];for(let h=1;h<o.length;h++)p.push(p[h-1]+Math.hypot(o[h][0]-o[h-1][0],o[h][1]-o[h-1][1]));const m=p[p.length-1]||1,u=h=>{h=(h%m+m)%m;let d=1;for(;d<p.length-1&&p[d]<h;)d++;const y=(h-p[d-1])/Math.max(1e-4,p[d]-p[d-1]);return[o[d-1][0]+(o[d][0]-o[d-1][0])*y,o[d-1][1]+(o[d][1]-o[d-1][1])*y,Math.atan2(o[d][1]-o[d-1][1],o[d][0]-o[d-1][0])]},l=e.arrow;if(r==="chevron"){const h=(26*i+8)*l.gap,d=Math.max(2,Math.floor(m/h));t.shadowColor=s(Math.min(1,l.heat+.2)),t.shadowBlur=8*i*l.glow;for(let y=0;y<d;y++){const v=y*h+a*42*l.speed%h;if(!f&&v>m-4)continue;const[w,c,M]=u(v),S=7.5*i,x=8.5*i,C=.45+.4*Math.sin(v/m*6.283-a*2.2*l.speed);t.strokeStyle=s(l.heat-.05+C*.3),t.lineWidth=3.2*i,t.lineJoin="round",t.lineCap="round",t.save(),t.translate(w,c),t.rotate(M),t.beginPath(),t.moveTo(-x*.5,-S),t.lineTo(x*.5,0),t.lineTo(-x*.5,S),t.stroke(),t.restore()}return!0}if(r==="comet"){const h=a*.35*l.speed%1*m,d=m*l.tail,y=Math.max(24,o.length*2);t.lineCap="round";for(let c=0;c<y;c++){const M=h-c/y*d,S=h-(c+1)/y*d;if(!f&&S<0)break;const x=1-c/y,[C,A]=u(M),[k,R]=u(S);!f&&Math.hypot(k-C,R-A)>m*.4||(t.globalAlpha=Math.pow(x,1.6),t.strokeStyle=s(Math.max(.05,l.heat-.2)+x*.55),t.lineWidth=(1.5+x*4.5)*i,x>.72?(t.shadowColor=s(Math.min(1,l.heat+.3)),t.shadowBlur=x*12*i*l.glow):t.shadowBlur=0,t.beginPath(),t.moveTo(C,A),t.lineTo(k,R),t.stroke())}t.globalAlpha=1,t.lineCap="butt",t.shadowBlur=0;const[v,w]=u(h);return t.fillStyle=rgba($.ink,.95),t.shadowColor=s(.9),t.shadowBlur=16*i,t.beginPath(),t.arc(v,w,2.6*i,0,7),t.fill(),t.shadowBlur=0,!0}if(t.strokeStyle=n.color||s(l.heat),t.shadowColor=s(Math.min(1,l.heat+.15)),t.shadowBlur=(n.glow??8)*i*l.glow,r==="taper")for(let h=1;h<o.length;h++)t.lineWidth=(.5+h/o.length*4.5)*i,t.beginPath(),t.moveTo(o[h-1][0],o[h-1][1]),t.lineTo(o[h][0],o[h][1]),t.stroke();else ut(t,i,a,e),t.beginPath(),o.forEach(([h,d],y)=>y?t.lineTo(h,d):t.moveTo(h,d)),f&&t.closePath(),t.stroke();return t.setLineDash([]),t.lineCap="butt",t.lineDashOffset=0,t.shadowBlur=0,!0}function St(t,o,a,i,n,e){const s=13*i.halo,r=e.lut;t.clearRect(0,0,o,o),t.lineJoin="round";const f=o/220,p=o/2,m=18*a.round*f,u=40*f,l=48*f,h=o-80*f,d=o-96*f,y=[],v=(c,M,S,x)=>{for(let C=0;C<=1;C+=.12)y.push([c+(S-c)*C,M+(x-M)*C])};v(u+m,l,u+h-m,l),v(u+h,l+m,u+h,l+d-m),v(u+h-m,l+d,u+m,l+d),v(u,l+d-m,u,l+m),t.shadowColor=r(.6),t.shadowBlur=s*.8;const w=4*e.arrow.w*f;e.arrow.line==="solid"?(t.setLineDash([10*a.dash*f,8*f]),t.lineDashOffset=-n*22*f,t.strokeStyle=r(.45),t.lineWidth=w,t.beginPath(),t.roundRect(u,l,h,d,m),t.stroke(),t.setLineDash([]),t.lineDashOffset=0):nt(t,y,n,e.arrow.w*f,{color:r(.45),closed:!0},e),a.feet>.05&&e.foot&&(e.foot(t,!1,p-16*a.feet*f,p+6*f,26*a.feet*f),e.foot(t,!0,p+16*a.feet*f,p+6*f,26*a.feet*f)),t.shadowBlur=0}function vt(t,o,a,i,n,e,s,r){const f=13*i.halo,p=4*e.arrow.w*(o/220),m=e.lut;t.clearRect(0,0,o,o),t.lineJoin="round";const u=o/220,l=s||[[45*u,130*u],[110*u,60*u],[175*u,110*u]],h=r??n*.5%1,d=Math.min(1,h*1.25)*(l.length-1),y=Math.min(l.length-1,Math.floor(d+.35));t.shadowColor=m(.7),t.shadowBlur=f;const v=[[l[0][0],l[0][1]]];for(let w=1;w<=l.length-1;w++){const c=Math.max(0,Math.min(1,d-(w-1)));if(c<=0)break;v.push([l[w-1][0]+(l[w][0]-l[w-1][0])*c,l[w-1][1]+(l[w][1]-l[w-1][1])*c])}v.length>1&&nt(t,v,n,e.arrow.w*u,{color:m(.62)},e),t.setLineDash([4*u,7*u]),t.lineDashOffset=0,t.globalAlpha=.3,t.strokeStyle=m(.45),t.lineWidth=p,t.beginPath(),l.forEach(([w,c],M)=>M?t.lineTo(w,c):t.moveTo(w,c)),t.stroke(),t.globalAlpha=1,t.setLineDash([]),t.lineCap="butt",t.lineDashOffset=0,l.forEach(([w,c],M)=>{const S=M===y,x=S?1+Math.sin(n*6)*.14:1;t.strokeStyle=m(S?.8:.45),t.lineWidth=p*(S?1.3:.9),t.shadowBlur=S?f*1.6:f*.6,t.beginPath(),t.arc(w,c,12*a.node*x*u,0,Math.PI*2),t.stroke(),e.num&&(t.globalAlpha=M<=y?1:.45,e.num(t,String(M+1),w,c,16*a.numS*x*u,Math.round(14*a.numS*u)),t.globalAlpha=1)}),t.shadowBlur=0}function kt(t,o,a,i,n,e,s){const r=e.lut,f=13*i.halo,p=o/220,m=o/2,u=e.arrow&&e.arrow.w||1,l=(C,A)=>r(C).replace("rgb(","rgba(").replace(")",`,${A})`);t.clearRect(0,0,o,o),t.lineJoin="round",t.lineCap="round";const h=(a.r!=null?a.r:.42)*o,d=h*(a.rt!=null?a.rt:.36),y=3.4*u*p,v=s!=null?Math.max(0,Math.min(1,s)):n*(a.tempo||.6)%1,w=Math.pow(v,1.6),c=Math.max(0,(v-.9)/.1);t.save(),t.translate(m,m);const M=(C,A,k,R=1)=>{if(C<=.6)return;const T=y*2.6*R,I=Math.max(.1,C-T),B=C+T,D=t.createRadialGradient(0,0,I,0,0,B);D.addColorStop(0,l(A-.05,0)),D.addColorStop(.5,l(A,k*.85)),D.addColorStop(1,l(A-.05,0)),t.globalAlpha=1,t.fillStyle=D,t.shadowBlur=0,t.beginPath(),t.arc(0,0,B,0,Math.PI*2),t.fill(),t.globalAlpha=Math.min(1,k*1.1),t.lineWidth=y*.85,t.strokeStyle=r(Math.min(.98,A+.12)),t.shadowColor=r(.88),t.shadowBlur=f*.6,t.beginPath(),t.arc(0,0,C,0,Math.PI*2),t.stroke(),t.shadowBlur=0},S=t.createRadialGradient(0,0,0,0,0,d*1.08);S.addColorStop(0,l(.6,.1+.18*c)),S.addColorStop(.65,l(.5,.05+.08*c)),S.addColorStop(1,l(.5,0)),t.globalAlpha=1,t.fillStyle=S,t.beginPath(),t.arc(0,0,d*1.08,0,Math.PI*2),t.fill();const x=1+.02*Math.sin(n*2.6);M(d*x,.55+.4*c,.5+.45*c,.9);for(let C=2;C>=0;C--){const A=Math.pow(Math.max(0,v-C*.05),1.6),k=h-(h-d)*A,R=C===0?.6+.4*w:.18/C*(1-c);M(k,.55+.4*w,R*(1-c*.45),1.15-.35*w)}c>.01&&M(d*(1+1.4*c),.9,(1-c)*.8,1.1),t.globalAlpha=.6+.3*c,t.shadowColor=r(.85),t.shadowBlur=f*(.9+c),t.fillStyle=r(.62+.3*c),t.beginPath(),t.arc(0,0,y*.85+3*p*c,0,Math.PI*2),t.fill(),t.restore(),t.globalAlpha=1,t.shadowBlur=0}function _t(t,o,a,i,n,e,s,r){const f=e.lut,p=13*i.halo,m=o/220,u=o/2,l=e.arrow&&e.arrow.w||1,h=l*m,d=(b,_)=>f(b).replace("rgb(","rgba(").replace(")",","+_+")");t.clearRect(0,0,o,o),t.lineJoin="round",t.lineCap="round";const y=o*.42*(a.spread!=null?a.spread:1),w=(r||[[-.95,.5],[-.48,-.42],[0,-.8],[.48,-.42],[.95,.5]]).map(([b,_])=>[u+b*y,u+_*y]),c=80,M=[];for(let b=0;b<=c;b++){const _=b/c*(w.length-1),L=Math.min(w.length-2,Math.floor(_)),P=_-L,G=w[Math.max(0,L-1)],E=w[L],z=w[L+1],V=w[Math.min(w.length-1,L+2)],N=(K,W,j,g)=>.5*(2*W+(-K+j)*P+(2*K-5*W+4*j-g)*P*P+(-K+3*W-3*j+g)*P*P*P);M.push([N(G[0],E[0],z[0],V[0]),N(G[1],E[1],z[1],V[1])])}const S=b=>{const _=Math.max(0,Math.min(c,b*c)),L=Math.floor(_),P=_-L,G=M[L],E=M[Math.min(c,L+1)];return[G[0]+(E[0]-G[0])*P,G[1]+(E[1]-G[1])*P]},x=.68;let C,A,k;if(s!=null)C=Math.max(0,Math.min(1,s)),A=1,k=0;else{const b=n*(a.tempo||.42)%1;if(b<x)C=b/x,A=1,k=0;else{const _=(b-x)/(1-x);C=1,A=1-_*_,k=_}}if(A<=.012)return;const R=C*C*C*(C*(6*C-15)+10),T=Math.min(1,16*C*C*(1-C)*(1-C));a.taper!=null&&a.taper;const I=.36*(a.tail!=null?a.tail:1),B=a.width!=null?a.width:1;{const b=t.createLinearGradient(M[0][0],M[0][1],M[c][0],M[c][1]);b.addColorStop(0,d(.46,0)),b.addColorStop(.3,d(.46,.03*A)),b.addColorStop(.8,d(.46,.045*A)),b.addColorStop(1,d(.46,0)),t.globalAlpha=1,t.strokeStyle=b,t.lineWidth=9*h,t.shadowColor=f(.6),t.shadowBlur=p*2,t.beginPath(),M.forEach(([_,L],P)=>P?t.lineTo(_,L):t.moveTo(_,L)),t.stroke(),t.shadowBlur=0}const D=40,U=Math.max(0,R-I*(1-k)),F=[];for(let b=0;b<=D;b++)F.push(S(U+(R-U)*(b/D)));const H=()=>{t.beginPath(),F.forEach(([b,_],L)=>L?t.lineTo(b,_):t.moveTo(b,_)),t.stroke()},X=()=>{const b=t.createLinearGradient(F[0][0],F[0][1],F[D][0],F[D][1]);return b.addColorStop(0,d(.55,0)),b.addColorStop(.4,d(.56,0)),b.addColorStop(.68,d(.6,.09)),b.addColorStop(.88,d(.64,.24)),b.addColorStop(1,d(.68,.44)),b},rt=1+.5*T;t.globalAlpha=A,t.strokeStyle=X(),t.lineWidth=(20+10*T)*h*B,t.shadowColor=f(.72),t.shadowBlur=p*2.2,H(),t.strokeStyle=X(),t.lineWidth=(10+5*T)*h*B,t.shadowBlur=p*1,H(),t.shadowBlur=0;for(let b=1;b<=D;b++){const _=b/D;t.globalAlpha=Math.pow(_,2.2)*.95*A,t.strokeStyle=f(.55+.38*_),t.lineWidth=(1.6+6.5*Math.pow(_,.7))*h*B*rt,t.beginPath(),t.moveTo(F[b-1][0],F[b-1][1]),t.lineTo(F[b][0],F[b][1]),t.stroke()}const Q=F[D][0],Z=F[D][1];t.globalAlpha=.8*A,t.fillStyle=f(.6),t.shadowColor=f(.8),t.shadowBlur=p*1.6,t.beginPath(),t.arc(Q,Z,(9+5*T)*h*B,0,Math.PI*2),t.fill(),t.globalAlpha=A,t.fillStyle=f(.93),t.shadowBlur=p*.6,t.beginPath(),t.arc(Q,Z,(3.4+1.8*T)*h*B,0,Math.PI*2),t.fill(),t.globalAlpha=1,t.shadowBlur=0}function Rt(t,o,a,i,n,e,s){const r=e.lut,f=13*i.halo,p=o/220,m=o/2,u=e.arrow&&e.arrow.w||1;t.clearRect(0,0,o,o),t.lineJoin="round",t.lineCap="round";const l=(a.r!=null?a.r:.3)*o,h=a.width!=null?a.width:1,d=4.2*u*p*h,y=a.dir!=null?a.dir:1,v=(a.sweep!=null?a.sweep:.66)*Math.PI*2,w=s!=null?Math.max(0,Math.min(1,s)):n*(a.tempo||.5)%1,c=-Math.PI/2+y*w*Math.PI*2;t.save(),t.translate(m,m),t.globalAlpha=.16,t.lineWidth=d*.7,t.strokeStyle=r(.44),t.shadowColor=r(.6),t.shadowBlur=f*.4,t.beginPath(),t.arc(0,0,l,0,Math.PI*2),t.stroke(),t.shadowBlur=0;const M=16;for(let T=0;T<M;T++){const I=T/(M-1),B=c-y*I*v,D=c-y*(I+1.2/M)*v;t.globalAlpha=(1-I)*.9,t.strokeStyle=r(.55+.35*(1-I)),t.lineWidth=d*(.55+.55*(1-I)),t.shadowColor=r(.8),t.shadowBlur=f*(.4+.5*(1-I)),t.beginPath(),t.arc(0,0,l,Math.min(B,D),Math.max(B,D),!1),t.stroke()}t.shadowBlur=0;const S=Math.cos(c)*l,x=Math.sin(c)*l,C=c+y*Math.PI/2,A=8*p*h;t.save(),t.translate(S,x),t.rotate(C+Math.PI/2),t.globalAlpha=1;const k=3.4*A*(.7+.3*u),R={color:r(.96),glowColor:r(.9),glow:f*1.2};e.glyph&&(e.glyph(t,"LIFT_TIP",0,0,k,R)||e.glyph(t,"TIP_TRI",0,0,k*.93,R))||(t.rotate(-Math.PI/2),t.strokeStyle=r(.96),t.lineWidth=d*.9,t.shadowColor=r(.9),t.shadowBlur=f*1.2,t.beginPath(),t.moveTo(-A,-A*.9),t.lineTo(A*.5,0),t.lineTo(-A,A*.9),t.stroke()),t.restore(),t.globalAlpha=.62,t.shadowColor=r(.75),t.shadowBlur=f*.6,t.fillStyle=r(.6),t.beginPath(),t.arc(0,0,d*.6,0,Math.PI*2),t.fill(),t.restore(),t.globalAlpha=1,t.shadowBlur=0}export{xt as M,pt as N,q as P,st as S,vt as a,Ct as b,kt as c,St as d,_t as e,Rt as f,bt as g,At as h,dt as i,nt as j,$ as k,Mt as l,yt as m,wt as n,it as o,mt as r,ht as s};
