...
//     node scripts/check_beam_fit.mjs            # 기준 잉크 반경 0.16m
//     INK=0.60 node scripts/check_beam_fit.mjs   # 민감도 확인용 — 전건 실패해야 정상이다
//
...
const INK = +(process.env.INK || 0.16);   // 발 잉크 반길이(m) — FOOT_LEN_M*0.5 + 약간의 여유
...
console.log(`투사영역 이탈 검사 — 잉크 반경 ${INK}m\n`);
...
for (const [id, vts] of Object.entries(VTS)) {
  for (const vt of vts) {
    ...
    const r = await p.evaluate(({ id, vt, INK }) => {
      ...
      for (const [k, fm] of [['L', H.fRl || H.sL2], ['R', H.fRr || H.sR2]]) {
        ...
        let worst = null;
        for (const dx of [-INK, INK]) for (const dz of [-INK, INK]) {
...
if (bad) {
  ...
  console.log('처방: session.js _beamFit 의 반경(SB_FIT_R)을 키우거나, 무대(SB_BOX)를 창 안쪽으로 좁힌다.');
  console.log('      원점만 클램프하면 안 된다 — 몸의 반경까지 넣어야 한다.');
  process.exitCode = 1;
} else {
  console.log(`\n통과 — ${n}건 전부 창 안. 최소 여유 ${minMargin.toFixed(3)}m`);
  console.log('민감도 확인:  INK=0.60 node scripts/check_beam_fit.mjs  → 전건 실패해야 검사가 살아 있는 것이다.');
}
await b.close();
