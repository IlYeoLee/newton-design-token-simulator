...
await p.evaluate(() => {
  ...
  window.__vtPin = setInterval(() => { s.stageIdx = i; s._followLatch = true; s._aWatchEnd = 0; if (s.t < 8) s.t = 8.5; if (s.bkB1) s.bkB1.count = 9; }, 8);
});
...
const info = await p.evaluate(() => {
  ...
  const e = new window.__dbg.THREE.Euler().setFromQuaternion(cam.quaternion, "YXZ");
  const H = window.__dbg.session.bkB1;
  return { pitchDeg: +(e.x * 180 / Math.PI).toFixed(1), camY: +cam.position.y.toFixed(2), op: H.mat.material.opacity, prog: H.mat._prim.P.prog, vis: H.mat.visible,
