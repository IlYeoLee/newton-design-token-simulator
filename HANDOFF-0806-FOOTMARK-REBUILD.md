...
## 정본 변경 ② — 압력 투명도 `uPressA` (`src/fx-core.js`)

압력을 **색에만** 태우면 판때기로 읽힌다. 알파에도 태우면 눌린 자리는 차 있고 덜 눌린 자리가
은은하게 비쳐서 입체가 된다.

```glsl
// markState 안, inFill 정의 바로 뒤 — 7상태 전부가 이 한 줄을 지난다
if (uPressA > 0.001) {
...
- **여기 한 곳이 7상태 전부에 걸린다** — 아래 `lay(...)` 가 모두 `inFill` 을 곱하기 때문이다.
  상태마다 따로 넣으면 반드시 어긋난다.
- 바닥 0.30 은 남긴다. 0 까지 뚫으면 아치가 끊겨 형태가 깨진다 — 요점은 *은은함*이다.
- 기본 0 = 기존 화면 무변화. 룩 키 이름은 `pressA`(`applyMarkLookTo` 맵에 등록).
- `heatmap7.html` 은 `pressA: 0.6`.
