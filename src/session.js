...
export const STAGES = {
  ...
  boxing: [
    ...
    { id:'BX_C3', wall:true, dur:10.875, live:true, boost:true, label:'C3 · 실전 3/4 — 콤비네이션', voice:['고수','잽, 잽 — 마지막은 몸을 실어서 훅! 리듬 놓치지 말고.'], wear:'BOOST 스텝 추진', cue:'구간 종료 Match Rate' },   // dur = 클립 6.04 × 2바퀴(08-05). 사이클이 클립 길이와 같아야 마커와 실루엣이 매 바퀴 같은 위상에서 만난다
  ...
  _updateBoxing(id, st, beat, FMU) {
    ...
    if (id === 'BX_READY' || id === 'BX_T1') {
      ...
      if (id === 'BX_C3' && this.bxCombo) {
        ...
        const CY = 3.625, tc = this.t % CY;
        const AT = [1.17, 1.80, 2.33];
