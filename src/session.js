...
function livePrimEnv() {
  ...
  return {
    ...
    // ★ markBlend 를 주간 신호로 쓰면 안 된다 — 기본값이 'ink' 라 시뮬이 **항상** 주간
    //   팔레트로 돌았고, 어두운 바닥에 어두운 적(lut 0.26)을 칠해 매트가 통째로 사라졌다
    //   (유저 신고). 잉크 합성 여부와 '바닥이 밝은가'는 다른 질문이다.
    //   주간은 **명시 플래그**로만 켠다(FXP.day). 기본 = 꺼짐.
    day: FXP.day ? 1 : 0,
...
// ★★ 좌표 재저작(08-11 유저): **봇이 실제로 추는 클립(cmu124_06)의 발 월드좌표 실측**으로 갈았다.
//   구표는 다른 테이크(시범영상 stepback_fwd)에서 손으로 적은 안무라 '왼발 옆·뒤 슬라이드' 이야기
//   였는데, 정본 클립은 **오른발 크로스 → 몸+오른발이 뒤로 1.3m 싹 빠짐 → 모으기**다(전진 페이크
//   →후방 이탈). 다리는 발자국 IK 를 따르므로(d621275) 표가 틀리면 봇이 실제 동작을 안 춘다.
//   실측: scripts/_probe_cmufeet.mjs (클립을 봇에 직접 재생, getProbes 발 월드좌표).
//   가이드 시각→클립 시각 = d621275 사상표(2.10·2.20 은 역재생 구간 = 모으기).
//     clip 1.35  L(-0.096,-0.073) R( 0.019,-0.214)   두 발 좁은 스탠스(폭 0.17m)
//     clip 1.75  L( 0.019, 0.014) R(-0.218, 0.311)   ① R 크로스 착지 — L 왼쪽을 지나 반대편
//     clip 2.25  L( 0.050, 0.106) R(-0.201, 1.256)   ② R+몸 후방 1.3m — 깊게 갈라짐
//     clip 1.50  L(-0.051,-0.012) R(-0.104,-0.206)   ③ 모으기(역재생) — 시작 근방 복귀
//   정규화: v = 깊이 등비 맞춤(반스팬 0.7365m → ±1, 실비율 보존 — '뒤로 싹'이 세로축 전체를 쓴다).
//   u = 가로 ×2.5 가독 게인(클립 스탠스 0.17m 를 그대로 등비로 넣으면 마크 두 장이 4cm 로 겹친다.
//   main.js 커서 증폭 ×2.2 와 같은 계열의 문서화된 연출값). L 은 실이동 0.15m(마크 반지름 이하)라
//   규약대로 자리를 지킨다 — 대사('왼발로 바닥을 밀면서')와도 맞는다: 미는 발은 안 가는 발이다.
// ★★★ 재재저작(08-11 유저: "발자국이 또 꼬였다 — CMU 실측을 100% 따라라"). 두 가지를 고친다.
//   ① 523925a 는 '다리가 발자국 IK 를 따른다'를 전제로 갈았는데, 그 전제는 14시간 전
//      5f7ab02(setFootIK(null))에서 이미 죽어 있었다 — 봇은 클립만 춘다. 그러니 마크의 정본은
//      **화면에 실제로 재생되는 봇의 발**이다(빔이 무릎에 달려 몸과 함께 움직이므로,
//      발자국 안무는 월드가 아니라 몸 기준 좌표가 물리적으로도 맞다 = demoInPlace 렌더 그대로).
//   ② 축별 게인 금지. 구표는 가로 ×2.5 · 세로 등비로 종횡비(실측 0.33)를 뭉갰다.
//      등비 하나(×2.03)만 쓴다 — 좁은 스탠스는 왜곡이 아니라 스텝백의 실제 모양이다.
//   실측: scripts/_probe_sbfeet.mjs (BK_T1 라이브, 힙 기준 발 월드좌표, 0.05s 격자 121샘플).
//     vt 1.05  L( 0.010, 0.015) R( 0.223,-0.093)   좁은 스탠스(전진 자세)
//     vt 1.47  L( 0.034, 0.101) R( 0.185,-0.215)   ① R 이 몸 쪽으로·L 은 앞볼로 민다
//     vt 1.70  L( 0.012, 0.397) R( 0.104,-0.590)   ② 몸이 빠진다 — L 마크는 멀어지고 R 은 몸 밑(스팬 1.14m)
//     vt 2.10  L( 0.030, 0.065) R( 0.216,-0.124)   ③ 복귀(역재생)
//     vt 2.20  L( 0.007, 0.018) R( 0.225,-0.069)       유지
//   정규화: 중심(x 0.116 · z −0.0965) 빼고 S=2.03 (깊이 반스팬 0.4935m → ±1). u·v 동일 게인.
export const SB_POSE = [
  { t: 1.05, L: [-0.22, 0.23], R: [0.22, 0.01] },                        // 시작 = 좁은 스탠스(실측 0.21m)
  { t: 1.47, L: [-0.17, 0.40, 1], R: [0.14, -0.24] },                    // ① L 앞볼로 밀기 시작 · R 몸 쪽으로
  { t: 1.70, L: [-0.21, 1.00], R: [-0.02, -1.00, 0, 'slide'] },          // ② 몸 이탈 — L 마크 멀어지고 R 은 몸 밑으로 '쓰윽'
  { t: 2.10, L: [-0.17, 0.33], R: [0.20, -0.06] },                       // ③ 모으기(역재생 구간)
  { t: 2.20, L: [-0.22, 0.20], R: [0.22, 0.06] },                        //     그 자세로 유지(구간 끝)
];
...
// ★ 08-07 재조정: v0 0.24 → 0.36 · v1 0.72 → 0.84 (**구간 폭 0.48 유지**, 통째로 0.12 앞으로).
//   근거는 1인칭 화면 좌표 실측이다(scripts/audit_fp_framing.mjs, 농구 BK_B2, 눈높이 1.68m · fov 60°):
//     빔 근단 0.30m → 화면y −1.564   ← **빔 근단은 1인칭에서 영영 안 보인다**(구조적)
//     빔 1.15m      → −0.533
//     빔 원단 1.90m → −0.072
//     화면 하단(−1.0)에 해당하는 전방거리 = **0.77m**
//   beamUV 의 d(v) = fpNear + M + (fpFar − fpNear − 2M)·v = 0.48 + 1.24·v 이므로
//     구값 v0 0.24 → d 0.778m = 화면 경계선 **바로 위**. 그래서 발마크가 물려 잘렸다
//     (실측 하단 y −1.087 · −1.081, 유저 스샷의 L 발자국).
//     신값 v0 0.30 → d 0.852m.
//   ★★ 0.42 까지 밀었다가 되돌렸다 — 그러면 이번엔 **타이틀 알약을 침범한다**(유저 스샷).
//     화면에서 앞 = 위라, 마크를 앞으로 밀면 타이틀 자리로 올라간다. 배치를 옮겨서 풀 문제가
//     아니었다: 1인칭에 보이는 바닥이 1.13m 뿐인데 코치 판·타이틀·마크 세 층이 다 들어가야 한다.
//     **자리는 시선 각도로 만든다**(main.js GAZE MAT −52°). 박스는 겹치지 않을 만큼만 민다.
//     (0.36 도 잘림은 없앴지만 하단 −0.902 로 빠듯했다 — 두 단계로 재서 0.42 를 골랐다.)
//   ⚠ **v0 만 올리면 안 된다.** 그러면 v0~v1 이 좁아져 스텝백 세로 이동이 더 압축된다
//     (아래 ⚠ 주석이 이미 지적한 0.13m 압축이 더 나빠진다). 둘을 같은 양만큼 민다.
...
const SB_FIT_U = FOOT_LEN_M * 0.22;   // 좌우 = 발 **폭** 절반(0.066m). 스탠스 라인이 쓰는 그 여백과 같은 값
const SB_FIT_V = FOOT_LEN_M * 0.55;   // 앞뒤 = 발 **길이** 절반 + 숨(0.165m)
...
//   MAX 가 실제 거리보다 작으면 **1 에서 포화해 선이 실제보다 짧게 그려진다** — 라이브 실측
//   prog [1, 0.14, 1] 의 두 1 이 그 포화였다(경로선 하나가 빠진 채 녹화될 상태였다).
//   1.60 = 대지 폭(1.6m) — 지면 위 두 점 사이 거리가 이보다 클 수 없으므로 **구조적으로 포화가 없다.**
//   0.14 쪽은 발 여백(IN)이 구간을 먹은 것 — 아래 IN 을 발 **폭** 기준으로 내린다(스탠스선과 같은 이유).
const SB_TRAIL_MAX = 1.60;
...
export const STAGES = {
  running: [
    ...
    { id:'A3', label:'A3 · 준비운동 3/3 — 하이니', voice:['션','마지막! 하이니예요. 무릎을 배 높이까지, 좌우 번갈아 올려요.'], foot:'완료 후 두 번 구르기 → 다음' },
    { id:'T1', label:'T1 · 전환 — 페이스 훈련 시작', voice:['션','몸이 다 풀렸네요, 최고예요! 이제 페이스를 끌어올리는 훈련 세 가지를 순서대로 해볼게요. 발 두 번 구르면 시작!'], foot:'발 두 번 구르기 → 훈련 시작' },
    ...
    { id:'C4', dur:7, live:true, boost:true, label:'C4 · 실전 4/5 — 마지막 스퍼트', voice:['션','여기서부터 마지막 구간이에요! 힘내요, 저만 보고 따라와요!'], wear:'BOOST 추진 보조 · 리듬 저하 시 강도↑', cue:'구간 종료 일치율 표시' },
  ...
  basketball: [
    ...
    //   긴 문장을 깔면 첫 비트에서 잘리고, speak() 가 앞 음성을 pause 해 버려 둘 다 안 들린다.
    //   설명은 비트가 한다: '오른발' · '왼발' · '모아'.
    ...
    { id:'BK_B3', label:'B2 · 스텝백 2/3 — 왼발로 밀어 뒤로 빠지기', voice:['커리','둘째. 왼발로 바닥을 밀면서 몸을 뒤로 쓱 빼요 — 그 순간 두 손으로 공을 잡아요.'], cue:'L 로 밀고 몸은 뒤로 · 두 손 개더' },   // 클립 실측: 뒤로 가는 건 오른발+몸, 왼발은 밀며 자리 지킴(SB_POSE 재저작과 함께 label 만 정정 — 대사는 원래 맞았다)
...
export class Session {
  ...
  _sbPlace(H, id, fmL, fmR, arrows) {
    ...
    const one = (side, fm, ar) => {
      ...
      const p = this._beamFit(q.u, q.v, SB_FIT_U, SB_FIT_V, H.mL);   // 발 잉크 반경까지 창 안에
      ...
      if (q.moving && q.slide) {
        ...
        //   ★ 3.5% → **7%**(유저: 쫀득한 키감). 얕으면 '눌렸나?' 싶고 안 눌린 것과 구분이 안 된다.
        //     마지막 비트(④ 슛)는 **9%** — 끝맺음이라 한 번 더 눌러 준다. 파형과 같은 박자다.
        //     10%를 넘기면 각인이 아니라 '떠 있는 판때기'가 튀는 걸로 읽힌다 — 거기가 상한.
        fm.op(1); fm.at(p.x, p.z, FOLLOW_S * (AD ? 1 + ((H?.beat ?? 0) >= 3 ? 0.09 : 0.07) * pop : 1));
      } else if (landed) {
      ...
      if (AD) {
        //   ★★ 접지 창(CoP) — 유저 08-07: "운동하는 발 같지 않아 ... 그건 진짜 촥촥촥 움직인다".
        //     bac55c6 이 만들어 두고 `uCopA 0 = 끔(기본)`으로 잠가 둔 시스템이다. 그 커밋의 말:
        //     "하중 세기만 애니메이션하면 같은 그림이 밝아졌다 어두워질 뿐이다" — 지금이 딱 그 상태였다.
        //     실제 걸음은 **닿는 자리가 옮겨 간다**: 뒤꿈치 바깥 → 바깥날 → 발바닥 전체 → 앞볼 → 엄지.
        //     표는 gait.html 정본을 그대로 옮겼다(보행 문헌의 gait line). 새 값 0개.
        //     구동은 **착지 후 경과(age)** 로 한다 — 딛는 순간 창이 열리고 입각기를 훑는다.
        //     STANCE 0.62s: 스텝백 한 비트(CFG.per 0.9)의 입각 구간 어림. 짧으면 촥, 길면 흐물.
        if (fm._U?.uCopA) {
          const GAIT = [[0.00, -0.10, -0.46, 0.17, 0.14], [0.12, -0.13, -0.12, 0.26, 0.40],
                        [0.45, -0.02,  0.08, 0.40, 0.56], [0.62,  0.06,  0.34, 0.36, 0.26],
                        [0.85,  0.16,  0.52, 0.22, 0.17], [1.00,  0.20,  0.60, 0.14, 0.11]];
          //   ★ 이징(유저 08-07: 더 찰지게) — 선형으로 훑으면 '미끄러진다'. 실제 입각기는
          //     앞이 빠르다: 뒤꿈치가 닿는 순간 하중이 왈칵 실리고, 앞볼로 갈수록 버티며 느려진다.
          //     ease-out(1-(1-u)^2.4)이 그 비대칭을 만든다 — 초반 40%를 3분의 1 시간에 지난다.
          //     차수는 키감 손잡이다: 낮추면 미끄럽고, 3 이상이면 첫 프레임에 튄다.
          const uL = Math.max(0, Math.min(1, age / 0.62));
          const u = 1 - Math.pow(1 - uL, 2.4);
          let i = 0; while (i < GAIT.length - 2 && u > GAIT[i + 1][0]) i++;
          const a = GAIT[i], b2 = GAIT[i + 1];
          const w = Math.max(0, Math.min(1, (u - a[0]) / Math.max(1e-4, b2[0] - a[0])));
          const s = w * w * (3 - 2 * w);   // smoothstep — gait.html 과 같은 보간
          const L = (j) => a[j] + (b2[j] - a[j]) * s;
          fm._U.uCop.value.set(L(1), L(2));
          fm._U.uCopR.value.set(L(3), L(4));
          //   창은 딛는 동안만 연다. 체공(moving)엔 0 — 닿지 않은 발에 접지면이 있으면 안 된다.
          fm._U.uCopA.value = q.moving ? 0 : Math.max(0, 1 - Math.max(0, age - 0.62) / 0.25);
        }
        //   파동 — 착지 직후에 몰아준다(age 는 위에서 잰 그 값). 상시로 깔면 '지저분'해지고,
        //   순간에 몰면 타격으로 읽힌다. 0.45s 안에서 스러진다.
        const rk = Math.max(0, 1 - age / 0.45);
        //   ★ 마지막 비트에는 **파형을 크게 남긴다**(유저 08-07: "마지막엔 파형").
        //     스텝백은 ④ 슛에서 끝난다 — 거기서 여운이 없으면 동작이 '끊긴다'.
        //     끝 비트만 파동을 배로 주고 감쇠도 길게(0.45 → 0.9s) 끌어 잔향처럼 남긴다.
        //     H.beat 는 이 프레이즈의 현재 비트(0~3). 배수라 정본이 0 인 상태는 0 그대로다.
        const LAST = (H?.beat ?? 0) >= 3;
        const rkL = LAST ? Math.max(0, 1 - age / 0.9) : rk;
        if (fm._U?.uRip) fm._U.uRip.value = MARK_LOOK.rip * (1 + (LAST ? 3.2 : 1.8) * rkL);
        //   도트 흐름 ×1.4 — 발 안 각인이 흐르는 속도. 커밋이 적어 둔 그 값 그대로.
        //   마지막 비트엔 흐름도 같이 올린다 — 파형과 같은 방향의 신호라 따로 놀면 안 된다.
        if (fm._U?.uFlow) fm._U.uFlow.value = (MARK_LOOK.flow ?? 0) * (LAST ? 1.9 : 1.4);
      }
      ...
      const sp = this._beamFit(q.su, q.sv, SB_FIT_U, SB_FIT_V, H.mL);
      const tp = this._beamFit(q.tu, q.tv, SB_FIT_U, SB_FIT_V, H.mL);
      ...
      // ★ 배율 원복(유저 08-10c: 종아리 화살표와 100% 같은 토큰이어야).
      //   가시성 부족은 순백 코어(drawStemArrow)가 토큰 차원에서 해결했다 — 인스턴스
      //   배율로 또 올리면 같은 토큰이 화면마다 다른 물건이 된다. 발표 프리셋만 예외.
      ...
      if (tok) ar._scale = tok.scale * (AD ? 1.6 : 1);   // 토큰 원값 — 두께 배율은 발표 프리셋만(유저: 100% 같은 토큰으로)
  ...
  /** ★ 반경은 **좌우와 앞뒤가 다르다**(rU · rV). 발을 원으로 보고 좌우도 발 길이만큼 밀어넣으면
  ...
  _updateRunning(id, st, beat, FMU) {
    ...
    if (id === 'READY' || id === 'T1') {
      ...
      // ★ 홀드 **밖에서는 Hold 로 두지 않는다**(유저 08-06: 3초 내내 저 상태다).
      //   P.fill = inHold ? cyc.prog : 0 이라, 봇이 홀드 구간이 아닐 때 fill 이 0 이 되고
      //   구식(`setHold(max(0.02, fill))`)은 그걸 **0.02 로 바닥 처리**했다 —
      //   진행이 2% 에 못 박힌 진행 링, 즉 영원히 안 차는 타이머가 사이클의 나머지 내내
      //   화면에 떠 있었다. 한 사이클 5.8초 중 홀드가 3초니까 **나머지 2.8초가 통짜 정지**다.
      //   (앞서 고친 받치는 발과 같은 병인데, 이쪽은 늘어나는 발에도 걸려 있었다.)
      //   실제로 차오르는 동안만 Hold, 그 밖은 Preview(셰이더 자체 시계로 도는 유일한 상태).
      if (P.fill > 0.02) work.setHold(P.fill); else work.countdown(-1);
      ...
      //   ★ 최종(유저 08-06, 4시간 반복 끝): 홀드 중에는 **두 발이 같이 찬다**.
      //     은은한 숨쉬기(밝기 ±6%)도, Preview 자체 차오름도 화면에선 '안 움직인다'로 읽혔다.
      ...
      // ★ 받치는 발은 **숨 쉰다**(유저 08-06: 여기 애니메이팅이 들어가야 하는데 고정으로 남는다).
      //   이 발은 setHold(0.02) 로 파킹돼 있다 — 진행이 없는 진행 링이라 화면에서 완전히 굳는다.
      //   Hold 셰이더에 숨쉬기가 있지만 그건 **스트로크** 밝기라, prog 0.02 면 숨 쉴 획 자체가 없다.
  ...
  _updateBasketball(id, st, beat, FMU) {
    ...
    if (id === 'BK_READY') {
      ...
      // ★ 실전은 화살표를 0 으로 죽인다 — **광고 프리셋은 예외**. 이 게이트를 안 열면
      //   아래 MUTE 를 아무리 열어도 `!arrK` 가 다시 닫는다(08-07: ?ad=1 인데 화살표가 안 떴다).
      const arrK = (LIVE && !AD) ? 0 : 1;
      ...
      if (POSE) {
        ...
        for (const k of ['fC', 'fLl', 'fLr']) {
          ...
          // ★★★ 광고 프리셋에서도 **끈다**(유저 08-07 재신고: "고스트로 이미 밟은 곳을 한번 더
          //   표시해야 할 이유를 모르겠다"). 같은 지적이 두 번 나온 물건이다 — 위 주석의
          //   '하나도 이해 안 가고 복잡하다'와 같은 말이고, 프리셋이 그걸 도로 켠 게 재발 원인이다.
          //   이유를 적어 둔다: 경로 마크는 **앞으로 밟을 자리**를 미리 알려줄 때만 정보다.
          //   이미 밟은 자리에 남기면 지시가 아니라 이력이고, 지금 밟아야 할 마크와 같은 어휘로
          //   그려지니 '어느 게 지금 것인지'를 매번 다시 골라내야 한다. 지우는 게 정보량을 늘린다.
          //   되살리려면 여기를 AD 로 되돌리기 전에 **다른 어휘**(색·형태)부터 정할 것.
          const SHOW_PATH = false;   // 제품·광고 프리셋 모두 끔
