...
// 코치 판 정본 — 검산 스크립트(check_content_fit)가 **같은 파일**을 읽는다. 사본 금지.
import { COACH_CFG } from './coach-cfg.js';
...
async function boot() {
  ...
  function sessionGazeTarget() {
    ...
    // ★ 스텝백(전체 재생 + 3조각)도 **같은 이유로 MAT** 다(08-07 실측).
    //   1인칭 눈높이 1.70m · fov 60° 에서 화면 하단에 닿는 전방거리는 **0.77m** 다. 즉 −40°에선
    //   보이는 바닥이 0.77~1.90m = **겨우 1.13m** 이고, 거기에 코치 판 + 타이틀 알약 + 판정 마크
    //   세 층이 다 들어가야 한다. 물리적으로 모자라서 뭘 어디로 옮겨도 둘은 겹친다 —
    //   실제로 유저 스샷에서 타이틀이 판정 토큰을 침범했고, 그 전엔 발자국이 화면 밖으로 잘렸다.
    //   자리를 만드는 유일한 방법이 시선을 내려 근거리를 화면에 들이는 것이다(유저: 고개를 더 기울이든).
    //   B1 이 이미 같은 판정으로 MAT 을 쓰고 있었다 — 스텝백만 빠져 있었을 뿐이다.
    if (/^(BK_B[1234]|BK_T1)$/.test(id)) return GAZE.MAT;
...
void main(){
  ...
  //   (2f21e8f 에서 걷어냈다가 유저 08-10 재확정으로 복원 — 1인칭 고정은 유저 요청이 아니었다)
  ...
  const session = new Session(scene, tokens, xbot, rig, st => {
    ...
    // ★ '세션 1인칭 고정'(2f21e8f)은 유저 요청이 아니었다(08-10 확인) — 스테이지별 시점 표 복원.
  ...
  // ★ 표는 src/coach-cfg.js 로 나갔다 — scripts/check_content_fit.mjs 가 **같은 파일**을 읽는다.
  //   여기 사본을 되살리지 말 것: 사본 시절 검산 쪽에 BK_A2 가 빠져 인물 139% 가 안 잡혔다.
  // ★ 코치 판 표는 **src/coach-cfg.js 하나뿐이다**(위 import). 여기 사본을 되살리지 말 것 —
  //   사본 시절 검산 쪽에 BK_A2 가 빠져서 인물 139% 인 스테이지가 회귀 검사에 한 번도 안 걸렸다.
  ...
  function setHotspot(co, vt) {
    ...
    // ★ 타원은 **다리 하나를 통째로** 물어야 한다(발목만 물면 붉은 점이지 강조가 아니다).
    //   세로가 훨씬 긴 건 발+정강이 길이 때문. 값은 렌더로 맞췄다(scripts/shot_guide_look.mjs).
    const r = S.b.r;
    U.uHotE.value.set(S.b.at[0], S.b.at[1] + r * 0.55, r * 1.35, r * 2.10);
...
#include <clipping_planes_pars_fragment>
  ...
  function stepSim(h) {
    ...
    if (session.active && (!session.isLive || session.stage === 'BK_C2')) {
      ...
      // ★ (08-12) legLock 은 **C1 만** — 크로스오버 데모(vm_crossover, 하체 고정 유저 확정) 전용이다.
      //   C2 까지 물려 있던 건 크로스오버 시절 유산: 08-10에 'C2 실전 봇이 스텝백 무브를 한다'로
      //   바뀐 뒤에도 다리가 스냅샷에 얼어 있었다(실측: 클립은 돌고 폭 0.62 고정 — 전시에서
      //   3인칭이 이상해 보인 원인). 두 확정이 충돌하면 나중 것(무브)이 이긴다.
      xbot.legLock = (session.stage || '') === 'BK_C1';
      ...
      // ★ footLock 금지 — 세 번째 확인(08-11): 힙을 고정해도 클립이 발을 옮기는 동안 래치가
      //   옛 자리를 물면 IK 가 무릎을 역방향으로 꺾는다(유저 스샷: 인간이 못 하는 관절).
      //   이 봇의 정답은 클립 순정 + _clampFeet 뿐이다. 마크·래치로 다리를 끌지 마라.
      xbot.footLock = false;
      ...
      // ★ 스텝백 = 클립이 아니라 **발자국 좌표로 다리를 푼다**(IK). 위상은 상체용으로만 돈다.
        ...
        // ★ (08-12 재실측, tmp_scan_box auto_cmu124_06 0.1s 타임라인) — **모으기는 클립에 있었다.**
        //   "2.4s+ = 레이업 도움닫기라 못 쓴다"는 절반만 맞다: 도움닫기 '직전' 2.4~2.55 에서
        //   낮은 자세(힙 0.89) 그대로 스탠스 1.11 → 0.33 으로 발을 싹 끌어모은다 — 이게 ③이다.
        //     클립 1.35  폭 0.20 · 힙 1.10   두 발 붙어 섬
        //     클립 1.80  폭 0.43            오른발 크로스 딛기        ← 가이드 ①(1.47)
        //     클립 2.30  폭 1.11 · 힙 0.91   왼발 뒤로 깊게 갈라 앉음   ← 가이드 ②(1.70)
        //     클립 2.55  폭 0.33 · 힙 0.89   모으기 완료(도움닫기 직전) ← 가이드 ③(2.10)
        //   구(舊) 역재생 사상([2.10,1.50],[2.20,1.45])은 폐기 — 거꾸로 감은 체중이동은 사람
        //   동작으로 안 읽힌다(유저: 찰진 몸놀림이 안 느껴진다, 전시에서 3인칭을 꺼 둠).
        // ★ 재생 정책 = **비트 트리거 + 자연 배속 + 끝 프레임 홀드** (마크 문법 '확 밟고 잠김'과 동일).
        //   가이드 비트를 지나면 그 구간을 클립 원속(1.0×)으로 재생하고 구간 끝에서 멈춘다.
        //   시간사상 보간은 학습 0.5배속에서 동작 자체를 절반 속도로 늘여 흐물거리게 만들었다 —
        //   '차차착'은 속도의 성질이지 위상의 성질이 아니다. 정속(C2)에선 비트 간격(0.23~0.4s)이
        //   구간 길이(0.25~0.5s)와 비슷해 사실상 연속 재생이 된다(창 1.2s ≈ 영상 창 1.15s).
        // [비트시각, 그 비트에 도달할 클립시각] — 클립축 단조증가. 모으기 비트는 SB_POSE ③(2.10)보다
        //   앞당긴 1.90 — 정속(C2)에선 영상 창이 1.15s 뿐이라 2.10에 출발하면 되감기 전에 못 끝낸다
        //   (실측: ct 2.1~2.2 에서 리와인드에 잘림 — 모으기가 매 렙 증발). 학습에선 마크 ③(1.70~2.10
        //   애니)과 봇 모으기(1.90~)가 반 박자 겹치는 정도라 자연스럽다(선생이 먼저 딱 끝낸다).
        const SB_SEGS = [[1.05, 1.35], [1.47, 1.80], [1.70, 2.30], [1.90, 2.55]];
        const vt = session._ikVt ?? 1.05;
        // 목표 클립시각 = 지나온 비트 중 마지막 것의 클립시각. 되감김(영상 리와인드)은 처음으로 리셋.
        // ★ 실전(C2, 정속)은 계단을 안 밟는다 — 비트 대기(_ikVt 지연 포함)가 렙당 ~0.3s를 먹어
        //   모으기가 되감기에 잘렸다(실측 ct 2.03~2.13 에서 컷). 정속에선 비트 간격(0.23~0.4s)이
        //   어차피 재생보다 짧아 계단이 무의미하다: 렙 시작부터 끝(2.55)까지 1.15×로 내리 재생.
        //   1.15× = 창(1.15s) 안에 1.2s 무브를 완결시키는 최소 배속이고, 실전은 원래 폭발적이다.
        const live = session.stage === 'BK_C2';
        let tgt = SB_SEGS[SB_SEGS.length - 1][1];
        if (!live) { tgt = SB_SEGS[0][1]; for (const [bv, bc] of SB_SEGS) if (vt >= bv - 1e-3) tgt = bc; }
        if (session._sbClipT == null || vt < (session._sbVtPrev ?? vt) - 0.35) session._sbClipT = SB_SEGS[0][1];
        session._sbVtPrev = vt;
        session._sbClipT = Math.min(tgt, session._sbClipT + h * (live ? 1.15 : 1));
        _phase = session._sbClipT;
  ...
  //    실제 동작은 전부 기존 컨트롤에 위임한다(로직 이중화 금지). 표시 상태만 여기서 동기화.
  {
    ...
    if (pp) {
      ...
      const productStart = () => {
        ...
        // ★ 부팅 compileAsync 가 도는 동안 세션 시작(setPack) 금지 — 폴링(checkMaterialsReady)이
        //   물고 있는 재질 목록이 바뀌면 three 내부 인터벌에서 'isReady of undefined' 로 터진다
        //   (간헐, 헤드리스 프로브 08-12 실측). 끝나면 아래 재무장이 이어서 시작한다.
        if (!window.__nwPrecompiled) { setTimeout(productStart, 300); return; }
  ...
  function renderDesignFrame() {
    ...
    if (floorShown) {
      ...
      // ★ 08-10 유저: 농구에서 UI 가 좌우 빔 커버리지 밖으로 삐져나온다 — 특히 시범(스텝백).
      //   구조적 원인: 폭을 콘 **중간**(dMid)에서 재는데 판은 near~far 를 다 덮는다.
      //   콘은 멀수록 넓어지므로 near 쪽 절반은 항상 콘보다 넓다. 비율(균일 스케일)과
      //   far 앵커는 건드리지 않고 농구 지면만 조금 줄인다 — 시범은 판이 near 까지
      //   내려와 더 심하니 한 단 더. 러닝은 유저 확인분이라 손대지 않는다.
      const sUni = laneW / fView.w
        * (/^BK_(T1|B[1234]|C2)$/.test(session.curStage?.id || '') ? 0.86
          : /^BK_/.test(session.curStage?.id || '') ? 0.93 : 1);
      ...
      // ★ far 여백 0.12 → 0.30m (유저 08-10: 타이틀 영역이 절대 잘리지 않게).
      //   0.12 는 **빔 끝 12cm 앞**이라 알약이 사실상 지평선에 붙어 있었다 — 그 자리는 원근이
      //   극단으로 눌려 알약이 실 두께의 렌즈로 찌그러지고(실측 스샷), 코트 라인 한 줄이 글자를
      //   가로지른다. 게다가 러닝 A3 는 알약 위끝이 **빔 밖 2.4cm** 로 진짜 잘리고 있었다
      //   (scripts/check_title_beam.mjs). 0.30 이면 전 스테이지가 빔 안 + 눌림에서 벗어난다.
      //   ⚠ far 앵커라는 성질은 그대로다 — 빨간 끝라인에서 알약까지의 거리가 전 스테이지 동일.
      //     이 값을 **줄이는**(빔 쪽으로 미는) 방향은 08-05 에 두 번 커버리지 이탈을 냈다.
      //   ★ 0.30 → 0.20 (유저 08-10b: 판이 확대되고 아래가 잘린다). 0.30 은 판을 0.18m
  ...
  window.__nwPrecompiled = true;   // productStart 게이트 해제 — 컴파일 중 setPack 금지
  loop();
}
