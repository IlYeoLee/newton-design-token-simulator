...
void main(){
  ...
  const COACH_CFG = {
    ...
    BK_B1: { src: 'bhandle_pp.mp4', cropOff: 0.0, cropScale: 1.0, w: 0.9, h: 0.9, fwd: 0.10 },
    BK_B2: { src: 'bhandle_pp.mp4', cropOff: 0.0, cropScale: 1.0, w: 0.9, h: 0.9, fwd: 0.10 },
    BK_B3: { src: 'bhandle_pp.mp4', cropOff: 0.0, cropScale: 1.0, w: 0.9, h: 0.9, fwd: 0.10 },
  ...
  function tickA1Coach() {
    ...
    const COACH_IDS = ['A1', 'A2', 'A3', 'BK_A1', 'BK_A2', 'BK_A3', 'BK_B1', 'BK_B2', 'BK_B3'];
    const activeId = COACH_IDS.find(id => id === st && !(/^(A2|A3|BK_A2|BK_A3|BK_B[123])$/.test(id) && session._followLatch)) || null;
...
#include <clipping_planes_pars_fragment>
  ...
  function stepSim(h) {
    ...
    if (session.active && !session.isLive) {
      ...
      const _watchWin = /^(A2|A3|BK_A[23]|BK_B[123])$/.test(session.stage || '') && !session._followLatch;   // 훈련 3단계 전부 관찰5초→따라하기
