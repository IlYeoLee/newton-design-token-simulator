...
void main(){
  ...
  function clipExposure(v, st) {
    ...
    try {
      ...
      if (n > 12) st._exp = sum / n;               // 표본이 너무 적으면 직전 값 유지
  ...
  function tickA1Coach() {
    ...
    for (const id of COACH_IDS) {
      ...
      if (id === activeId) {
        ...
        setPersonUniforms(co.plane.material.uniforms, 0.98, 0, clipExposure(co.video, co));   // 대역·노출 모두 데모 판과 동일
...
#include <clipping_planes_pars_fragment>
        ...
        void main(){
          ...
          float mEro = smoothstep(0.16, 0.52, m);   // 침식 완화 — 골대 림 등 얇은 구조 보존(유저)
          // ── 안쪽으로만 페더 ────────────────────────────────────────────────────
          //   실루엣을 부드럽게 하려고 마스크를 그냥 흐리면 **바깥으로** 번져 크로마 저신뢰
          //   프린지(초록)가 딸려 들어온다 — 인물을 두른 회색 테두리가 그것이었다(유저 스샷).
          //   대신 링에서 **최솟값**을 취해 확실한 영역으로 깎고(침식), 그 침식폭 안에서만
          //   알파를 0→1 로 올린다. 페더가 원 실루엣 **안쪽**에만 살므로 바깥이 새지 않는다.
          float mErode = 1.0;
          for (int k = 0; k < 8; k++) { float a = 0.7854 * float(k);
            mErode = min(mErode, pmask(uv + vec2(cos(a), sin(a)) * 0.008)); }
          mEro *= smoothstep(0.04, 0.80, mErode);   // 침식폭이 크면 손·글러브가 갉아먹힌다
  ...
  function renderDemoPanel() {
    ...
    setPersonUniforms(PU, wallMode ? 0.98 : 0.72, 0, clipExposure(demoVideo, demoPanel));
