  ...
  function stepSim(h) {
    ...
    if (session.active && (!session.isLive || session.stage === 'BK_C2')) {
      ...
      // ★ 스텝백 = 클립이 아니라 **발자국 좌표로 다리를 푼다**(IK). 위상은 상체용으로만 돈다.
        ...
        // ★ 가이드 시각(SB_POSE) → 클립 시각은 **구간별**이다. 하나의 선형 사상으로는 못 맞는다
        //   (유저 08-10: 오른발 크로스만 비슷하고 나머지는 발자국을 아예 안 따른다 — 맞다).
        //   왜 안 맞았나: 선형 사상은 가이드 [1.05,2.20] 을 클립 [1.5,2.3] 에 통째로 늘렸는데,
        //   클립 [1.5,2.3] 은 **처음부터 끝까지 벌어지기만 하는 한 덩어리**다. 그래서 마지막
        //   가이드 키(③ 모으기)에서 봇은 정반대로 **최대로 벌어져** 있었다.
        //   실측(scripts 임시 스캔, 발 월드 좌표 · 스탠스 폭):
        //     클립 1.35  폭 0.191  두 발 붙어 섬 · 오른발이 막 뜬다
        //     클립 1.75  폭 0.348  오른발이 옮겨져 놓인다        ← 가이드 ①(1.47) 과 같은 사건
        //     클립 2.25  폭 1.068  깊게 갈라져 앉는다(힙 0.98→0.80) ← 가이드 ②(1.70)
        //     클립 2.3+        레이업 도움닫기 — 쓸 수 없다
        //   ③ '모으기'는 클립에 없다. 만들지 않는다 — **같은 구간을 거꾸로 돌린다**:
        //   벌어지며 앉는 동작의 역재생이 곧 모으며 일어서는 동작이고, 스텝백은 원래 모아서
        //   일어서며 쏜다. 사상표가 감소하면 재생이 역방향이 된다(코드는 그대로).
        const MAP = [[1.05, 1.35], [1.47, 1.75], [1.70, 2.25], [2.10, 1.50], [2.20, 1.45]];
        const v0 = Math.max(MAP[0][0], Math.min(MAP[MAP.length - 1][0], session._ikVt ?? MAP[0][0]));
        let mi = 0; while (mi < MAP.length - 2 && v0 > MAP[mi + 1][0]) mi++;
        const [a0, c0] = MAP[mi], [a1, c1] = MAP[mi + 1];
        _phase = c0 + (v0 - a0) / Math.max(1e-3, a1 - a0) * (c1 - c0);
