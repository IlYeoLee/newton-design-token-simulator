  ...
  function stepSim(h) {
    ...
    if (session.active && (!session.isLive || session.stage === 'BK_C2')) {
      ...
      // ★ 스텝백 = 클립이 아니라 **발자국 좌표로 다리를 푼다**(IK). 위상은 상체용으로만 돈다.
        ...
        const SB_SEGS = [[1.05, 1.35], [1.47, 1.80], [1.70, 2.30], [2.10, 2.55]];   // [비트시각, 그 비트에 도달할 클립시각] — 클립축 단조증가
        const vt = session._ikVt ?? 1.05;
        // 목표 클립시각 = 지나온 비트 중 마지막 것의 클립시각. 되감김(영상 리와인드)은 처음으로 리셋.
        let tgt = SB_SEGS[0][1];
        for (const [bv, bc] of SB_SEGS) if (vt >= bv - 1e-3) tgt = bc;
        if (session._sbClipT == null || vt < (session._sbVtPrev ?? vt) - 0.35) session._sbClipT = SB_SEGS[0][1];
        session._sbVtPrev = vt;
        session._sbClipT = Math.min(tgt, session._sbClipT + h);   // 원속 전진, 목표에서 정지
