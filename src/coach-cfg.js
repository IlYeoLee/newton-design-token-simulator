...
export const COACH_CFG = {
  ...
  // ★ 08-10 — 빔 커버리지 안으로 되돌린다(유저: 옆구리풀기가 커지면서 아래로 삐져나왔다).
  //   확대(e61bf2a)가 인물 실높이를 기준의 130~139% 로 올렸고, 그만큼 판이 커져 근단이
  //   빔 밖으로 나갔다(실측 BK_A1 근단 여유 −0.10m · 좌우 −0.143m). 크기는 이 시스템에서
  //   자유 변수가 아니다 — 인물 실높이 0.55m 규약(파일 머리말)이 곧 빔 안에 드는 조건이다.
  //   배율은 scripts/check_content_fit.mjs 가 계산해 준 값 그대로(종횡비 유지).
  A3: { src: 'ready-view/assets/sean_highknee.webm', cropOff: 0.0, cropScale: 1.0, w: 0.632, h: 0.632, fwd: -0.04, ph: 0.87 },   // 하이니 — 캡슐 카드 아래로(머리 겹침 방지, 캡슐 시스템)
  ...
  // ★ 08-10 유저: 농구 스트레칭 인물이 너무 작다(0.512m · 전 스테이지 최소)인데 짝인 BK_A2 는
  //   0.765m 라 나란히 1.5배 차이로 읽혔다 → 같은 배율로 올려 맞췄다.
  //   ★★ 그리고 **둘 다 0.55m 로 내렸다**(같은 날, 빔 초과 신고). 유저가 고친 건 '짝이 안 맞는
  //     것'이었고 그건 지금도 맞다 — BK_A1·BK_A2·A3 가 전부 0.55m 로 같다. 절대 크기만 규약으로
  //     되돌린 것이다. 다시 키우고 싶으면 크기가 아니라 **빔을 넓히거나 fwd** 로 풀 것.
  BK_A1: { src: 'ready-view/assets/bk_sidebend_pp.webm', cropOff: 0.0, cropScale: 1.0, w: 0.666, h: 0.688, fwd: -0.02, ph: 0.80, tone: 0.045 },   // 옆구리 스트레치
  BK_A2: { src: 'ready-view/assets/bk_highknee.webm', cropOff: 0.0, cropScale: 1.0, w: 0.647, h: 0.647, fwd: -0.18, ph: 0.85 },   // 무릎 들기
  ...
  // ★ zoom 0.77 (유저 08-10: 드리블할 때 좌우가 회색으로 잘린다). 소스가 잘린 게 아니다 —
  //   실측(720×1280 · 그린 제외 바운딩박스): 피사체 x 22~665 = uv 0.031~0.924 로 프레임 안에
  //   다 들어와 있다. 자른 건 **refEdge 4변 페이드**다: 좌우 각 14%(uv 0~0.14, 0.86~1)를 지운다.
  //   공이 크게 나가는 프레임(t=0.6)에서 왼손·공이 uv 0.031 이라 89% 지워졌다.
  //   uZoom 은 정확히 이 경우를 위한 손잡이다(A2 가 이미 쓴다) — 판 안에서 인물을 줄여
  //   페이드가 **여백만** 먹게 한다. 필요한 값: 0.5 − 0.4694·z ≥ 0.14 → z ≤ 0.767.
  //   w·h 는 1/0.77 로 같이 키운다 — 인물 실높이 = h × ph × zoom 규약(파일 머리말)을 지켜야
  //   0.55m 가 유지된다. 안 키우면 드리블만 23% 작은 사람이 된다.
  BK_B1: { src: 'bhandle_pp.mp4', cropOff: 0.0, cropScale: 1.0, w: 0.647, h: 1.152, fwd: -0.06, zoom: 0.77, ph: 0.62 },   // 워밍업 위계와 크기·거리 통일(유저 #75) — 9:16 유지
