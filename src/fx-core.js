...
vec4 markState(vec2 uv, float state, float prog, float strong, float t){
  ...
  if (uShape > 0.5 && uImp > 0.001) {
    ...
    // ★★ 하프톤은 **필과 같은 온도 창 안에서만** 움직인다(유저: 빨강 위에 흰색, 흰색 위에 빨강
    //   이렇게 안 되게). 전엔 정점에서 palPick(uImpDotCol=순백)으로 **창 밖으로 튀어나갔다** —
    //   그래서 붉은 필에서는 '빨강 위 흰 점'(밝은 점), 옅은 고스트 필에서는 '연주황 점'(어두운 점)이
    //   되어 상태마다 **극성이 뒤집혔다.** 점이 밝은지 어두운지가 상태마다 다르면 같은 기호로 안 읽힌다.
    //   규칙: 저압 = 창의 밝은 끝 · 고압 = 창 안에서 뜨거운 쪽으로 65%까지. **창을 절대 안 나간다.**
    //   → 필이 빨갛든 옅든 점은 늘 '필보다 조금 밝은 같은 계열'이다. 극성이 구조적으로 못 뒤집힌다.
    float dwHi = (uTHi > 0.0001) ? uTHi : 0.86;   // 창의 밝은 끝(샌드 쪽)
    float dwLo = (uTHi > 0.0001) ? uTLo : 0.56;   // 창의 뜨거운 끝(레드 쪽)
    vec3 dotC = lut(mix(dwHi, mix(dwHi, dwLo, 0.65), dotT));
