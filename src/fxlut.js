...
export const FXP = {
  ...
  person: { blur: 0, glow: 0, flow: 0, decay: 0, detail: 0.42, sweep: 0, grain: 0, tone: 1 },
  ...
  numSrc: 'glyph',
  // 빔 글로우 — 시작 화면·타이머·프로그레스 배경의 그 덩어리. 값을 코드에 박고 렌더해서
  // 확인하는 방식으로는 못 맞춘다(다섯 번 시도해 다섯 번 다 한 문제를 다른 문제로 바꿨다:
  // 청록 과다 → 칙칙 → 원반 회귀 → 주황 원반 → 빨강 원반). 반경·알파·falloff·색이 서로
  // 얽혀 있어 눈으로 돌려가며 찾아야 하는 종류다. 그래서 손잡이로 뺀다.
  //   scale    호출부 기본 반경 대비 배율
  //   core     중심 알파
