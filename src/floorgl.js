...
export class FloorGL {
  ...
  _headBox(title, step, y = LAYOUT.HEAD.y, o = {}) {
    ...
    //   ★ 타이틀이 접힌 타이머 단독(원형 알약)에서도 이 보정이 걸려 링이 원 중심에서
    //     왼쪽으로 밀렸다(유저 08-12 스샷: 스트라이드·인터벌 링이 상하좌우 여백이 안 같다).
    //     보정의 근거가 '왼쪽=옅은 트랙 vs 오른쪽=솔리드 글자' 비대칭인데, 타이머 단독은
    //     양쪽이 다 링 트랙이라 대칭이다 — 타이틀이 있을 때만 보정한다.
    const optL = RR * 0.12 * ringK * (tw > 0 ? 1 : 0);
