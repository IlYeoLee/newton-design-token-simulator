...
//   ★ 타이틀 크기는 **시작화면 것을 그대로 유지**한다(유저) — 같은 물건이 줄어드는 것인데
  ...
  _paint_ready() {
    ...
    //   **자리를 비켜주는 안무**가 원래 의도다(6cdb627): 숫자가 주인공이 되면 배경이 내려간다.
    ...
    //   숫자 하나뿐이라 광이 화면의 배경이자 주인공이다. p2 에 비례해 올려 두 뷰를 갈랐다.
    //   러닝·농구 공통(둘 다 이 페인터를 쓴다).
    // ★ saturate 제거(유저: 처음 이후는 이 컬러 그대로) — 채도를 1.55 까지 밀면 피그마
    //   342:3082 의 그라디언트(FA3030 · FE6E3C · FEC389 · D1FEFF)가 다른 색으로 보인다.
    //   에셋 색을 그대로 살리고 **밝기만** 조금 올린다(광량은 유저가 원한 '쨍함' 쪽).
    if (p2 > 0.01) ctx.filter = `brightness(${(1 + .22 * p2).toFixed(3)})`;
