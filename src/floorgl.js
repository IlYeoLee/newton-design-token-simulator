...
export function rollNum(ctx, target, t, delay, cd, x, y, size, o = {}) {
  ...
  // ★ PADX 를 8 로 고정하면 **오른쪽 획이 잘린다**(유저 08-06: 러닝·농구 둘 다, 웹에서도).
  //   오프셋 캔버스 폭을 `total`(진행폭 합) + PADX*2 로 잡는데, 도트 폰트는 잉크가 진행폭보다
  //   넓다. 게다가 호출부가 자간을 음수로 주면(벽 스코어 ls:-8, 5.0km ls:-14) 진행폭이 더 줄어
  //   마지막 자리 잉크가 캔버스 밖으로 나간다. 아래 자리별 클립창도 잉크 경계(+6)를 쓰므로
  //   여백이 그보다 좁으면 거기서 다시 깎인다. 크기에 비례해 넉넉히 준다 — 비용은 캔버스 몇 px 뿐.
  const WH = size * 0.92, PADX = Math.ceil(size * 0.35) + 8, PADY = Math.ceil(size * 0.10);
  const px0 = px;
...
export const TOK = {
  ...
  // ★ 200 → **340** (08-07 실측). CLAUDE.md §3 이 이미 "위쪽 289px 은 투사면 밖(TOK.headY 340)"
  //   이라고 정본을 적어 뒀는데 코드만 200 에 남아 있었다 — 문서·코드가 갈린 자리다.
  //   실측(BK_B4·BK_C2 공통, 대지 1600×2670): 알약 y200 h316 → **위 89px(28.2%)이 투사면 밖**.
  //   그 28% 는 빔 페더에 먹혀 그릇(저알파 글래스)이 사라지고 고대비 글자·링만 남는다
  //   = 유저 "다른 뷰는 알약으로 보이는데 이 뷰는 왜 이렇게 투명하냐". 두 뷰가 화소까지
  //   같은 값으로 그려지고 있었으므로 원인은 조판이 아니라 **깊이**였다.
  //   340 이면 알약 340~656 이 전부 투사면 안이다(289 아래).
  headY: 340,        // 헤더 알약 상단 — 투사 far 경계(289) + safePad
  ...
  gapHP: 96,    // 132 → 96 (라벨 밴드에 30 회수)        // 알약 ↔ 진행 아크
  ...
  //   SPM 숫자는 172px = 1.71° 로 **이미 알약 타이틀의 2.4배**다. 위계는 거기서 이미 다
  //   말해지고 있었고, 알약까지 줄이니 위계를 두 번 준 꼴이 됐다 — 러닝만 다른 시스템처럼
  //   보였다(유저: 농구는 안정감 있는데 러닝은 아직 너무 작다, 공통이 아닌 느낌).
...
const CAPS = {
  ...
  BK_B1: { watch: 'video', adv: 'reps', reps: 16 },    // 드리블 16회 = 반복 → 관찰 없음 (session BK_B1_TOTAL 과 같은 값)
...
function buildScene(stage, p) {
  ...
  //   헤더 = 타이머 + 타이틀, 아래 = 수행(SPM) 하나. 러닝에 2단 구조는 두지 않는다.
  if (isP) col.push(node('train-row', { type: 'trainRow' }));
...
export class FloorGL {
  ...
  _headBox(title, step, y = LAYOUT.HEAD.y, o = {}) {
    ...
    //   시각적 경계가 기하학적 경계보다 44px(=0.25·RR) 안쪽에서 시작하는 반면, 오른쪽 끝은
    //   솔리드 흰 글자라 마지막 화소까지 꽉 찬다. 실측 광학 여백 109 vs 67 — 42px 차이.
    //   그 절반만큼 내용을 왼쪽으로 당긴다. 링이 없으면(ringK 0) 보정도 없다.
    const optL = RR * 0.12 * ringK;
    const x = CX - w / 2, x0 = x + (w - inner) / 2 - optL;
    // ★ **HUG 는 폭에만.** 높이는 상수다.
...
//   ★ 타이틀 크기는 **시작화면 것을 그대로 유지**한다(유저) — 같은 물건이 줄어드는 것인데
  ...
  _liveRow(n, y) {
    // ★ 2단(SPM + Pace) → **SPM 단독**(유저: 2단 구조 있으면 다 제거). Pace 는 SPM 에서
  ...
  _paint_ready() {
    ...
    //   두 에셋을 p2 로 크로스페이드한다(보간이 아니라 **교대** — 색이 섞이면 어느 쪽도 아니게 된다).
    // ★ 페이즈2 뉴턴 광 = **알약(_paint_capsule)이 쓰는 CAP_GLOWS 그대로**(유저 08-06:
    //   "지금 여기 뒤에 깔린 컬러랑 같잖아 · 블렌드 모드 체크하라고 · 같은 거 주면 될 일인데").
    //   같은 에셋(glow-ell-newton.svg)인데 여기만 **다른 지오메트리(1300×871, 아래로 내려앉음)**
    //   + brightness 필터로 그리고 있었다. 그래서 알약은 레드(채도 63%/색상각 5.8°)인데
    //   시작화면은 탄색(58%/20°)으로 갈렸다 — 색 문제가 아니라 **같은 걸 다르게 그린** 문제.
    //   이제 팩 면 ↔ 뉴턴 면이 **같은 박스(1288.36×1709.36)에서 자리만 교대**한다.
    //   ※ 앞서 흰 3겹을 알파로 죽이던 보정(HLK)은 폐기 — 원인이 흰 겹이 아니었다.
    ...
    //   brightness 는 채도를 **내린다**. 원인은 지오메트리였고 위에서 고쳤다.
    //   밝기를 올리면 명도가 올라가면서 **채도가 내려간다**(FA3030 이 탄색으로 읽힌 두 번째 이유).
    //   알약은 필터 없이 그린다 — 같은 불이면 같은 조건으로 그려야 같은 색이 나온다.
    ...
    // ⑥ 발 그래픽은 **여기서 안 그린다**(유저 08-05, 재확인 08-06). 캔버스 신발 SVG 를
    //   되살렸다가 지운다 — 발자국은 이미 3D FootMark 토큰(session.js readyFeet, tap2 어포던스)이
    //   갖고 있다. 룩(도트·풀·글로우)이 mark-look.json 정본을 따르는 쪽이 그 토큰이고,
    //   캔버스에 사제 신발을 또 그리면 같은 물건이 두 벌이 된다.
    //   ※ 대신 **표시 구간**을 CTA 와 맞춰야 한다 — session.js _readyFeetTick 참조(순서 재배치 반영).
  ...
  _paint_capsule(cfg) {
    ...
    // ★ **READY 직후에만** 시작화면 캡슐에서 줄어든다(유저 08-06: 프리뷰가 더 이상 큰
    //   컴포넌트도 아닌데 왜 굳이 큰 것에서 과하게 줄어드나).
    //   원래 이유는 유효했다 — 시작화면 세로 캡슐(1018×1491)이 화면에 떠 있다가 탭하면 그게
    //   줄어들어 알약이 된다: '새 물건이 나타난' 게 아니라 '같은 물건이 계속 있다'로 읽힌다.
    //   그런데 t 는 스테이지마다 0 으로 리셋되므로 **모든 캡슐 스테이지**가 이 모션을 했다.
    //   A2·A3·BK_B3 에 들어갈 때도 화면에 없는 1018×1491 유령에서 줄어든 것이다.
    //   알약이 956×244 로 작아진 지금은 세로 6배 축소라 더 과하다.
    //   READY 다음에 오는 건 A1(러닝)·BK_A1(농구) 뿐이다 — 거기만 남긴다.
    const fromReady = /^(A1|BK_A1)$/.test(this.stage || '');
    const en = eOut(clamp01(t / .9));
    //   그 외 스테이지는 **제자리 스케일 인**(0.94 → 1) — 칼럼 조판(_paint)이 쓰는 것과 같은 규약.
    //   ★ 캔버스 변환(ctx.scale)이 아니라 **기하로만** 준다. save/restore 짝을 놓치면 이 함수
    //     뒤쪽(링·타이틀·아크)까지 통째로 딸려 들어간다. 상자에서 파생되는 구조라 이걸로 충분하다.
    ...
    const RR = RRp;
    // ★ 광학 보정 — _headBox 의 x0 와 같은 값을 쓴다(이 경로는 모프 중 x·w 가 달라 x0 를
    //   그대로 못 받는다). 왜 필요한지는 _headBox 쪽 주석 참고: 왼쪽 끝이 링의 옅은 트랙이라
    //   시각 경계가 안쪽 0.25·RR 에서 시작하는데 오른쪽 끝은 솔리드 글자라 꽉 찬다.
    const x0p = x + (w - INNER) / 2 - RR * 0.12 * ringK;
