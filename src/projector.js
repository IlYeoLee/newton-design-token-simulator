...
export class ProjectorRig {
  constructor(scene, xbot) {
    ...
    this.fpFar = 2.5;     // 끝 거리 (m) — 2.0 은 1인칭에서 고개 40°+ 존만 비췄다(유저). 원단 확장으로 시선 부담 완화
    this.wallW = 2.6;     // 벽면 투사 가로 (m) — 자취방 스케일 (유저 확정, 16:10)
    this.wallH = 1.63;    // 벽면 투사 세로 (m) — 2.6×1.63 = 16:10
