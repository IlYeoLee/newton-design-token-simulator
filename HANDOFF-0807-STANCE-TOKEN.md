...
- **알파 추출이 아직 없다.** `export_ui.mjs` 는 세션을 안 돌려 셋업 막을 재현 못 하고,
  `export_live_ui.mjs` 에는 띠 크롭(1816×510) 옵션이 없다. 토큰을 실제로 뽑으려면 이걸 붙여야 한다.
- **발 크기가 미결.** 랩 슬라이더는 미리보기 배율일 뿐이고 정본은 `FOOT_LEN_M = 0.30` 이다.
  유저가 "바닥 타일이 30cm"라고 했는데, 임시 배경은 월드 스케일이 없어 비교가 성립하지 않는다.
  지면 모드 + 배경 스케일 슬라이더로 타일을 30cm 에 맞춘 뒤 판단해야 한다.
