...
## 10. 인물 룩 — 몸통 대역과 부위 강조 (2026-08-07 이어붙임)

유저: "몸통이 너무 주황이라 붉게 되어야 할 강조가 안 된다 → 진짜 하양에 가까운 코랄로,
강조가 다리면 새빨간~주황으로 은은하게."

### 원인은 램프가 아니라 **대역의 위치**였다

`look2Ramp` 를 t 0~1 로 뽑으면:

```
0.02 #fff5f5   0.08 #ffd7d7   0.13 #fdbfbf   ← 하양에 가까운 맑은 코랄
0.20 #fb5959   0.25 #FA3030                  ← 새빨강
0.36 #fc3a1a   0.50 #ff4000                  ← 붉은-주황
0.70 #ff7e4b   1.00 #ff3300                  ← 종전 몸통이 살던 곳
```

종전 몸통 밴드는 `0.3 + 0.7*x` = **0.30~1.00 고정**. 붉은 자리도, 흰 자리도 안 지난다.
강조를 아무리 세게 줘도 더 주황이 될 뿐이었다.

### 노브 둘

| 유니폼 | FXP 키 | 기본(=종전과 픽셀 동일) | 뜻 |
|---|---|---|---|
| `uPBandLo` / `uPBandHi` | `person.bandLo/bandHi` | 0.30 / 1.00 | 몸통이 램프의 **어느 구간**을 쓰나 |
| `uPEmph` `uPEmphT` `uPEmphY0/Y1` `uPEmphSoft` | `person.emph.{k,t,y0,y1,soft}` | k 0 | 띠 안에서 t 를 `uPEmphT` 로 **끌어내린다** |

- 강조는 `mix(t, uPEmphT, e*k)` 다. **k 를 1 로 못 박으면 단색**이 되고 '은은한 그라디언트'가 죽는다.
- 얼굴은 제외(`face`) — 이미 저열로 눌러 놨는데 또 만지면 이목구비 소거가 풀려 반점이 돌아온다.
- 다리 부스트(`+0.115`)는 대역 폭에 비례시켰다. 폭 0.7 시절 절대값이라 좁은 대역에서 상한을 밀었다.
- `uv.y` 기준이라 **0 = 아래(발)**.

### 제안값 — personlab-live 버튼 하나

```
bandLo 0.02 · bandHi 0.13 · emph k 0.70 · t 0.36 · y 0~0.42 · soft 0.16
```
...
| | 종전 | 제안값 |
|---|---|---|
| 복싱·벽 | #ff562c · 채도 0.89 | **#ff8b8a · 0.459** |
| 러닝·바닥 | #ec4119 · 0.87 | **#e96361 · 0.584** |
| 농구·바닥 | #fd6840 · 0.75 | **#fd7a77 · 0.530** |

> 더 하양으로 가려면 `bandHi` 를 더 내리거나 `uPSat`(채도 부스트 1.28)을 같이 낮춘다.
> 제안값은 정본이 아니다 — 슬라이더로 고른 뒤 박을 것.

---

## 11. 미결 — 농구 1인칭에서 코치 판 아래가 잘린다

유저: "농구 1인칭일 때 너무 가린다, 아래가 너무 잘려서 보이지도 않아."

`BK_READY` 1인칭 캡처에서 **커리 판의 다리·발이 화면 하단 밖**이다. 하단 버튼 바가 그 위에 겹쳐
가림이 더 커진다. 드리블 매트가 밟은 것과 **같은 종류의 문제**이고 선택지도 같다:

- ㉮ 시선을 더 내린다 → 근거리가 들어오지만 판 위쪽(타이틀 'Curry's Step Back')이 나간다
- ㉯ 판을 더 멀리 두거나 작게 → 다 들어오지만 인물이 작아진다
- ㉰ 판을 위로 올린다(지면 기준 y 오프셋) → 원근이 어긋나 '떠 있는' 느낌이 날 위험

카메라를 임의로 안 건드렸다. **유저 결정 대기.** 정하면 `sessionGazeTarget`(㉮) 또는
코치 판 배치(㉯·㉰) 한 곳만 만지면 된다.
