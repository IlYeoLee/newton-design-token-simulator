...
## 레퍼런스 이미지 (docs/character-refs/)

- **`base_framing.png`** (1448×1086) — **베이스 프레이밍 규격**. image→video 시작 프레임.
  인물이 프레임 중앙, 상하좌우 여백 넉넉 → 잽·스텝·훅 움직여도 신체 안 잘림. 그린스크린.
- **`face_front.png` / `face_side.png` / `face_back.png`** — 얼굴·스타일 일관 레퍼런스
  (동일 인물 유지용 identity/style reference).

## 공통 스펙 (10개 전부 지켜야 함)

- **배경**: **순수 초록 그린스크린** (RGB ~12,219,15). 파이프라인이 초록 우세도로 인물 추출 —
  배경에 초록 그림자·반사·의상 초록 금지. 배경이 초록이 아니면 벽에 안 입혀짐.
- **프레이밍**: `base_framing.png` 규격 유지 — 전신, 정면, 카메라 고정(팬/줌 없음),
  **움직임 중에도 발끝~머리·양손 전부 프레임 안**(신체 잘림 금지 = base_framing 여백이 보장).
- **아이덴티티**: `base_framing.png` = 프레이밍 앵커, `face_*.png` = 얼굴·스타일 일관 레퍼런스.
- **의상**: 검정 민소매 + 검정 트렁크 + 검정 글러브 (레퍼런스 그대로). 초록·형광 금지.
- **루프성(A·B·READY)**: 시작·끝 포즈가 같아 매끄럽게 반복되게. 3~5초.
- **C 클립**: 6초, 동적 OK.
- **fps**: 30. 16:9 캔버스 중앙 배치.
