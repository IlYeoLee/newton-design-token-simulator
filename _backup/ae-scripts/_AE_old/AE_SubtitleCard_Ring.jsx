...
(function () {
    ...
    var CFG = {
        ...
        ringWidth:   Math.round(H * 0.0018),    // 링 두께
        ringColor:   [1, 1, 1],
        ringOpacity: 65,                        // %

        // 1행(아바타+링+파형) 이 아래에서 올라오며 자리잡는 동작
        row1Delay:   0.05,                      // 시작 시각(초)
        row1Dur:     0.65,                      // 올라오는 시간(초)
        row1Rise:    Math.round(H * 0.013),     // 올라오는 거리(px)
        ...
        // ---- 간격 ----
        gapRowCard:  Math.round(H * 0.007),     // 아바타·파형 ~ 카드 위쪽. 줄이면 더 내려온다
    ...
    var row1H = Math.max((AVR + CFG.ringGap) * 2, waveH);   // 링 포함 1행 높이
    var enH   = CFG.enSize * 0.72;              // 대문자 높이 (베이스라인 위)
    var krH   = CFG.krSize * 0.72;

    // 카드 안에는 텍스트 두 줄만. 아바타와 파형은 카드 바깥 위쪽에 뜬다.
    var CH = Math.round(CFG.padY * 2 + enH + CFG.gapEnKr + krH);

    var cx      = comp.width / 2;
    var cardCY  = Math.round(H * (1 - CFG.bottomPct) - CH / 2);
    var cardTop = cardCY - CH / 2;

    var enY = Math.round(cardTop + CFG.padY + enH);         // 영문 베이스라인
    var krY = Math.round(enY + CFG.gapEnKr + krH);          // 한글 베이스라인

    var row1CY = Math.round(cardTop - CFG.gapRowCard - row1H / 2);  // 카드 위, 1행 중심
    ...
    /* 사진에 직접 스트로크를 못 준다 — 마스크 경계에는 선이 안 생긴다.
       ...
       ringGap 만큼 띄워야 사진에 딱 붙지 않고 여유 있어 보인다. */
    var ring = null;
    if (CFG.ringOn && avatarSrc) {
        ring = tpl.layers.addShape();
    ...
    /* 투명도는 아래 ROW1 널이 전담한다. 여기선 크기만.
       오버슈트를 뺐다 — 1행은 튕기지 않고 자리를 잡아야 안정감이 산다. */
    ...
    // ---- 1행 그룹: 아바타 + 링 + 파형이 함께 올라온다 --------------------
    /* 셋을 널 하나에 묶어야 따로 놀지 않고 한 덩어리로 자리를 잡는다.
       오버슈트 없이 순수 이지만 써서 튕김 없이 안착 = 안정감.
       부모는 위치만 물려주고 투명도는 안 물려주므로 표현식으로 잇는다. */
    var row1 = tpl.layers.addNull(CFG.dur);
    row1.name = "ROW1";
    row1.enabled = false;
    row1.label = 11;

    var r1xf = row1.property("ADBE Transform Group");
    var r1P  = r1xf.property("ADBE Position");
    var r1O  = r1xf.property("ADBE Opacity");

    r1P.setValueAtTime(CFG.row1Delay,                 [cx, row1CY + CFG.row1Rise]);
    r1P.setValueAtTime(CFG.row1Delay + CFG.row1Dur,   [cx, row1CY]);
    ease(r1P, 1); ease(r1P, 2);

    r1O.setValueAtTime(CFG.row1Delay, 0);
    r1O.setValueAtTime(CFG.row1Delay + CFG.row1Dur * 0.75, 100);
    ease(r1O, 1); ease(r1O, 2);

    var r1Kids = [];
    if (avLay) r1Kids.push(avLay);
    if (ring)  r1Kids.push(ring);
    r1Kids.push(wave);

    var r1Op = 'try { value * parent.transform.opacity / 100 } catch (e) { value }';
    for (var q = 0; q < r1Kids.length; q++) {
        r1Kids[q].parent = row1;
        r1Kids[q].property("ADBE Transform Group")
                 .property("ADBE Opacity").expression = r1Op;
    }
