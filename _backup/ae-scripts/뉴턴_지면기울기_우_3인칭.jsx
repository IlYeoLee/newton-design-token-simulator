// 뉴턴 — 지면 기울기 맞추기 · 우 (3인칭 드리블 컷)
//
//   선택한 레이어를 **그 자리 그대로** 두고, 지면 원근에 맞는 사다리꼴로만 눕힌다.
//   위치·크기·회전은 건드리지 않는다. 코너핀만 쓴다.
//
//   ── 각도의 출처 (감으로 정하지 않았다) ──────────────────────────────────────
//   소스 `PRESS ON _ #3-5.mov` (4096×2160) 의 코트 타일 이음매를 실측했다.
//   카메라 정면으로 뻗는 지면 세로줄의 화면상 좌우 간격 s(y) 는 s = k·(y − y_h) 로
//   소실선에서 선형으로 벌어진다. 인물을 피해 화면 좌측 지면(x 60~1250) 을
//   y=1650~2050 의 9밴드로 재고 로버스트 회귀:
//
//       1.0s  y_h = 1026 px = 0.4748 H   (R² 0.981, n 9)
//       3.0s  y_h = 1040 px = 0.4815 H   (R² 0.981, n 8)
//       5.0s  y_h = 1026 px = 0.4748 H   (R² 0.981, n 9)
//      13.0s  y_h = 1030 px = 0.4769 H   (R² 0.986, n 9)
//       → 채택 0.477 H
//       (8.0s·10.0s 는 0.421 H 가 나왔으나 R² 0.939 로 낮아 버렸다)
//
//   지평선이 프레임 거의 정중앙이다 = 카메라가 지면과 거의 수평이다. 좌 컷(1인칭,
//   −1.17 H)과 정반대이고, 그래서 같은 UI 라도 두 화면의 눕는 정도가 크게 다르다.
//   초점거리는 몰라도 되고 쓰지도 않았다 — 사다리꼴 모양은 소실점 하나로 결정된다.
//
//   ★ 판의 좌우 방향은 카메라 정면 기준으로 놓는다(소실점 x = 프레임 중앙).
//     코트 라인 방향이 아니다 — 뉴턴_지면합성.jsx 의 시뮬 투영값도 좌우대칭이다.
//   ★ alert 를 쓰지 않는다. 결과는 바탕화면 '뉴턴_지면기울기_우_결과.txt'.

(function () {
    var CLIP_HINT = '#3-5';        // 배경 클립을 자동으로 찾을 때 쓰는 이름 조각
    var YH_RATIO  = 0.477;         // 소실선 y ÷ 소스 프레임 높이 (실측값)
    ...
    function save() {
        try {
            var f = new File(Folder.desktop.fsName + '/뉴턴_지면기울기_우_결과.txt');
            f.encoding = 'UTF-8'; f.open('w'); f.write(L.join('\r\n')); f.close();
        } catch (e) {}
    }
    function r(n) { return Math.round(n * 10) / 10; }

    log('=== 지면 기울기 · 우 (3인칭) · 소실선비 ' + YH_RATIO + ' ===');
